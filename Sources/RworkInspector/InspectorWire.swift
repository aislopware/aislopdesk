...
/// `RworkProtocol` rather than the message type itself, so the two protocols share a
...
///
/// Frame layout (mirrors `RworkProtocol` framing style, separate namespace):
/// ```
/// [ UInt32 BE payloadLength ][ UInt8 typeTag ][ body... ]
/// ```
/// `payloadLength` counts `typeTag + body` (excludes the 4 prefix bytes), capped at
/// `Rwork.maxFramePayloadLength` (16 MiB) — reusing the terminal protocol's ceiling.
/// `.event` and `.subscribe` bodies are JSON; `.keepAlive` has an empty body.
public enum InspectorCodec {
    /// Errors distinct from `RworkProtocol.RworkError` (decode-time, inspector frames).
    ...
    public static func encode(_ message: InspectorWireMessage) throws -> Data {
        ...
        guard body.count <= Rwork.maxFramePayloadLength else {
...
/// `RworkProtocol.FrameDecoder`, separate namespace). Reassembles whole frames from
...
// MARK: - Local big-endian helpers (self-contained; the RworkProtocol ones are internal)
