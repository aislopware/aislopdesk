...
/// The exact surface ``RworkClient/RworkClient`` drives on its session transport: a logical
/// channel riding a shared ``MuxNWConnection`` (``MuxClientTransport``).
///
/// ## Why a protocol
/// `RworkClient` takes an injectable `makeTransport` factory so the per-pane client can be backed
