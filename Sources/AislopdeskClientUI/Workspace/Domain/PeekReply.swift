...
public struct PeekContent: Equatable, Sendable {
    ...
    /// with a stand-in (no `AislopdeskTerminal` import in the test).
...
/// line and a short status label. `CommandBlock` (in `AislopdeskTerminal`) conforms via an extension, so the
/// builder stays pure (no `AislopdeskTerminal` import here) and a test can feed a stand-in struct.
