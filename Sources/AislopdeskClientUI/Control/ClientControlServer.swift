...
//
// The CLIENT-side runtime control surface the new `aislopdesk` CLI talks to (windows/tabs/panes, badges,
// jump/view/edit, config, theme/font/keybind dumps, pane capture/send-keys, agent status). It MIRRORS the
// host's `AgentControlAcceptor` (`AislopdeskHost/AgentControlListener.swift`): an `AF_UNIX` stream socket
...
// answered with a response line. It reuses the SAME line protocol the host socket / `AislopdeskCtlCore`
...
import AislopdeskWorkspaceCore // ClientControlDispatcher + ClientControlBackend
import Darwin
import Foundation
...
final class ClientControlServer: @unchecked Sendable {
    ...
    static let socketEnvVar = "AISLOPDESK_CLIENT_SOCKET"
    ...
    /// `AISLOPDESK_CLIENT_SOCKET` env override, else the stable Application Support default. (The CLI's
    ...
    /// `<Application Support>/Aislopdesk/cli-control.sock` (sibling of `workspace.json` /
    ...
    func start() throws {
        ...
        // Best-effort parent-dir creation (Application Support/Aislopdesk) — a missing dir would fail bind.
        ...
        // `AISLOPDESK_CLIENT_SOCKET`). Best-effort; a separately-launched CLI computes the same default.
