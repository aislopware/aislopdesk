...
///   `RWORK_PRESENT_ON_ARRIVAL=0` for A/B (pure vsync cadence).
...
///   8.3 ms tick as fallback cadence. `RWORK_PRESENT_ON_ARRIVAL=0` ⇒ pure 120 Hz-quantized
///   cadence (avg hold ≈ 4.2 ms); `RWORK_TICK_HZ` overrides the resolved rate for A/B.
...
public final class FramePacer: @unchecked Sendable {
    ...
    /// Depth-change observability for the adaptive controller (env `RWORK_VIDEO_DEBUG`): one
    ...
    private static let dbgEnabled = ProcessInfo.processInfo.environment["RWORK_VIDEO_DEBUG"] != nil
    ...
    // Enabled via `RWORK_PACER=deadline`; `RWORK_PLAYOUT_MS` tunes the delay (default 20).
    ...
    public init(maxFrameRate: Double = 60.0, targetDepth: Int = 2, maxDepth: Int = 5, adaptiveJitter: Bool = false, presentOnArrival: Bool = true,
        ...
        if Self.dbgEnabled {
            FileHandle.standardError.write(Data("Rwork[video.client]: pacer up — tick=\(Int(maxFrameRate))Hz depth=\(clampedTarget) adaptive=\(adaptiveJitter) presentOnArrival=\(presentOnArrival) mode=\(deadlineMode ? "deadline(playout=\(Int(playoutDelaySec * 1000))ms)" : "arrival")\n".utf8))
        }
    }
    ...
    public func submit(_ frame: CVImageBuffer) {
        ...
        if adaptiveJitter {
            ...
            if Self.dbgEnabled && liveDepth != before {
                depthChangeLine = "Rwork[video.client]: jitter depth \(before)→\(liveDepth) (arrival jitter \(String(format: "%.1f", jitterMs))ms)\n"
            }
        }
    ...
    public func frameForVSync() -> CVImageBuffer? {
        ...
        if !queue.isEmpty {
            ...
            if adaptiveJitter && wasTransientDip {
                ...
                if Self.dbgEnabled && liveDepth != before {
                    FileHandle.standardError.write(Data("Rwork[video.client]: jitter depth \(before)→\(liveDepth) (underrun)\n".utf8))
                }
            }
            return next
        }
    ...
    private func dbgNoteHold(since submittedAt: Double) {
        ...
        if dbgLastPresentAt > 0, now - dbgLastPresentAt > 0.028 {
            FileHandle.standardError.write(Data("Rwork[video.client]: present gap \(Int((now - dbgLastPresentAt) * 1000))ms\n".utf8))
        }
        ...
        let line = "Rwork[video.client]: pacer hold n=\(sorted.count) p50=\(ms(sorted[sorted.count / 2]))ms p90=\(ms(sorted[min(sorted.count - 1, (sorted.count * 9) / 10)]))ms max=\(ms(sorted[sorted.count - 1]))ms\n"
    ...
    /// an unknown/headless screen). `RWORK_TICK_HZ` overrides for A/B, clamped to a sane band.
