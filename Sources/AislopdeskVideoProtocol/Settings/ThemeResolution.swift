...
/// `themeDark`/`customDarkSlug` the DARK slot. When `useSeparateDarkTheme` is OFF (or `nil`) the primary slot
/// applies for EVERY OS appearance (otty's default — one theme, no follow-OS), EXCEPT the legacy `.system`
/// built-in choice which still follows the OS by construction. When it is ON, the OS appearance selects the
...
public enum ThemeResolution {
    /// The compile-time default DARK built-in id (Monokai Pro Classic) — the all-`nil` appearance default and
    /// the `.system` / OS-dark fallback. MIRRORS `OttyTheme.monokaiProClassic.id`.
    public static let defaultDarkID = "monokai-classic"
    /// The compile-time default LIGHT built-in id (Monokai Pro Classic Light) — the `.system` / OS-light
    /// fallback. MIRRORS `OttyTheme.monokaiProClassicLight.id`.
    ...
    ///   - `nil` (unset slot) ⇒ the compile-time default Monokai Pro Classic (``defaultDarkID``);
    ///   - `.system` ⇒ FOLLOWS the OS (dark → ``defaultDarkID``, light → ``defaultLightID``);
    ...
    public static func builtinID(for choice: ThemeChoice?, osIsDark: Bool) -> String {
        guard let choice else { return defaultDarkID }
