// aislopdesk-fuzzybench — benchmark + parity validator for the vendored `FuzzyMatcher` (the in-tree
...
// scales the corpus, so it lives outside `swift test`. Run: `swift run -c release aislopdesk-fuzzybench`
...
print("aislopdesk-fuzzybench — FuzzyMatcher (fzf V2 port) vs real fzf + Bitap baseline")
