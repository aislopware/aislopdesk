...
/// W10 — `aislopdesk integration install claude` (docs/41 §4.2 signal 2, docs/42 W10). The
...
public enum AgentInstaller {
    ...
    public static let hookMarker = "aislopdesk-agent"
    ...
    public static func hookCommand(scriptPath: String) -> String {
        // The script path contains `aislopdesk-agent`, which IS the marker — so an entry built
    ...
    /// host's Unix-domain listener socket (`AISLOPDESK_SOCKET_PATH`, exported into every pane's
    /// env by the host). Uses `nc -U` (Muxy's transport, docs/41 §2.1); a missing socket var is
    /// a silent no-op so a non-aislopdesk shell that sources these hooks never errors.
    ///
    ...
    public static func hookScript() -> String {
        let marker = ["aislopdesk", "agent"].joined(separator: "-")
        return """
        #!/bin/sh
        # \(marker) — Claude Code hook → aislopdesk host listener (W10).
        # Reads the hook event JSON on stdin and POSTs it to the per-host Unix socket the
        # aislopdesk host exported as AISLOPDESK_SOCKET_PATH. No socket → silent no-op (so a
        # shell that sources these hooks outside aislopdesk never errors). Read-only relay.
        sock="${AISLOPDESK_SOCKET_PATH:-}"
        [ -z "$sock" ] && exit 0
        [ -S "$sock" ] || exit 0
        # One record = a `pane=<id>` header line (the host's AISLOPDESK_PANE_ID), then the raw
        # hook JSON. The host routes by the pane id; an empty id still parses (the host drops it).
        payload="$(cat)"
        { printf 'pane=%s\\n' "${AISLOPDESK_PANE_ID:-}"; printf '%s\\n' "$payload"; } \\
    ...
    /// The default hook-script path (`~/.claude/hooks/aislopdesk-agent.sh`). The basename
    ...
    {
        configBaseURL(environment: environment, home: home)
            .appendingPathComponent("hooks/aislopdesk-agent.sh").path
    }
}
