...
///   it parses via the W8-extended ``AislopdeskInspector/HookParser``, maps the typed
///   ``AislopdeskInspector/HookPayload`` → an ``AislopdeskAgentDetect/ClaudeHookEvent``, folds
...
/// `ClaudeStatus.urgency` / `ClaudeHookEvent.NotificationKind` — `AislopdeskProtocol` does not
/// depend on `AislopdeskAgentDetect`, so the bytes are the contract):
...
public struct AgentHookHandler: Sendable {
    ...
    /// `AislopdeskInspector.NotificationKind` → `AislopdeskAgentDetect.ClaudeHookEvent.NotificationKind`
...
/// `AISLOPDESK_PANE_ID` the hook script forwards), resolved by the owner before constructing
...
public final class AgentHookListener: @unchecked Sendable {
    ...
    /// Binds the socket at `path` (the value the host exports as `AISLOPDESK_SOCKET_PATH`).
    public func start(path: String) throws {
