...
public enum ShellIntegration {
    ...
    public static let optOutEnvKey = "RWORK_SHELL_INTEGRATION"
    ...
    static let realZDotDirEnvKey = "RWORK_REAL_ZDOTDIR"
    ...
    /// child env: `ZDOTDIR` → the shim dir, and `RWORK_REAL_ZDOTDIR` → the user's real ZDOTDIR
    ...
    static func writeShimDirectory(into tmpDir: URL) -> URL? {
        let dir = tmpDir.appendingPathComponent("rwork-zdotdir-\(UUID().uuidString)", isDirectory: true)
    ...
    ///
    /// `${RWORK_REAL_ZDOTDIR:-$HOME}` — the real dir, defaulting to `$HOME` when the var is unset.
    /// `${ZDOTDIR:-$HOME}` (`__rwork_shim`) — captured at the TOP so we can restore it after the
    ...
    static func shimSource(forwarding fileName: String, reassertZDotDir: Bool) -> String {
        ...
        if reassertZDotDir {
            ...
            lines.append("ZDOTDIR=\"$__rwork_shim\"")
        }
        lines.append("unset __rwork_shim __rwork_real")
    ...
    /// `add-zsh-hook` (composes with starship/omz/p10k; gated by `RWORK_OSC133`), then restore
    /// `ZDOTDIR` to the user's real value as the LAST act so the running shell sees the env it
    /// expects.
    static let zshrcBody: String = """
    # rwork shell-integration shim — sources the user's real .zshrc, then installs a
    ...
    if (( $+functions[TRAPWINCH] )); then
      functions[__rwork_user_winch]=$functions[TRAPWINCH]
    fi
    TRAPWINCH() {
      (( $+functions[__rwork_user_winch] )) && __rwork_user_winch "$@"
      zle && zle reset-prompt
    }

    # rwork OSC 133 shell integration — emit FinalTerm/iTerm2 semantic command marks so the
    ...
    # RWORK_OSC133=0; the whole shim is already gated by RWORK_SHELL_INTEGRATION upstream.
    case "${RWORK_OSC133:-1}" in
      0|false|no|off) ;;
      *)
        ...
        __rwork_osc133_preexec() { printf '\\033]133;C\\007' }
        ...
        __rwork_osc133_precmd() {
          local __rwork_exit=$?
          printf '\\033]133;D;%s\\007' "$__rwork_exit"
          printf '\\033]133;A\\007'
        }
        add-zsh-hook preexec __rwork_osc133_preexec
        add-zsh-hook precmd  __rwork_osc133_precmd
        ;;
    esac

    # Restore ZDOTDIR to the user's real value: only startup-file resolution was intercepted.
    if [ "$__rwork_real" = "$HOME" ]; then
      unset ZDOTDIR
    else
      ZDOTDIR="$__rwork_real"
    fi
    unset __rwork_shim __rwork_real
