...
///
/// All reads consume from the current offset and throw ``RworkError/truncated``
...
struct BigEndianReader {
    ...
    private mutating func nextByte() throws -> UInt8 {
        guard offset < data.count else { throw RworkError.truncated }
    ...
    mutating func readBytes(_ count: Int) throws -> Data {
        guard count >= 0, bytesRemaining >= count else { throw RworkError.truncated }
