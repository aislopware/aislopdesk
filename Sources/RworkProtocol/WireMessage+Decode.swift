...
extension WireMessage {
    ...
    ///
    /// - Throws: ``RworkError/truncated`` if the body is shorter than the type
    ///   requires, ``RworkError/unknownMessageType(_:)`` for an unrecognized type
    ///   byte, or ``RworkError/malformedBody(_:)`` for a right-length-but-invalid
    ///   body (e.g. bad UTF-8).
    static func decode(payload: Data) throws -> WireMessage {
        ...
        case 10: // hello
            ...
            guard let sessionID = UUID(dataBytes: idBytes) else {
                throw RworkError.malformedBody("hello: invalid sessionID bytes")
            }
        ...
        case 20: // helloAck
            ...
            guard let sessionID = UUID(dataBytes: idBytes) else {
                throw RworkError.malformedBody("helloAck: invalid sessionID bytes")
            }
            return .helloAck(
                sessionID: sessionID,
        ...
        case 21: // title
            ...
            guard let string = String(data: bytes, encoding: .utf8) else {
                throw RworkError.malformedBody("title: invalid UTF-8")
            }
            return .title(string)
