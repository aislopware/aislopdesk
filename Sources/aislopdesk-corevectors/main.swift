...
func fragmentRecord(streamSeq: UInt32, frameID: UInt32, fragIndex: UInt16, fragCount: UInt16,
                    flags: UInt8, hostTs: UInt32, payload: [UInt8]) -> [String: Any] {
    let header = FrameFragmentHeader(streamSeq: streamSeq, frameID: frameID, fragIndex: fragIndex,
                                     fragCount: fragCount, flags: .init(rawValue: flags),
                                     payloadLength: UInt16(payload.count), hostSendTsMillis: hostTs)
    let frag = FrameFragment(header: header, payload: Data(payload))
    return ["streamSeq": streamSeq, "frameID": frameID, "fragIndex": fragIndex, "fragCount": fragCount,
            "flags": flags, "hostTs": hostTs, "payloadHex": hex(payload), "hex": hex(frag.encode())]
}
root["fragmentEncode"] = [
    fragmentRecord(streamSeq: 0x0102_0304, frameID: 0x0506_0708, fragIndex: 0x090A, fragCount: 0x0B0C,
                   flags: 0b0000_0101, hostTs: 0x0D0E_0F10, payload: [0xAA, 0xBB, 0xCC]),
    fragmentRecord(streamSeq: 0, frameID: 0, fragIndex: 0, fragCount: 1, flags: 0, hostTs: 0, payload: []),
    fragmentRecord(streamSeq: 0xFFFF_FFFF, frameID: 7, fragIndex: 2, fragCount: 9,
                   flags: 0b1101_1010, hostTs: 1234, payload: Array(0..<UInt8(200)).map { $0 }),
]
...
func fecRecoverRecord(data: [[UInt8]?], parity: [[UInt8]?], groupSize: Int) -> [String: Any] {
    let fec = XORParityFEC(groupSize: 5)
    let recovered = fec.recover(dataFragments: data.map { (bytes: [UInt8]?) in bytes.map { Data($0) } },
                                parityFragments: parity.map { (bytes: [UInt8]?) in bytes.map { Data($0) } }, groupSize: groupSize)
    func opt(_ b: [UInt8]?) -> Any { b.map { hex($0) } ?? NSNull() }
    func optD(_ d: Data?) -> Any { d.map { hex($0) } ?? NSNull() }
    return ["dataHex": data.map(opt), "parityHex": parity.map(opt), "groupSize": groupSize,
            "recoveredHex": recovered.map(optD)]
}
let g5parity = XORParityFEC(groupSize: 5).parity(forDataFragments:
    [[1, 2], [3], [4, 5, 6], [7], [8, 9]].map { Data($0) }, groupSize: 5).map { Array($0) }
root["fecRecover"] = [
...
root["naluJoin"] = [
    ["unitsHex": [hex([1, 2, 3]), hex([4, 5]), hex([6])], "hex": hex(NALUnit.join([[1, 2, 3], [4, 5], [6]].map { Data($0) }))],
...
func cursorUpdateRecord(shapeID: UInt16, visible: Bool, x: Double, y: Double, hx: Double, hy: Double) -> [String: Any] {
    let u = CursorUpdate(position: VideoPoint(x: x, y: y), shapeID: shapeID, hotspot: VideoPoint(x: hx, y: hy), visible: visible)
...
func cursorShapeRecord(shapeID: UInt16, w: Double, h: Double, hx: Double, hy: Double, bitmap: [UInt8]) -> [String: Any] {
    let s = CursorShapeMessage(shapeID: shapeID, size: VideoSize(width: w, height: h),
                               hotspot: VideoPoint(x: hx, y: hy), bitmap: Data(bitmap))
...
root["inputEvent"] = [
    ...
    ie("mouseDown", .mouseDown(button: .right, normalized: VideoPoint(x: 0.1, y: 0.2), clickCount: 2, modifiers: .init(rawValue: mods), tag: 7),
       ["button": 1, "nx": 0.1, "ny": 0.2, "clickCount": 2, "mods": mods, "tag": 7]),
    ie("mouseUp", .mouseUp(button: .left, normalized: VideoPoint(x: 0.3, y: 0.4), clickCount: 1, modifiers: .init(rawValue: 0), tag: 8),
       ["button": 0, "nx": 0.3, "ny": 0.4, "clickCount": 1, "mods": 0, "tag": 8]),
    ie("mouseDrag", .mouseDrag(button: .other, normalized: VideoPoint(x: 0.5, y: 0.6), clickCount: 1, modifiers: .init(rawValue: InputModifiers.control.rawValue), tag: 9),
       ["button": 2, "nx": 0.5, "ny": 0.6, "clickCount": 1, "mods": InputModifiers.control.rawValue, "tag": 9]),
    ie("scroll", .scroll(dx: -3.5, dy: 12.0, normalized: VideoPoint(x: 0.0, y: 1.0), tag: 10),
       ["dx": -3.5, "dy": 12.0, "nx": 0.0, "ny": 1.0, "tag": 10]),
    ie("key", .key(keyCode: 0x35, down: true, modifiers: .init(rawValue: InputModifiers.option.rawValue), tag: 11),
...
root["videoControl"] = [
    vc("hello", .hello(protocolVersion: 7, requestedWindowID: 0xDEAD_BEEF, viewport: VideoSize(width: 1280.0, height: 800.0)),
       ["version": 7, "windowID": 0xDEAD_BEEF, "vw": 1280.0, "vh": 800.0]),
    vc("helloAck", .helloAck(accepted: true, streamID: 42, captureWidth: 1920, captureHeight: 1080,
                             windowBoundsCG: VideoRect(x: 0.0, y: 25.0, width: 800.0, height: 600.0), fullRange: true),
       ["accepted": true, "streamID": 42, "cw": 1920, "ch": 1080, "bx": 0.0, "by": 25.0, "bw": 800.0, "bh": 600.0, "fullRange": true]),
    vc("bye", .bye, [:]),
    vc("resizeRequest", .resizeRequest(desired: VideoSize(width: 640.5, height: 480.25), epoch: 3), ["w": 640.5, "h": 480.25, "epoch": 3]),
...
root["recovery"] = [
    ...
    rc("requestLTRRefresh", .requestLTRRefresh(fromFrameID: 10, toFrameID: 12, lastDecodedFrameID: RecoveryMessage.noFrameDecodedSentinel),
    ...
    rc("networkStats", .networkStats(NetworkStatsReport(
...
root["muxBare"] = [
    ["channelID": 0x0102_0304, "payloadHex": hex([9, 8, 7]), "hex": hex(VideoMuxHeaderCodec.encode(channelID: 0x0102_0304, payload: Data([9, 8, 7])))],
]
let muxHeader = MuxFrameFragmentHeader(channelID: 0xAABB_CCDD, streamSeq: 1, frameID: 2, fragIndex: 3,
                                       fragCount: 4, flags: .keyframe, payloadLength: 2)
root["muxFragment"] = [
    ["channelID": 0xAABB_CCDD, "streamSeq": 1, "frameID": 2, "fragIndex": 3, "fragCount": 4,
     "flags": FrameFragmentHeader.Flags.keyframe.rawValue, "payloadHex": hex([0xEE, 0xFF]),
...
func coordRecord(nx: Double, ny: Double, bx: Double, by: Double, bw: Double, bh: Double) -> [String: Any] {
    let p = CoordinateMapping.windowPoint(normalized: VideoPoint(x: nx, y: ny),
                                          windowBounds: VideoRect(x: bx, y: by, width: bw, height: bh))
    return ["nx": nx, "ny": ny, "bx": bx, "by": by, "bw": bw, "bh": bh,
            "outXBits": p.x.bitPattern, "outYBits": p.y.bitPattern]
}
...
func ycbcrRecord(_ range: ColorRange, _ name: String) -> [String: Any] {
    ...
    return ["range": name,
            "lumaScale": c.lumaScale.bitPattern, "lumaBias": c.lumaBias.bitPattern, "chromaBias": c.chromaBias.bitPattern,
            "crToR": c.crToR.bitPattern, "cbToG": c.cbToG.bitPattern, "crToG": c.crToG.bitPattern, "cbToB": c.cbToB.bitPattern]
}
...
func tierRecord(loss: Double, prevTier: UInt8, allowOff: Bool) -> [String: Any] {
    ["lossBits": loss.bitPattern, "prevTier": prevTier, "allowOff": allowOff,
...
root["adaptiveGroupSize"] = (0...7).map { groupSizeRecord(tier: UInt8($0), def: 5) } + [groupSizeRecord(tier: 200, def: 7)]
...
do {
    ...
    for _ in 0..<60 { arrival += 16; ts &+= 16; est.note(arrivalMs: arrival, sendTs: ts) }   // steady
    for _ in 0..<40 { arrival += 41; ts &+= 16; est.note(arrivalMs: arrival, sendTs: ts) }   // +25ms ramp
    root["trendlineDrive"] = [
...
do {
    ...
    let sizes: [Int] = [10_000, 20_000, 15_000, 30_000, 12_000, 18_000, 22_000, 9_000, 40_000, 11_000]
...
do {
    ...
    for i in 0..<30 { t += gaps[i % gaps.count]; dp.noteArrival(t); dp.notePresent(t) }
...
let sidA = UUID(uuid: (0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88,
                       0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00))
let sidB = UUID(uuid: (0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
...
root["terminalWireMessages"] = [
    wmRecord("output", .output(seq: 1, bytes: Data("hello".utf8)),
             ["seq": Int64(1), "bytesHex": hex(Data("hello".utf8))]),
    wmRecord("output", .output(seq: Int64.max, bytes: Data()),
             ["seq": Int64.max, "bytesHex": ""]),
    wmRecord("output", .output(seq: 42, bytes: Data([0x1b, 0x5b, 0x32, 0x4a])),
    ...
    wmRecord("input", .input(Data([0x00, 0xff, 0x80, 0x7f])),
             ["bytesHex": hex([0x00, 0xff, 0x80, 0x7f])]),
    wmRecord("hello", .hello(protocolVersion: 1, sessionID: WireMessage.newSessionID, lastReceivedSeq: 0),
             ["protocolVersion": Int(1), "sessionIdHex": hex(uuidBytes(WireMessage.newSessionID)), "lastReceivedSeq": Int64(0)]),
    wmRecord("hello", .hello(protocolVersion: UInt16.max, sessionID: sidA, lastReceivedSeq: Int64.max),
             ["protocolVersion": Int(UInt16.max), "sessionIdHex": hex(uuidBytes(sidA)), "lastReceivedSeq": Int64.max]),
    wmRecord("resize", .resize(cols: 80, rows: 24, pxWidth: 640, pxHeight: 384),
             ["cols": Int(80), "rows": Int(24), "pxWidth": Int(640), "pxHeight": Int(384)]),
    wmRecord("resize", .resize(cols: 65535, rows: 65535, pxWidth: 65535, pxHeight: 65535),
    ...
    wmRecord("helloAck", .helloAck(sessionID: sidB, resumeFromSeq: 9, returningClient: true),
             ["sessionIdHex": hex(uuidBytes(sidB)), "resumeFromSeq": Int64(9), "returningClient": true]),
    wmRecord("helloAck", .helloAck(sessionID: WireMessage.newSessionID, resumeFromSeq: 0, returningClient: false),
             ["sessionIdHex": hex(uuidBytes(WireMessage.newSessionID)), "resumeFromSeq": Int64(0), "returningClient": false]),
    ...
    wmRecord("commandStatus", .commandStatus(.idle(exitCode: 130, durationMS: 12_000)),
             ["cmd": "idle", "hasExit": true, "exitCode": Int(130), "durationMs": UInt64(12_000)]),
    wmRecord("commandStatus", .commandStatus(.idle(exitCode: Int32.min, durationMS: UInt32.max)),
             ["cmd": "idle", "hasExit": true, "exitCode": Int(Int32.min), "durationMs": UInt64(UInt32.max)]),
    wmRecord("commandStatus", .commandStatus(.idle(exitCode: nil, durationMS: 0)),
             ["cmd": "idle", "hasExit": false, "exitCode": Int(0), "durationMs": UInt64(0)]),
    wmRecord("notification", .notification(title: "CI", body: "green ✅ — đa byte"),
             ["title": "CI", "body": "green ✅ — đa byte"]),
    wmRecord("notification", .notification(title: "", body: "build done"),
             ["title": "", "body": "build done"]),
    wmRecord("notification", .notification(title: "semis;in;title", body: "and;in;body;too"),
...
root["muxEnvelopes"] = [
    muxRecord("channelOpen", .channelOpen(channelID: 1, sessionID: WireMessage.newSessionID, lastReceivedSeq: 0, channelClass: 0),
              ["channelId": UInt32(1), "sessionIdHex": hex(uuidBytes(WireMessage.newSessionID)), "lastReceivedSeq": Int64(0), "channelClass": Int(0)]),
    muxRecord("channelOpen", .channelOpen(channelID: UInt32.max, sessionID: sidA, lastReceivedSeq: -1, channelClass: 255),
              ["channelId": UInt32.max, "sessionIdHex": hex(uuidBytes(sidA)), "lastReceivedSeq": Int64(-1), "channelClass": Int(255)]),
    muxRecord("channelOpenAck", .channelOpenAck(channelID: 3, accepted: true),
              ["channelId": UInt32(3), "accepted": true]),
    muxRecord("channelOpenAck", .channelOpenAck(channelID: 5, accepted: false),
              ["channelId": UInt32(5), "accepted": false]),
    muxRecord("channelData", .channelData(channelID: 9, payload: WireMessage.output(seq: 42, bytes: Data("vt ✅".utf8)).encode()),
              ["channelId": UInt32(9), "payloadHex": hex(WireMessage.output(seq: 42, bytes: Data("vt ✅".utf8)).encode())]),
    muxRecord("channelData", .channelData(channelID: 4, payload: Data()),
              ["channelId": UInt32(4), "payloadHex": ""]),
    muxRecord("channelClose", .channelClose(channelID: 6), ["channelId": UInt32(6)]),
    muxRecord("windowAdjust", .windowAdjust(channelID: 7, bytesToAdd: 262_144),
              ["channelId": UInt32(7), "bytesToAdd": UInt64(262_144)]),
    muxRecord("windowAdjust", .windowAdjust(channelID: 1, bytesToAdd: UInt32.max),
