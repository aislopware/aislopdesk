...
public enum LiveBitratePolicy {
    ...
    /// to 1× (¼ the bytes) made scroll markedly smoother. So this is now env-tunable (`RWORK_BPP`) to
    ...
    public static let bitsPerPixelPerFrame: Double = {
        if let s = ProcessInfo.processInfo.environment["RWORK_BPP"], let v = Double(s), v > 0, v <= 1.0 { return v }
        return 0.15
    }()
