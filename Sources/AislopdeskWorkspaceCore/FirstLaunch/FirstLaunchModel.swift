...
public enum FirstLaunchStep: String, CaseIterable, Identifiable, Sendable {
    ...
    public var title: String {
        ...
        case .installCLI: "Install the Aislopdesk CLI"
    ...
    public var subtitle: String {
        switch self {
        case .onLaunch:
            "Choose what happens when Aislopdesk opens — restore your last session or start fresh."
        case .defaultTerminal:
            "Register Aislopdesk as the system handler for terminal scripts and `ssh://` links."
        case .installCLI:
            "Add `aislopdesk` to your PATH so you can drive the app from any shell."
        case .theme:
            "Pick a colour theme. You can fine-tune it later in Settings → Appearance."
        case .installClaudeHooks:
            "Let Claude Code stream its live state back to Aislopdesk for tab badges and notifications."
        }
    }
...
public enum CLIShellShim {
    ...
    public static func snippet(allowOverwrite: Bool, binary: String = "aislopdesk") -> String {
        var lines: [String] = [
            "# Aislopdesk CLI — prefix-less shell functions (Omit Prefix).",
            "# Managed by Aislopdesk; edits here are overwritten when the toggle changes.",
        ]
