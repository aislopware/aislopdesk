...
// `aislopdesk version` — prints the marketing version, an optional build hash, and a brief
...
public enum CLIVersion {
    ...
    public static let buildHashEnvKey = "AISLOPDESK_BUILD_HASH"

    /// Builds the multi-line `version` output:
    /// ```
    /// aislopdesk <version>[ (<hash>)]
    ...
    public static func versionSummary(
        ...
        var head = "aislopdesk \(version)"
        if let hash = environment[buildHashEnvKey], !hash.isEmpty {
            head += " (\(hash))"
        }
        let proto = "terminal protocol v\(Aislopdesk.protocolVersion)"
