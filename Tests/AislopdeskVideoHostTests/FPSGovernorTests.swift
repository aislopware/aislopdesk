...
final class FPSGovernorTests: XCTestCase {
    ...
    func testUnseededEWMANeverSteps() {
        var gov = FPSGovernor(baseFps: 60)
        for _ in 0..<100 {
            XCTAssertEqual(gov.onTick(targetBps: 1, congested: true), 60,
    ...
    func testAnchorFramesExcludedFromEWMA() {
        ...
        gov.noteEncodedFrame(bytes: 10_000, isAnchor: false)      // first non-anchor seeds exactly
        XCTAssertEqual(gov.bytesPerFrameEWMA, 10_000)
        gov.noteEncodedFrame(bytes: 500_000, isAnchor: true)
        XCTAssertEqual(gov.bytesPerFrameEWMA, 10_000, "a later anchor still does not move the EWMA")
    }

    func testEWMAAlphaConvergence() {
        var gov = FPSGovernor(baseFps: 60)
        gov.noteEncodedFrame(bytes: 10_000, isAnchor: false)
        gov.noteEncodedFrame(bytes: 20_000, isAnchor: false)
        // 10000 × 0.875 + 20000 × 0.125 = 11250 (alpha 0.125 — the loss-EWMA discipline).
        XCTAssertEqual(gov.bytesPerFrameEWMA, 11_250, accuracy: 1e-9)
        // Converges toward a sustained new level.
        for _ in 0..<60 { gov.noteEncodedFrame(bytes: 20_000, isAnchor: false) }
        XCTAssertEqual(gov.bytesPerFrameEWMA, 20_000, accuracy: 20)
    ...
    func testOverBudgetWithoutCongestionNeverStepsDown() {
        ...
        for _ in 0..<100 {
            XCTAssertEqual(gov.onTick(targetBps: 10_000_000, congested: false), 60,
    ...
    func testCongestionWithoutOverBudgetNeverStepsDown() {
        var gov = FPSGovernor(baseFps: 60)
        gov.noteEncodedFrame(bytes: 1_000, isAnchor: false)       // offered ≈ 480 kbps — tiny
        for _ in 0..<100 {
            XCTAssertEqual(gov.onTick(targetBps: 10_000_000, congested: true), 60,
    ...
    func testStepDownAfterStreakThenHoldWindowSpacesRungs() {
        var gov = FPSGovernor(baseFps: 60)
        gov.noteEncodedFrame(bytes: 1_000_000, isAnchor: false)
        ...
        for _ in 0..<40 {
            ...
            if next != fps {
                XCTAssertEqual(FPSGovernor.ladder(baseFps: 60).firstIndex(of: next)!,
                               FPSGovernor.ladder(baseFps: 60).firstIndex(of: fps)! + 1,
        ...
        XCTAssertEqual(stepTicks[20]! - stepTicks[30]!, FPSGovernor.stepDownHoldTicks)
        XCTAssertEqual(stepTicks[15]! - stepTicks[20]!, FPSGovernor.stepDownHoldTicks)
    ...
    func testOverBudgetRunResetsOnCleanTick() {
        var gov = FPSGovernor(baseFps: 60)
        gov.noteEncodedFrame(bytes: 1_000_000, isAnchor: false)
        for _ in 0..<FPSGovernor.warmupTicks { _ = gov.onTick(targetBps: 1_000_000_000, congested: false) } // warmup, clean
    ...
    private func governorAt20() -> FPSGovernor {
        ...
        for _ in 0..<200 { gov.noteEncodedFrame(bytes: 1_000, isAnchor: false) }
        return gov
    }
    ...
    func testStepUpRequiresCleanRunAndProjectedFitOneRungPerRun() {
        ...
        for _ in 0..<(FPSGovernor.stepUpTicks * 2 + 2) {
            ...
            if next != fps { fpsTrail.append(next); fps = next }
        }
    ...
    func testStepUpBlockedByStrictProjectedFit() {
        ...
        for _ in 0..<200 { gov.noteEncodedFrame(bytes: 10_000, isAnchor: false) }
        for _ in 0..<(FPSGovernor.stepUpTicks * 3) {
            XCTAssertEqual(gov.onTick(targetBps: 3_000_000, congested: false), 30,
                           "clean run alone is not enough — the next rung must PROJECT under target")
        }
        XCTAssertGreaterThanOrEqual(gov.cleanRun, FPSGovernor.stepUpTicks, "the run keeps accruing; only the fit blocks")
    }
    ...
    func testCongestionEvidenceABRBelowCeiling() {
        XCTAssertTrue(FPSGovernor.congestionEvidence(lastLossSample: 0, smoothedRTTMillis: 10,
                                                     minRTTMillis: 10, abrCurrent: 5_000_000, abrCeiling: 10_000_000))
        XCTAssertFalse(FPSGovernor.congestionEvidence(lastLossSample: 0, smoothedRTTMillis: 10,
                                                      minRTTMillis: 10, abrCurrent: 10_000_000, abrCeiling: 10_000_000),
                       "ABR AT the ceiling is not congestion evidence")
    }

    func testCongestionEvidenceRawLossOverThreshold() {
        XCTAssertTrue(FPSGovernor.congestionEvidence(lastLossSample: LiveCongestionController.lossThreshold + 0.01,
                                                     smoothedRTTMillis: 10, minRTTMillis: 10,
                                                     abrCurrent: nil, abrCeiling: nil))
        XCTAssertFalse(FPSGovernor.congestionEvidence(lastLossSample: LiveCongestionController.lossThreshold,
                                                      smoothedRTTMillis: 10, minRTTMillis: 10,
                                                      abrCurrent: nil, abrCeiling: nil),
    ...
    func testCongestionEvidenceRTTInflation() {
        ...
        XCTAssertTrue(FPSGovernor.congestionEvidence(lastLossSample: 0, smoothedRTTMillis: 30,
                                                     minRTTMillis: 10, abrCurrent: nil, abrCeiling: nil))
        XCTAssertFalse(FPSGovernor.congestionEvidence(lastLossSample: 0, smoothedRTTMillis: 20,
                                                      minRTTMillis: 10, abrCurrent: nil, abrCeiling: nil),
                       "past the factor but inside the absolute slack = LAN wobble, not congestion")
        XCTAssertFalse(FPSGovernor.congestionEvidence(lastLossSample: 0, smoothedRTTMillis: 11,
                                                      minRTTMillis: 10, abrCurrent: nil, abrCeiling: nil))
    }

    func testCongestionEvidenceInfiniteMinRTTFallsBackToOtherArms() {
        XCTAssertFalse(FPSGovernor.congestionEvidence(lastLossSample: 0, smoothedRTTMillis: 100,
                                                      minRTTMillis: .infinity, abrCurrent: nil, abrCeiling: nil),
                       "no RTT baseline (telemetry off) ⇒ the RTT arm can never fire")
        XCTAssertTrue(FPSGovernor.congestionEvidence(lastLossSample: 0.05, smoothedRTTMillis: 100,
                                                     minRTTMillis: .infinity, abrCurrent: nil, abrCeiling: nil),
...
final class EncodeCadenceGateTests: XCTestCase {
    ...
    func testInertWhenTargetIntervalNonPositive() {
        ...
        for i in 0..<10 {
            XCTAssertTrue(gate.admit(now: Double(i) * slot, targetIntervalSeconds: 0, toleranceSeconds: tol, forced: false))
            XCTAssertTrue(gate.admit(now: Double(i) * slot, targetIntervalSeconds: -1, toleranceSeconds: tol, forced: false))
        }
    }
    ...
    func testFirstCallAdmitsAndAnchors() {
        ...
        XCTAssertFalse(gate.admit(now: 100.0 + slot, targetIntervalSeconds: 1.0 / 30.0, toleranceSeconds: tol, forced: false),
    ...
    func testRegularCadenceAtEveryLadderRung() {
        for (fps, expectStride) in [(30, 2), (20, 3), (15, 4)] {
            ...
            for i in 0..<48 {
                if gate.admit(now: Double(i) * slot, targetIntervalSeconds: 1.0 / Double(fps),
                              toleranceSeconds: tol, forced: false) {
                    admittedSlots.append(i)
                }
            }
            XCTAssertGreaterThan(admittedSlots.count, 3)
            for pair in zip(admittedSlots, admittedSlots.dropFirst()) {
                XCTAssertEqual(pair.1 - pair.0, expectStride,
    ...
    func testArrivalJitterNeverSlipsASlotAndScheduleDoesNotDrift() {
        ...
        for pair in zip(admitted, admitted.dropFirst()) {
            XCTAssertEqual(pair.1 - pair.0, interval, accuracy: 1e-9,
    ...
    func testContentStallReanchorsWithoutBurstCatchUp() {
        ...
        XCTAssertFalse(gate.admit(now: 0.5 + slot, targetIntervalSeconds: interval, toleranceSeconds: tol, forced: false))
        XCTAssertTrue(gate.admit(now: 0.5 + 2 * slot, targetIntervalSeconds: interval, toleranceSeconds: tol, forced: false))
    }
    ...
    func testRejectionExposesStableNextDueDeadline() {
        ...
        XCTAssertFalse(gate.admit(now: 100.0 + slot, targetIntervalSeconds: interval, toleranceSeconds: tol, forced: false))
        XCTAssertEqual(gate.nextDue, 100.0 + interval, accuracy: 1e-9, "a rejection must not move the boundary")
        XCTAssertFalse(gate.admit(now: 100.0 + 1.5 * slot, targetIntervalSeconds: interval, toleranceSeconds: tol, forced: false))
        ...
        XCTAssertTrue(gate.admit(now: gate.nextDue, targetIntervalSeconds: interval, toleranceSeconds: tol, forced: false))
...
final class SelfHealCadenceTests: XCTestCase {
    ...
    func testDisabledPassthroughAndDegenerateBase() {
        XCTAssertEqual(SelfHealCadence.effectiveEvery(baseEvery: 0, baseFps: 60, governedFps: 30), 0,
                       "AISLOPDESK_SELF_HEAL=0 stays disabled at every governed fps")
        XCTAssertEqual(SelfHealCadence.effectiveEvery(baseEvery: 6, baseFps: 0, governedFps: 30), max(2, 6 * 30),
