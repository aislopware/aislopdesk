...
final class RworkTransportErrorClassifierTests: XCTestCase {

    func testCanonicalAddressInUsePhraseMatches() {
        XCTAssertTrue(RworkTransportError.listenerDetailIndicatesAddressInUse("Address already in use"))
        XCTAssertTrue(RworkTransportError.listenerDetailIndicatesAddressInUse("POSIXErrorCode: Address already in use"))
        XCTAssertTrue(RworkTransportError.listenerDetailIndicatesAddressInUse("address already IN USE"))
    }

    func testNumericErrnoRenderingMatchesAsStandaloneToken() {
        XCTAssertTrue(RworkTransportError.listenerDetailIndicatesAddressInUse("posix(48)"))
        XCTAssertTrue(RworkTransportError.listenerDetailIndicatesAddressInUse("NWError errno 48"))
        XCTAssertTrue(RworkTransportError.listenerDetailIndicatesAddressInUse("48"))
        XCTAssertTrue(RworkTransportError.listenerDetailIndicatesAddressInUse("error 48: bind"))
    }

    func testEmbedded48DoesNotMisclassify() {
        // None of these are EADDRINUSE — "48" is part of a longer number, not a standalone errno.
        XCTAssertFalse(RworkTransportError.listenerDetailIndicatesAddressInUse("could not bind port 4843"))
        XCTAssertFalse(RworkTransportError.listenerDetailIndicatesAddressInUse("posix errno 148: network down"))
        XCTAssertFalse(RworkTransportError.listenerDetailIndicatesAddressInUse("buffer size 1048576 exceeded"))
        XCTAssertFalse(RworkTransportError.listenerDetailIndicatesAddressInUse("port 8048 refused"))
    }

    func testUnrelatedErrorsDoNotMatch() {
        XCTAssertFalse(RworkTransportError.listenerDetailIndicatesAddressInUse("Network is down"))
        XCTAssertFalse(RworkTransportError.listenerDetailIndicatesAddressInUse("Connection refused"))
        XCTAssertFalse(RworkTransportError.listenerDetailIndicatesAddressInUse(""))
    }

    func testStandaloneTokenHelperBoundaries() {
        XCTAssertTrue(RworkTransportError.containsStandaloneNumber("x 48 y", 48))
        XCTAssertTrue(RworkTransportError.containsStandaloneNumber("48", 48))
        XCTAssertTrue(RworkTransportError.containsStandaloneNumber("(48)", 48))
        XCTAssertFalse(RworkTransportError.containsStandaloneNumber("4843", 48))
        XCTAssertFalse(RworkTransportError.containsStandaloneNumber("148", 48))
        XCTAssertFalse(RworkTransportError.containsStandaloneNumber("1048576", 48))
        // Multiple occurrences: a standalone one later in the string still matches.
        XCTAssertTrue(RworkTransportError.containsStandaloneNumber("4843 then 48", 48))
    }
}
...
final class WaitingBindConflictClassifierTests: XCTestCase {
    ...
    func testEADDRINUSEIsFatalInWaiting() {
        XCTAssertTrue(RworkTransportError.waitingErrnoIsFatalBindConflict(EADDRINUSE),
                      "a stuck .waiting on EADDRINUSE never auto-recovers → surface it immediately")
        XCTAssertTrue(RworkTransportError.waitingErrnoIsFatalBindConflict(48),
    ...
    func testTransientNetworkErrnosKeepWaiting() {
        ...
        for errno in [ENETDOWN, ENETUNREACH, ETIMEDOUT, EAGAIN, ECONNREFUSED, EHOSTUNREACH, ENOTCONN] {
            XCTAssertFalse(RworkTransportError.waitingErrnoIsFatalBindConflict(errno),
                           "transient waiting errno \(errno) must keep waiting, not fail")
        }
        XCTAssertFalse(RworkTransportError.waitingErrnoIsFatalBindConflict(0),
    ...
    func testFatalWaitingDetailClassifiesAsAddressInUse() {
        XCTAssertTrue(RworkTransportError.listenerDetailIndicatesAddressInUse("Address already in use"))
        XCTAssertTrue(RworkTransportError.listenerDetailIndicatesAddressInUse("POSIXErrorCode(rawValue: 48): Address already in use"))
    }
}
