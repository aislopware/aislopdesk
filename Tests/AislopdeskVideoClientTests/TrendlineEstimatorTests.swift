...
final class TrendlineEstimatorTests: XCTestCase {
    ...
    private func feed(_ est: inout TrendlineEstimator, count: Int,
                      arrival: inout Double, ts: inout UInt32,
                      arrivalStepMs: Double = 16, sendStepMs: UInt32 = 16) {
        for _ in 0..<count {
    ...
    func testLinearRampSignalsOveruse() {
        ...
        XCTAssertLessThanOrEqual(steep ?? .max, 6,
        ...
        XCTAssertLessThanOrEqual(gentle ?? .max, 16,
    ...
    func testSendTsWrapContinuity() {
        var est = TrendlineEstimator()
        var arrival = 1000.0
        var ts: UInt32 = UInt32.max - 100   // wraps inside the run
        est.note(arrivalMs: arrival, sendTs: ts)
        for _ in 0..<200 {
            arrival += 16
            ...
            XCTAssertLessThan(abs(est.modifiedTrend), TrendlineEstimator.thresholdMin,
    ...
    func testIdleGapResetsWindow() {
        ...
        for _ in 0..<25 {
            ...
            ts &+= 16
            est.note(arrivalMs: arrival, sendTs: ts)
            if est.state == .overusing { fired = true; break }
        }
    ...
    func testVerdictGoesStaleAcrossIdleGapWithoutNewArrival() {
        ...
        XCTAssertFalse(est.isStale(nowMs: arrival + TrendlineEstimator.resetGapMs),
                       "at exactly the reset gap it is still fresh (mirrors note()'s strict >)")
        XCTAssertTrue(est.isStale(nowMs: arrival + TrendlineEstimator.resetGapMs + 1),
                      "past the reset gap the verdict is stale — NO new arrival was needed")
        XCTAssertEqual(est.state, .overusing,
                       "the estimator state itself is untouched — the gate is report-side")
        XCTAssertNotEqual(est.wireTrendFlags & 0x3, 0,
    ...
    func testWireFieldClamping() {
        ...
        XCTAssertEqual(TrendlineEstimator.packTrendMilli(-2_000_000), UInt32(bitPattern: -1_000_000_000))
        XCTAssertEqual(TrendlineEstimator.packTrendMilli(12.5), UInt32(bitPattern: 12_500))
    ...
    func testWireFlagsPackAndReportAccessorsRoundTrip() {
        ...
        XCTAssertEqual(report.owdTrendModifiedMilliSigned, -42_500)
