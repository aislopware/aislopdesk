...
final class AgentInstallerTests: XCTestCase {
    private let command = "\"/Users/dev/.claude/hooks/aislopdesk-agent.sh\""
    ...
    func testHookScriptIsRecognizedExecutableShellAndReferencesSocketEnv() {
        ...
        XCTAssertTrue(script.contains("AISLOPDESK_SOCKET_PATH"), "it reads the host's socket path from env")
    ...
    func testCommandCarriesTheMarkerViaScriptPath() {
        let cmd = AgentInstaller.hookCommand(scriptPath: "/Users/dev/.claude/hooks/aislopdesk-agent.sh")
    ...
    func testDefaultPathsHonorClaudeConfigDir() {
        ...
        XCTAssertEqual(
            ...
            "/tmp/cfg/hooks/aislopdesk-agent.sh",
        )
    ...
    func testInstallThenUninstallOnDiskIsIdempotentAndRemovable() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("aislopdesk-installer-test-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: dir) }
        let settingsPath = dir.appendingPathComponent("settings.json").path
        let scriptPath = dir.appendingPathComponent("hooks/aislopdesk-agent.sh").path
