...
final class AgentPreferencesSidecarTests: XCTestCase {
    ...
    // MARK: AISLOPDESK_AGENT_PREVENT_SLEEP — DEFAULT-OFF (`== "1"`)
    ...
    // MARK: AISLOPDESK_AGENT_RESUME_ON_RECOVERY — DEFAULT-ON (`!= "0"`)
    ...
    func testDefaultPreferencesLeaveCompileTimeDefaults() {
        ...
        XCTAssertNil(env["AISLOPDESK_AGENT_PREVENT_SLEEP"])
        XCTAssertNil(env["AISLOPDESK_AGENT_RESUME_ON_RECOVERY"])
    ...
    /// The sidecar path: a client `resumeOnRecovery: false` reaches the host as `AISLOPDESK_AGENT_RESUME_ON_RECOVERY=0`
