...
/// Proves the SHIPPED binaries: launches the real `rwork-hostd` and `rwork-client`
...
final class SubprocessE2ETests: XCTestCase {
    ...
    /// Locates a built product (e.g. `rwork-hostd`) next to the test bundle. SwiftPM puts
    /// the executables in the same `debug`/`release` directory as the xctest bundle.
    private func builtProductURL(_ name: String) -> URL? {
        let bundleURL = Bundle(for: Self.self).bundleURL // …/debug/RworkPackageTests.xctest
    ...
    func testShippedBinariesEchoOverTCP() throws {
        guard let hostdURL = builtProductURL("rwork-hostd"),
              let clientURL = builtProductURL("rwork-client") else {
            throw XCTSkip("built rwork-hostd / rwork-client not found next to test bundle")
        }

        // --- Launch rwork-hostd on an OS-chosen ephemeral port (--port 0). ---
        let hostd = Process()
        ...
        do {
            ...
            throw XCTSkip("could not launch rwork-hostd subprocess: \(error)")
        }
        defer {
        ...
        // --- Launch rwork-client --no-raw with a piped stdin script. ---
        let client = Process()
        ...
        do {
            ...
            throw XCTSkip("could not launch rwork-client subprocess: \(error)")
        }
        defer {
