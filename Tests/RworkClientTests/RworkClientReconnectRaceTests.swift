...
/// close()/pause()/resume()/concurrent-connect variants). `RworkClient` is an `actor`, so it is
...
final class RworkClientReconnectRaceTests: XCTestCase {
    ...
    func testCloseDuringInflightConnectTearsDownNewTransportNoZombie() async throws {
        for _ in 0..<200 {
            ...
            let client = RworkClient(makeTransport: { rec.make() })
