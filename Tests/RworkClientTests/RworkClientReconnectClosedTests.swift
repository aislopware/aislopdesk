...
///
/// The two deliberate-shutdown paths are asymmetric: ``RworkClient/pause()`` yields its own
/// `.disconnected` and sets `paused`, but ``RworkClient/close()`` sets `closed` and finishes the
...
final class RworkClientReconnectClosedTests: XCTestCase {
    ...
    /// `RworkClient.connect` exercises its real path. Inbound is an inert stream.
