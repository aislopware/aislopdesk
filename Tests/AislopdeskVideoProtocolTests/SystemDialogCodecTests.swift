...
final class SystemDialogCodecTests: XCTestCase {
    ...
    func testSystemDialogListRoundTripPreservesEveryField() throws {
        let dialogs = [
            ...
            SystemDialogSummary(windowID: 1966, owner: "SecurityAgent", title: "", width: 260, height: 312, isSecure: true),
            SystemDialogSummary(windowID: 1755, owner: "Open and Save Panel Service", title: "Open",
                                width: 880, height: 448, isSecure: false),
        ]
        let msg = VideoControlMessage.systemDialogList(dialogs)
        XCTAssertEqual(msg.messageType, 12)
        XCTAssertEqual(try VideoControlMessage.decode(msg.encode()), .systemDialogList(dialogs),
    ...
    func testSecureFlagRoundTripsBothWays() throws {
        let secure = SystemDialogSummary(windowID: 1, owner: "SecurityAgent", title: "Untitled", width: 9, height: 9, isSecure: true)
        let open = SystemDialogSummary(windowID: 2, owner: "UserNotificationCenter", title: "Alert", width: 9, height: 9, isSecure: false)
        XCTAssertEqual(try VideoControlMessage.decode(VideoControlMessage.systemDialogList([secure, open]).encode()),
                       .systemDialogList([secure, open]))
    }

    func testNonASCIIOwnerAndTitleRoundTrip() throws {
        let d = SystemDialogSummary(windowID: 7, owner: "Tiến trình hệ thống", title: "Mật khẩu — 密码 🔐",
                                    width: 100, height: 50, isSecure: true)
        XCTAssertEqual(try VideoControlMessage.decode(VideoControlMessage.systemDialogList([d]).encode()),
                       .systemDialogList([d]), "UTF-8 owner/title survive the length-prefixed round-trip")
    }
