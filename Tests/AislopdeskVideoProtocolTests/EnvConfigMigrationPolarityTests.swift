...
///   • `AISLOPDESK_DETACH_ENABLED`        (LivePaneSession)   `env[k] != "0"`  → `EnvConfig.boolDefaultOn`
///   • `AISLOPDESK_ADAPTIVE_QP`           (WindowCapturer)    `env[k] == "1"`  → `EnvConfig.boolDefaultOff`
///   • `AISLOPDESK_IDLE_SKIP`             (WindowCapturer)    `env[k] == "1"`  → `EnvConfig.boolDefaultOff`
///   • `AISLOPDESK_SYSTEM_DIALOG_PANES`   (AislopdeskClientApp) raw `String?` 3-state → `EnvConfig.string`
///   • `AISLOPDESK_SCROLL_RESAMPLE_HZ`    (InputInjector)     `Int?`+clamp on `env[k]`→ `EnvConfig.string`
///
/// `ProcessInfo` env is read-only at runtime, so the `{"0","1",garbage}` rows are driven through the
/// (process-wide) overlay for keys GUARANTEED absent from the real env (`AISLOPDESK_*` test keys); the
...
final class EnvConfigMigrationPolarityTests: XCTestCase {
    ...
    // MARK: default-ON (`!= "0"`) — AISLOPDESK_DETACH_ENABLED
    ...
    func testDetachEnabledPreservesDefaultOnPolarity() {
        assertMigratedIdiom(
            key: "AISLOPDESK_DETACH_ENABLED",
            accessor: EnvConfig.boolDefaultOn,
            legacy: { $0 != "0" },
        )
    }

    // MARK: default-OFF (`== "1"`) — AISLOPDESK_ADAPTIVE_QP / _IDLE_SKIP
    ...
    func testIdleSkipPreservesDefaultOffPolarity() {
        assertMigratedIdiom(
            key: "AISLOPDESK_IDLE_SKIP",
            accessor: EnvConfig.boolDefaultOff,
            legacy: { $0 == "1" },
        )
    }

    // MARK: 3-state raw String? — AISLOPDESK_SYSTEM_DIALOG_PANES
    ...
    func testSystemDialogPanesPreservesRawThreeStateString() {
        let key = "AISLOPDESK_SYSTEM_DIALOG_PANES"
        for value in [nil, "0", "force", "1", "garbage"] as [String?] {
            ...
            // The two call-site branches (verbatim from AislopdeskClientApp) over the resolved value.
    ...
    // MARK: Int-parse + clamp — AISLOPDESK_SCROLL_RESAMPLE_HZ
    ...
    func testScrollResampleHzPreservesIntParseAndClamp() {
        let key = "AISLOPDESK_SCROLL_RESAMPLE_HZ"
