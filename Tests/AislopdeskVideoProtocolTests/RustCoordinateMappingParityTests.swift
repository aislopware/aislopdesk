...
final class RustCoordinateMappingParityTests: XCTestCase {
    ...
    func testPinnedValues() {
        ...
        XCTAssertEqual(CoordinateMapping.windowPoint(normalized: VideoPoint(x: 0.5, y: 0.5), windowBounds: b), VideoPoint(x: 500, y: 500))
        XCTAssertEqual(CoordinateMapping.windowPoint(normalized: VideoPoint(x: 1, y: 1), windowBounds: b), VideoPoint(x: 900, y: 800))
        XCTAssertEqual(CoordinateMapping.cgRectToCocoa(VideoRect(x: 0, y: 0, width: 400, height: 200), primaryHeight: 1080), VideoRect(x: 0, y: 880, width: 400, height: 200))
        XCTAssertEqual(CoordinateMapping.windowPoint(pixel: VideoPoint(x: 400, y: 300), windowBoundsCG: b, backingScaleFactor: 2.0), VideoPoint(x: 300, y: 350))
    }
    ...
    func testBackingScaleMultiMonitorAndNil() {
        ...
        XCTAssertEqual(CoordinateMapping.backingScaleFactor(forWindowBoundsCG: VideoRect(x: 100, y: -1000, width: 1280, height: 800), screens: [primary, retina], primaryHeight: 1080), 2.0)
        XCTAssertNil(CoordinateMapping.backingScaleFactor(forWindowBoundsCG: VideoRect(x: 10_000, y: 0, width: 100, height: 100), screens: [primary], primaryHeight: 1080))
        XCTAssertNil(CoordinateMapping.backingScaleFactor(forWindowBoundsCG: VideoRect(x: 0, y: 0, width: 10, height: 10), screens: [], primaryHeight: 1080))
    }
    ...
    func testFuzzWindowPointAndPixelMatchNative() {
        ...
        for _ in 0..<5000 {
            ...
            let b = VideoRect(x: Double.random(in: -4000...4000, using: &rng), y: Double.random(in: -4000...4000, using: &rng),
                              width: Double.random(in: 0...4000, using: &rng), height: Double.random(in: 0...4000, using: &rng))
    ...
    func testFuzzBackingScaleMatchesNative() {
        ...
        for _ in 0..<3000 {
            ...
            let screens = (0..<count).map { _ in
                ScreenInfo(
                    cocoaFrame: VideoRect(x: Double.random(in: -3000...3000, using: &rng), y: Double.random(in: -3000...3000, using: &rng),
                                          width: Double.random(in: 0...3000, using: &rng), height: Double.random(in: 0...3000, using: &rng)),
                    backingScaleFactor: [1.0, 2.0, 3.0].randomElement(using: &rng)!
                )
            }
            let winCG = VideoRect(x: Double.random(in: -3000...3000, using: &rng), y: Double.random(in: -3000...3000, using: &rng),
                                  width: Double.random(in: 0...3000, using: &rng), height: Double.random(in: 0...3000, using: &rng))
