...
/// E9 / WI-7 (ES-E9-5, B2): pins the four `Details: *` jump commands — the `.selectDetailsTab(_:)` action +
/// its four `chord: nil` `.view` registry bindings — that switch the right-hand Details panel to a specific
...
final class DetailsTabRoutingTests: XCTestCase {
    ...
    /// `.selectDetailsTab(tab)` forwards EACH of the four tabs to the supplied `selectDetailsTab` closure
    ...
    /// four commands are never DEAD chords, just inert until the view installs the closure).
    ...
    // MARK: - Registry: the four bindings exist, are `.view`, and are chord-less

    /// Each of the four `Details: *` commands has a registered binding with the documented id, the `.view`
    ...
    /// of the four registry rows drops its `binding(for:)` to nil here.
    func testFourDetailsBindingsExistAreViewAndChordless() {
        let expected: [(id: String, tab: DetailsPanelTab)] = [
            ("view.detailsInfo", .info),
            ("view.detailsOutline", .outline),
            ("view.detailsGit", .git),
    ...
    /// The four commands surface in the cheat-sheet / palette display set (the View group of
    ...
    func testDetailsBindingsSurfaceInTheViewDisplayGroup() {
        ...
        XCTAssertTrue(
            ids.isSuperset(of: ["view.detailsInfo", "view.detailsOutline", "view.detailsGit", "view.detailsFiles"]),
            "the four Details: * jump commands surface in the View display group (palette / cheat sheet)",
        )
    }
