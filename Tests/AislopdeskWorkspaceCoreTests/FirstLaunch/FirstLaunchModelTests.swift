...
final class FirstLaunchModelTests: XCTestCase {
    ...
    func testShellShimSnippetGuardsOnOverwrite() {
        let safe = CLIShellShim.snippet(allowOverwrite: false, binary: "aislopdesk")
        for name in CLIShellShim.functionNames {
            XCTAssertTrue(
                safe.contains("command -v \(name) >/dev/null 2>&1 || \(name)() { aislopdesk \(name) \"$@\"; }"),
                "Allow-Overwrite OFF must guard \(name) with command -v",
            )
        }

        let force = CLIShellShim.snippet(allowOverwrite: true, binary: "aislopdesk")
        XCTAssertFalse(
            force.contains("command -v"),
            "Allow-Overwrite ON defines functions unconditionally (no command -v guard)",
        )
        for name in CLIShellShim.functionNames {
            XCTAssertTrue(force.contains("\(name)() { aislopdesk \(name) \"$@\"; }"))
        }
    }
}
