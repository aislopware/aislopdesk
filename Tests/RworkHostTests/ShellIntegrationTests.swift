...
final class ShellIntegrationTests: XCTestCase {
    ...
    private func makeTempDir() -> URL {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("rwork-si-test-\(UUID().uuidString)", isDirectory: true)
    ...
    func testEnabledByDefault() {
        ...
        XCTAssertTrue(ShellIntegration.isEnabled(parent: ["RWORK_SHELL_INTEGRATION": "1"]))
        XCTAssertTrue(ShellIntegration.isEnabled(parent: ["RWORK_SHELL_INTEGRATION": "yes"]))
    }

    func testDisabledByFalsyOptOut() {
        for value in ["0", "false", "no", "off", "OFF", "False", "No"] {
            XCTAssertFalse(
                ShellIntegration.isEnabled(parent: ["RWORK_SHELL_INTEGRATION": value]),
                "RWORK_SHELL_INTEGRATION=\(value) must disable the shim"
            )
        }
    }
    ...
    func testNoOverridesWhenOptedOut() {
        let overrides = ShellIntegration.makeEnvironmentOverrides(
            parent: ["HOME": "/Users/x", "RWORK_SHELL_INTEGRATION": "0"],
            shellPath: "/bin/zsh",
            tmpDir: makeTempDir()
        )
    ...
    func testOverridesPointZDotDirAtFreshShimAndCarryRealZDotDir() {
        ...
        XCTAssertEqual(env?["RWORK_REAL_ZDOTDIR"], "/Users/x")
    }
    ...
    func testRealZDotDirHonoursInheritedZDotDir() {
        ...
        XCTAssertEqual(overrides?["RWORK_REAL_ZDOTDIR"], "/Users/x/.config/zsh",
    ...
    func testZshrcInstallsChainedResetPromptWinchHook() throws {
        ...
        XCTAssertTrue(zshrc.contains("$__rwork_real/.zshrc"), "must source the real .zshrc")
        // Chains any pre-existing TRAPWINCH, then unconditionally reset-prompt under ZLE.
        XCTAssertTrue(zshrc.contains("functions[__rwork_user_winch]=$functions[TRAPWINCH]"),
        ...
        XCTAssertTrue(zshrc.contains("ZDOTDIR=\"$__rwork_real\"") || zshrc.contains("unset ZDOTDIR"),
    ...
    func testZshrcInstallsOSC133HooksViaAddZshHook() throws {
        ...
        XCTAssertTrue(zshrc.contains("add-zsh-hook preexec __rwork_osc133_preexec"),
                      "must register the preexec hook via add-zsh-hook")
        XCTAssertTrue(zshrc.contains("add-zsh-hook precmd  __rwork_osc133_precmd")
                        || zshrc.contains("add-zsh-hook precmd __rwork_osc133_precmd"),
    ...
    func testZshrcCapturesExitCodeFirstInPrecmd() throws {
        ...
        guard let funcRange = zshrc.range(of: "__rwork_osc133_precmd() {") else {
        ...
        XCTAssertEqual(firstNonEmptyLine, "local __rwork_exit=$?",
    ...
    func testZshrcOSC133IsGatedByRworkOSC133() throws {
        let dir = try XCTUnwrap(ShellIntegration.writeShimDirectory(into: makeTempDir()))
        let zshrc = try String(contentsOf: dir.appendingPathComponent(".zshrc"), encoding: .utf8)
        // A finer opt-out keeping the resize fix: RWORK_OSC133=0 skips just the marks.
        XCTAssertTrue(zshrc.contains("RWORK_OSC133"),
                      "OSC 133 emission must be gated by RWORK_OSC133")
        XCTAssertTrue(zshrc.contains("0|false|no|off"),
                      "RWORK_OSC133 must accept the standard falsy opt-out values")
    }
    ...
    func testEnvShimsForwardRealStartupFilesAndReassertZDotDir() throws {
        ...
        for name in [".zshenv", ".zprofile", ".zlogin"] {
            ...
            XCTAssertTrue(body.contains("source \"$__rwork_real/\(name)\""),
                          "\(name) must forward to the real \(name)")
            XCTAssertTrue(body.contains("ZDOTDIR=\"$__rwork_shim\""),
