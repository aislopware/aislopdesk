...
final class MuxChannelSessionBackpressureTests: XCTestCase {
    ...
    /// host's temp dir accumulated one `rwork-zdotdir-*` dir + 4 files per opened pane forever. Driven
    ...
    func testShutdownDeletesTheShimDirectory() throws {
        ...
        let dir = fm.temporaryDirectory.appendingPathComponent("rwork-zdotdir-\(UUID().uuidString)", isDirectory: true)
