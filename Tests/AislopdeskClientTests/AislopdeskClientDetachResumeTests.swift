...
/// Tests for the AISLOPDESK_DETACH_ENABLED resume-identity feed (Stage 2, C1/C4) and the
/// cold-launch scrollback replay contract (AISLOPDESK_SCROLLBACK_PERSIST, Stage 3).
///
...
final class AislopdeskClientDetachResumeTests: XCTestCase {
    ...
    /// COLD LAUNCH contract (AISLOPDESK_SCROLLBACK_PERSIST):
    ...
    func testWarmReconnectUsesLiveSeqNotSeededZero() async throws {
        ...
        let client = AislopdeskClient(makeTransport: { firstTransport })
        ...
        let client2 = AislopdeskClient(makeTransport: { warmTransport })
