...
/// Tests for the AISLOPDESK_DETACH_ENABLED capture path (Stage 2, C2/C3) and WorkspaceStore wiring.
///
...
final class DetachResumeIdentityTests: XCTestCase {
    ...
    private func makeVM() -> ConnectionViewModel {
        ConnectionViewModel(
            ...
            target: { .default },
            makeClient: { AislopdeskClient(makeTransport: { fatalError("not used in detach resume tests") }) },
        )
    }
    ...
    /// COLD LAUNCH contract (AISLOPDESK_SCROLLBACK_PERSIST, Stage 3):
