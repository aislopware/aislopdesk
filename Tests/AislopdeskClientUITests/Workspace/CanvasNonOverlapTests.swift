...
final class CanvasNonOverlapTests: XCTestCase {
    ...
    func testSlideTucksIntoInsideCorner() {
        ...
        let r = CanvasNonOverlap.slide(box.offsetBy(dx: 300, dy: 300), from: box.origin, bodies: [right, below], config: cfg)
    ...
    func testMakeSpacePartsTwoNeighboursSymmetrically() {
        ...
        let result = CanvasNonOverlap.makeSpace(target: target, draggedID: .pane(PaneID()), bodies: [left, right], config: cfg)
        let r = try! XCTUnwrap(result, "insert intent should arm")
        // 100 overlap + 16 gutter = 116 each, symmetric.
        XCTAssertEqual(r.frames[.pane(lID)]!.minX, -116, accuracy: 0.01, "left parted left by overlap+gutter")
        XCTAssertEqual(r.frames[.pane(rID)]!.minX, 416, accuracy: 0.01, "right parted right symmetrically")
        XCTAssertTrue(separated(r.frames[.pane(lID)]!, target))
        XCTAssertTrue(separated(r.frames[.pane(rID)]!, target))
        XCTAssertEqual(r.frames.count, 3, "dragged + both neighbours")
    }

    func testMakeSpaceLeavesDraggedAtDropExactly() {
        let left = pane(CGRect(x: 0, y: 0, width: 300, height: 300))
        let right = pane(CGRect(x: 300, y: 0, width: 300, height: 300))
        let dID = PaneID()
        let target = CGRect(x: 200, y: 50, width: 200, height: 200)
        let r = CanvasNonOverlap.makeSpace(target: target, draggedID: .pane(dID), bodies: [left, right], config: cfg)!
    ...
    func testIntentGateDisarmsBelowCoverage() {
        ...
        XCTAssertNil(CanvasNonOverlap.makeSpace(target: target, draggedID: .pane(PaneID()), bodies: [l, r], config: cfg))
    }
    ...
    func testMakeSpaceCascadesWithoutOutputOverlap() {
        ...
        let r = CanvasNonOverlap.makeSpace(target: target, draggedID: .pane(PaneID()), bodies: bodies, config: cfg)!
    ...
    func testMakeSpaceIsOrderIndependentUnderPermutedBodies() {
        let l = pane(CGRect(x: 0, y: 0, width: 300, height: 300))
        let r = pane(CGRect(x: 300, y: 0, width: 300, height: 300))
        let dID = PaneID()
        let target = CGRect(x: 200, y: 50, width: 200, height: 200)
        let a = CanvasNonOverlap.makeSpace(target: target, draggedID: .pane(dID), bodies: [l, r], config: cfg)!
        let b = CanvasNonOverlap.makeSpace(target: target, draggedID: .pane(dID), bodies: [r, l], config: cfg)!
        XCTAssertEqual(a, b)
    }

    func testDensePackTerminatesFiniteAndInBound() {
        var bodies: [CanvasNonOverlap.Body] = []
    ...
    func testGroupBodyIsSeparatedLikeAPane() {
        ...
        let r = CanvasNonOverlap.makeSpace(target: target, draggedID: .pane(PaneID()), bodies: [group], config: cfg)!
    ...
    func testClampResizeStopsGrowingEdgeAtNeighbour() {
        ...
        let out = CanvasNonOverlap.clampResize(grown, anchor: .right, bodies: [neighbour], minSize: Canvas.minItemSize, config: cfg)
    ...
    func testClampResizeIgnoresShrink() {
        ...
        let out = CanvasNonOverlap.clampResize(shrunk, anchor: .right, bodies: [neighbour], minSize: Canvas.minItemSize, config: cfg)
    ...
    func testClampResizeDisabledIsIdentity() {
        let neighbour = pane(CGRect(x: 380, y: 0, width: 200, height: 200))
        let grown = CGRect(x: 0, y: 0, width: 400, height: 200)
        XCTAssertEqual(CanvasNonOverlap.clampResize(grown, anchor: .right, bodies: [neighbour], minSize: Canvas.minItemSize, config: .disabled), grown)
    }
    ...
    func testCollisionBodiesIsUngroupedPanesPlusGroupBoxes() {
        ...
        let dragU = canvas.collisionBodies(excludingPane: u, excludingGroup: nil, region: region, groups: [PaneGroup(id: gid, name: "G")])
        ...
        let dragG1 = canvas.collisionBodies(excludingPane: g1, excludingGroup: gid, region: region, groups: [PaneGroup(id: gid, name: "G")])
        XCTAssertEqual(dragG1.map(\.id), [.pane(u)])
    }

    func testApplyingDistributesGroupDeltaToAllMembers() {
        let g1 = PaneID(), g2 = PaneID()
        let gid = PaneGroupID()
        let canvas = Canvas(items: [
            CanvasItem(id: g1, spec: spec(), frame: CGRect(x: 100, y: 100, width: 200, height: 200), z: 0, groupID: gid),
            CanvasItem(id: g2, spec: spec(), frame: CGRect(x: 350, y: 100, width: 200, height: 200), z: 1, groupID: gid),
        ])
        let box = canvas.groupBoundingBox(gid)!
        ...
        XCTAssertEqual(out.frame(of: g2)?.origin, CGPoint(x: 410, y: 140), "member 2 shifted rigidly (internal layout preserved)")
    }
    ...
    func testDisabledConfigIsIdentity() {
        ...
        XCTAssertEqual(CanvasNonOverlap.slide(target, from: box.origin, bodies: [n], config: .disabled).frame, target,
                       "disabled slide is identity")
        XCTAssertNil(CanvasNonOverlap.makeSpace(target: target, draggedID: .pane(PaneID()), bodies: [n], config: .disabled),
