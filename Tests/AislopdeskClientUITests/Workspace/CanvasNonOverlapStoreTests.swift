...
final class CanvasNonOverlapStoreTests: XCTestCase {
    ...
    private func makeStore(_ items: [CanvasItem], focused: PaneID, groups: [PaneGroup] = []) -> WorkspaceStore {
        WorkspaceStore(restoring: Workspace(canvas: Canvas(items: items), focusedPane: focused, groups: groups),
    ...
    func testSlidesFlushWhenNotInserting() {
        ...
        XCTAssertEqual(store.workspace.canvas.frame(of: a)!.maxX, 384, accuracy: 0.3, "flush at B.minX − gutter")
        XCTAssertEqual(store.workspace.canvas.frame(of: b)!.minX, 400, accuracy: 1e-6, "neighbour unmoved")
    }

    func testInsertPartsTwoNeighboursSymmetrically() {
        ...
        XCTAssertEqual(store.workspace.canvas.frame(of: d)!.origin, CGPoint(x: 200, y: 0), "dragged pinned at the drop")
        XCTAssertEqual(store.workspace.canvas.frame(of: a)!.minX, -116, accuracy: 0.01, "left parted left")
        XCTAssertEqual(store.workspace.canvas.frame(of: b)!.minX, 416, accuracy: 0.01, "right parted right")
    }

    func testDisabledBypassIsPlainMoveEvenIfOverlapping() {
        let a = PaneID(), b = PaneID()
        let store = makeStore([
            item(a, CGRect(x: 0, y: 0, width: 200, height: 200)),
            item(b, CGRect(x: 400, y: 0, width: 200, height: 200)),
        ...
        XCTAssertEqual(store.workspace.canvas.frame(of: a)!.minX, 350, accuracy: 1e-6, "no slide under ⌘ bypass")
    }

    // MARK: - Groups

    func testMoveGroupPartsOverlappedNeighbour() {
        ...
        XCTAssertEqual(store.workspace.canvas.frame(of: m)!.minX, 250, accuracy: 0.01, "the group's member moved rigidly to the box target")
        XCTAssertEqual(store.workspace.canvas.frame(of: p)!.minX, 566, accuracy: 0.01, "the neighbour parted (overlap + gutter)")
    }

    func testResizeGroupShovesNeighbour() {
        let m = PaneID(), p = PaneID(), gid = PaneGroupID()
        let store = makeStore([
        ...
        XCTAssertEqual(store.workspace.canvas.frame(of: m), CGRect(x: 0, y: 0, width: 400, height: 200), "member rescaled to fill the new box")
        XCTAssertEqual(store.workspace.canvas.frame(of: p)!.minX, 416, accuracy: 0.01, "neighbour shoved one gutter clear of the grown box")
    }

    func testResizeGroupShrinkKeepsMembersNonOverlapping() {
        ...
        let f1 = store.workspace.canvas.frame(of: m1)!
        let f2 = store.workspace.canvas.frame(of: m2)!
        XCTAssertTrue(f1.intersection(f2).isNull, "members must not overlap after a heavy group shrink")
    }

    func testWithinGroupMembersReflowOnMemberMove() {
        let m1 = PaneID(), m2 = PaneID(), gid = PaneGroupID()
        let store = makeStore([
        ...
        XCTAssertEqual(store.workspace.canvas.frame(of: m1)!.minX, 150, accuracy: 0.01, "dragged member at its target")
        XCTAssertEqual(store.workspace.canvas.frame(of: m2)!.minX, 366, accuracy: 0.01, "sibling reflowed clear")
    }

    func testCommitFocusesAndRaisesDragged() {
        let a = PaneID(), b = PaneID()
        let store = makeStore([
            item(a, CGRect(x: 0, y: 0, width: 200, height: 200)),
            item(b, CGRect(x: 400, y: 0, width: 200, height: 200)),
        ...
        XCTAssertEqual(store.workspace.canvas.item(a)!.z, store.workspace.canvas.maxZ, "and raised to front")
    }
}
