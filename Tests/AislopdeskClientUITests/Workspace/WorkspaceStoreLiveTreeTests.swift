...
final class WorkspaceStoreLiveTreeTests: XCTestCase {
    ...
    private func writeV9Fixture() throws -> (WorkspacePersistence, [PaneID]) {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("aislopdesk-w5-\(UUID().uuidString)", isDirectory: true)
    ...
    // MARK: - 1. The store-flip end-to-end: v9 file → migrate → tree → materialized live
    ...
    func testLiveTreeStoreMigratesV9FileAndMaterializesTreeLeaves() throws {
        ...
        XCTAssertEqual(
            ...
            Set(v9PaneIDs),
            "every v9 PaneID survived the migration into the tree",
        )

        // The LIVE store: liveModel == .tree ⇒ init reconciles the TREE, so the registry materializes the
        // migrated tree's leaves through the FakePaneSession factory (NOT the canvas).
        let store = treeStore(persistence: persistence, restoringTree: restoredTree)

        XCTAssertEqual(
            registryIDs(store),
            Set(v9PaneIDs),
            "init reconciled the LIVE tree — the registry backs the migrated tree leaves",
        )
        XCTAssertEqual(store.allSessions.count, v9PaneIDs.count, "one live handle per migrated leaf")
        for id in v9PaneIDs {
            let handle = store.handle(for: id)
            XCTAssertNotNil(handle, "tree leaf \(id) materialized at init")
            XCTAssertEqual(handle?.id, id, "the handle adopted its leaf id")
            XCTAssertEqual((handle as? FakePaneSession)?.events.first, .adopt(id), "materialized via adopt(id:)")
        }
        // The migrated tree carries the v9 connection onto its session (the gate prefill source).
        XCTAssertEqual(store.tree.activeSession?.connection?.host, "10.0.0.7")
    }
