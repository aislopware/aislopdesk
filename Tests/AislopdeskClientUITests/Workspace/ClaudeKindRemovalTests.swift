...
final class ClaudeKindRemovalTests: XCTestCase {
    ...
    // MARK: - 3. Migration rewrites a legacy claude pane (no data loss)

    func testMigrationRewritesLegacyClaudeCodePaneToTerminal() throws {
        let claudeID = UUID()
        let json = """
        {
          "schemaVersion": 9,
            ...
            "items": [
              {
                ...
                "spec": { "kind": "claudeCode", "title": "agent" },
        ...
        """
        let v9 = try JSONDecoder().decode(WorkspaceV9.self, from: Data(json.utf8))
        let v10 = WorkspaceMigrationV9toV10.migrate(v9)
        let paneID = PaneID(raw: claudeID)
        XCTAssertTrue(v10.allPaneIDs().contains(paneID), "the pane is not lost")
        XCTAssertEqual(v10.spec(for: paneID)?.kind, .terminal, "claudeCode → terminal")
        XCTAssertEqual(v10.spec(for: paneID)?.title, "agent", "title preserved")
