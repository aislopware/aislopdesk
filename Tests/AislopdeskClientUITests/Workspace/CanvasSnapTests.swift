...
final class CanvasSnapTests: XCTestCase {
    ...
    private var paneOnly: CanvasSnap.Config {
        var c = config; c.snapsToGrid = false; return c
    }
    ...
    func testNearTieSelectionIsOrderIndependentWithThreeCandidates() {
        ...
        let gutterSource = CGRect(x: 42, y: 500, width: 50, height: 50)    // maxX 92 → gutter target 108 (δ+8.0); edge 92 (δ−8.0)
        ...
        for others in orders {
            ...
            XCTAssertEqual(r.frame.minX, 107.6, accuracy: eps,
    ...
    func testMoveEngageBoundary() {
        ...
        let near = CanvasSnap.move(CGRect(x: 107.9, y: 855, width: 150, height: 80),
                                   others: [other], config: paneOnly)
        XCTAssertEqual(near.frame.minX, 100, accuracy: eps, "7.9 ≤ engage 8 → snaps")
        let far = CanvasSnap.move(CGRect(x: 108.2, y: 855, width: 150, height: 80),
                                  others: [other], config: paneOnly)
    ...
    func testHeldStickSurvivesToReleaseThenLandsUnderPointer() {
        let other = CGRect(x: 100, y: 500, width: 200, height: 100)
        let start = CanvasSnap.move(CGRect(x: 105, y: 855, width: 150, height: 80),
                                    others: [other], config: paneOnly)
        XCTAssertEqual(start.frame.minX, 100, accuracy: eps)

        // Drift to 11.9pt past the target: still held (release is 12).
        let held = CanvasSnap.move(CGRect(x: 111.9, y: 855, width: 150, height: 80),
                                   others: [other], config: paneOnly, previous: start)
        ...
        let released = CanvasSnap.move(CGRect(x: 112.1, y: 855, width: 150, height: 80),
                                       others: [other], config: paneOnly, previous: held)
    ...
    func testHeldStickIgnoresNearerNewcomer() {
        ...
        let start = CanvasSnap.move(CGRect(x: 102, y: 855, width: 150, height: 80),
                                    others: [a], config: paneOnly)
        XCTAssertEqual(start.frame.minX, 100, accuracy: eps)
        let held = CanvasSnap.move(CGRect(x: 106, y: 855, width: 150, height: 80),
                                   others: [a, b], config: paneOnly, previous: start)
    ...
    func testHeldStickReleasesWhenSourceVanishes() {
        ...
        let start = CanvasSnap.move(CGRect(x: 105, y: 855, width: 150, height: 80),
                                    others: [other], config: paneOnly)
        XCTAssertEqual(start.frame.minX, 100, accuracy: eps)
        let after = CanvasSnap.move(CGRect(x: 105, y: 855, width: 150, height: 80),
                                    others: [], config: paneOnly, previous: start)
    ...
    func testCorrectionNeverExceedsRelease() {
        ...
        while x <= 120 {
            ...
            XCTAssertLessThan(abs(r.frame.minX - x), config.release + eps,
    ...
    func testMoveSnapsToViewportInsetEdgeAndCenter() {
        ...
        let edge = CanvasSnap.move(CGRect(x: 21, y: 855, width: 150, height: 80),
                                   others: [], viewport: viewport, config: paneOnly)
        ...
        let center = CanvasSnap.move(CGRect(x: 520, y: 855, width: 150, height: 80),
                                     others: [], viewport: viewport, config: paneOnly)
    ...
    func testDisabledConfigSnapsNothing() {
        ...
        let r = CanvasSnap.move(dragged, others: [other],
    ...
    func testGuideDrawsAtExactSnappedPositionAndNeverDuplicates() {
        ...
        XCTAssertEqual(vertical.first?.position ?? 0, 100.7, accuracy: eps,
    ...
    func testGutterGuideKind() {
        ...
        XCTAssertEqual(r.guides.first?.position ?? 0, 316, accuracy: eps,
    ...
    func testResizeHysteresisHoldsAcrossFrames() {
        ...
        let first = CanvasSnap.resize(CGRect(x: 50, y: 600, width: 246, height: 200),
                                      anchor: .right, others: [other], config: paneOnly)
        ...
        let held = CanvasSnap.resize(CGRect(x: 50, y: 600, width: 260, height: 200),
                                     anchor: .right, others: [other], config: paneOnly, previous: first)
        XCTAssertEqual(held.frame.maxX, 300, accuracy: eps, "edge held inside the release band")
        // 13pt past: released, raw passes through.
        let released = CanvasSnap.resize(CGRect(x: 50, y: 600, width: 263, height: 200),
                                         anchor: .right, others: [other], config: paneOnly, previous: held)
