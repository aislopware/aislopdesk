// SlateTheme ← ThemeDocument tests (E15 WI-1) — the built-in ANSI palettes + the `init(document:)` bridge
// that turns a scanned custom `.aislopdesktheme` into a full chrome theme. Asserts on the STRING/Bool-typed fields
...
@MainActor
final class SlateThemeFromDocumentTests: XCTestCase {
    ...
    func testDarkDocumentDataFlowsIntoTheTheme() {
        let theme = SlateTheme(document: darkDoc())
    ...
    func testLightModeProducesALightTheme() {
        ...
        doc.mode = .light
        doc.slug = "solar"
        let theme = SlateTheme(document: doc)
    ...
    func testBuiltInThemesShipA16EntryPalette() {
        for theme in [
            SlateTheme.monokaiProClassic, .monokaiProClassicLight, .monokaiProOctagon,
    ...
    func testMonokaiClassicPaletteIsCanonical() {
        let p = SlateTheme.monokaiProClassic.ansiPalette
