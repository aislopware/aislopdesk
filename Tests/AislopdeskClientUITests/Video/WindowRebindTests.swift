...
final class WindowRebindTests: XCTestCase {
    ...
    func testLiveIDSameAppKeeps() {
        ...
        XCTAssertEqual(WindowRebind.resolve(windowID: 58, appName: "Code",
                                            title: "main.swift — aislopdesk", in: list), .keep)
    }
    ...
    func testRecycledIDRebindsByApp() {
        ...
        XCTAssertEqual(WindowRebind.resolve(windowID: 215, appName: "Code",
                                            title: "old.swift — aislopdesk", in: list),
                       .rebind(list[1]))
    }
    ...
    func testStaleIDExactTitleWins() {
        ...
        XCTAssertEqual(WindowRebind.resolve(windowID: 999, appName: "Code",
                                            title: "b.swift — proj", in: list), .rebind(list[1]))
    }
    ...
    func testStaleIDSoleAppWindowWins() {
        ...
        XCTAssertEqual(WindowRebind.resolve(windowID: 999, appName: "Code",
                                            title: "main.swift — proj", in: list), .rebind(list[0]))
    }

    func testStaleIDTitleContainmentTiebreak() {
        let list = [win(90, "Terminal", "zsh"), win(91, "Terminal", "vim — notes.md")]
        XCTAssertEqual(WindowRebind.resolve(windowID: 999, appName: "Terminal",
                                            title: "notes.md", in: list), .rebind(list[1]))
    }

    func testStaleIDMultipleNoTitleMatchTakesFirst() {
        let list = [win(90, "Terminal", "zsh"), win(91, "Terminal", "htop")]
        XCTAssertEqual(WindowRebind.resolve(windowID: 999, appName: "Terminal",
                                            title: "completely different", in: list), .rebind(list[0]))
    }
    ...
    func testAppGoneIsUnresolved() {
        ...
        XCTAssertEqual(WindowRebind.resolve(windowID: 999, appName: "Safari", title: "Apple", in: list),
                       .unresolved)
    }
    ...
    func testStaleIDNoSavedAppIsUnresolved() {
        ...
        let list = [win(58, "Code", "x")]
        XCTAssertEqual(WindowRebind.resolve(windowID: 999, appName: "", title: "x", in: list),
                       .unresolved)
    }
}
