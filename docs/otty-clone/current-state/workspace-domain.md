...
- **Tree model** (`TreeWorkspace` / `Session` / `Tab` / `SplitNode`) — the `Session → Tab → Pane` redesign (docs/42), promoted to live in the W5 cutover. `WorkspaceStore.liveModel == .tree` drives the IDE shell and all tree-path unit tests; it is the **current live model in the otty UI build**.

All assessments below focus on the **tree path** (the otty target) unless noted.
...
`saveLayoutPreset(name:triggerAppName:)` snapshots `workspace.canvas`. On the tree path the concept is covered by `SessionTemplate` (capture) but that is a different abstraction: a template spawns a new session, it does not restore the current session's geometry by name. **Gap: otty-spec ⌘S "save layout" for the tree shell is not implemented.**
