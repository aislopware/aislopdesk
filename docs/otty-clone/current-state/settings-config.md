...
- **`AislopdeskSettingsScene`** — a stock SwiftUI `Settings` scene (⌘,) that opens a separate system-chromed window on macOS. It hosts `SettingsView`, a `TabView` with five tabs: General, Terminal, Video, Keybindings, Advanced.
- **`PreferencesStore`** — the single `@MainActor @Observable` source of truth, backed by `UserDefaults` and a `video-prefs.json` sidecar. Lives in `AislopdeskWorkspaceCore` (headless-testable).
...
- **`EnvConfig` + `EnvBridge`** — the precedence chain (`ProcessInfo env → overlay → compile-time default`) and the bridge that maps typed prefs models onto `AISLOPDESK_*` env keys.
...
|---|---|---|
| **Settings window** (macOS ⌘, / stock scene) | done | `SettingsView.swift:34–46` — `AislopdeskSettingsScene` wraps `SettingsView` in a SwiftUI `Settings` scene; wired in `AislopdeskClientApp.swift:293–295` |
...
| **Typed preferences models** | done | `VideoPreferences.swift`, `TerminalPreferences.swift`, `AgentPreferences.swift`, `AppearancePreferences.swift`, `KeybindingPreferences.swift` — all in `Sources/AislopdeskVideoProtocol/Settings/`; all `Codable + Sendable + Equatable` |
| **Themes — Monokai Pro 6-filter + Paper + Dark + System** | done | `AppearancePreferences.swift:31–42` — `ThemeChoice` enum with 9 cases; `ThemeStore.swift:44–66` — `apply(_:)` resolves to `OttyTheme`; wired in `AislopdeskClientApp.swift:65–67` |
| **Theme live-apply across NSSplitViewController boundary** | done | `ThemeStore.swift:33` — `didChangeNotification`; `AislopdeskSplitViewController.swift:118,194` — observes and re-pins window appearance + re-injects hosted columns |
...
| **Advanced settings (raw `AISLOPDESK_*` overrides)** | done | `SettingsView.swift:353–412` — `AdvancedSettingsTab` with a `TextEditor` for `KEY=value` lines; commits into `store.rawOverrides`; folded LAST into overlay (typed prefs win over raw) |
...
| **Import/export UI (file picker in app menu or settings)** | missing | `exportWorkspaceData` / `importWorkspace` have zero call sites in any view or `Commands` group; no `.fileExporter` / `.fileImporter` modifier anywhere in `AislopdeskClientUI`. The codec + store methods exist but are not surfaced |
...
| **Snippet management UI (create/edit/delete)** | missing | `Snippet.swift` domain model exists; `WorkspaceStore` has `addSnippet`, `updateSnippet`, `deleteSnippet` methods (lines 1376–1409). No view in `AislopdeskClientUI` creates or edits snippets. `PaletteDataSource.swift` has no snippet entries. |
...
| **Config file format (`~/.config/aislopdesk/` or equivalent)** | na-remote | Aislopdesk is a remote-coding tool; configuration is stored in `UserDefaults` + the `video-prefs.json` sidecar (under `Application Support/Aislopdesk/`). There is no config-file reader/writer for a dotfile format. The otty spec's file-format reference describes a future target. |
...
Sources/AislopdeskClientUI/Settings/KeybindingsEditorView.swift
Sources/AislopdeskClientUI/AislopdeskClientApp.swift           — scene wiring (AislopdeskSettingsScene)
Sources/AislopdeskClientUI/Footer/PreferencesEnvironment.swift — env slot for deep view injection
Sources/AislopdeskClientUI/DesignSystem/ThemeStore.swift

Sources/AislopdeskWorkspaceCore/Workspace/Store/PreferencesStore.swift
Sources/AislopdeskWorkspaceCore/Workspace/Store/SettingsKey.swift
Sources/AislopdeskWorkspaceCore/Workspace/Store/AppearanceApplier.swift
Sources/AislopdeskWorkspaceCore/Workspace/Domain/WorkspaceTransfer.swift
Sources/AislopdeskWorkspaceCore/Workspace/Domain/Snippet.swift

Sources/AislopdeskVideoProtocol/Settings/EnvConfig.swift
Sources/AislopdeskVideoProtocol/Settings/EnvBridge.swift
Sources/AislopdeskVideoProtocol/Settings/TerminalPreferences.swift
Sources/AislopdeskVideoProtocol/Settings/VideoPreferences.swift
Sources/AislopdeskVideoProtocol/Settings/AgentPreferences.swift
Sources/AislopdeskVideoProtocol/Settings/AppearancePreferences.swift
Sources/AislopdeskVideoProtocol/Settings/KeybindingPreferences.swift
Sources/AislopdeskVideoProtocol/Settings/TerminalConfigBuilder.swift

Sources/AislopdeskHost/ClaudeCodeProfile.swift                 — only Term enum survives
```
...
2. **Snippet management UI absent.** Domain model, expander, send-keys parser, and CRUD store methods are all done. `PaletteDataSource.swift` has no snippet entries, and `AislopdeskClientUI` has no snippet editor view. Snippets can be neither created nor run from the current UI.
...
4. **Font picker is a plain TextField.** The Terminal settings tab accepts a font family name as raw text; there is no `NSFontPanel` integration, font browser, or font-size preview. This is functional but lower-fidelity than otty's design.
