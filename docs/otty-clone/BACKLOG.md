# Otty Clone — Implementation Backlog (Epics)
...
- **ottyRefs**: spec files implemented. **gapRefs** in prose map back to `GAP-ANALYSIS.md` rows.
...
| 21 | E21 Remote-window extension first-class through clone surfaces | 2 | M | E2,E6,E10,E11 |

---

## E1 — Default-keymap parity & command routing completion
**Goal.** Make the binding registry match otty's default chords and register every action the clone needs, so later epics can bind their commands without touching routing. Reuse `WorkspaceBindingRegistry` (single source of truth), `WorkspaceBindingRouting.routeTree`, override pipeline — all done; this fills the action/chord gaps and adds missing `routeTree` cases.
**ottyRefs.** `spec/customization__custom-keybindings.md`, `spec/reference__keybindings.md`, `spec/user-interface__window-tab-split.md`.
**Scope.** Register split-left/up (A7), sequential pane-cycle `⌘]`/`⌘[` (A10), scroll keys `⇧PageUp/Down`/`⇧Home/End` + command-jump `⌘PageUp/Down` (I12/I16), font-size `⌘+`/`⌘-`/`⌘0` (M8), `⌘⇧E`/`⌘⇧M`/`⌘⇧P`/`⌘⇧O`/`⌘⇧F`/`⌘⌃↩` action stubs; reconcile pane chords to otty's `⌘⌃`/`⌘⌃⇧` defaults (A8/A9/A11/A25); add `routeTree` cases for reopen-closed (delegated to E3) and palette/cheat/find (delegated to E2). Text/Sequence (`text:`/`csi:`/`esc:`) bindings (N4) + `unbind:`/param-action parse (N5). Optional: SwiftUI `.commands` menu-bar (N6).
...
**ottyRefs.** `spec/user-interface__command-palette.md`, `spec/customization__custom-keybindings.md`, `spec/user-interface__find.md`.
...
**ottyRefs.** `spec/user-interface__window-tab-split.md`.
...
**ottyRefs.** `spec/user-interface__details-panel.md`, `spec/agents__history.md`, `spec/terminal-features__shell-integration.md`.
...
**ottyRefs.** `spec/user-interface__find.md`.
...
**Goal.** Bring `OttyTabRow` to otty's visual spec and make the sort hamburger real.
**ottyRefs.** `spec/user-interface__window-tab-split.md`, `spec/terminal-features__progress-state.md`, `spec/agents__parallel-tasks.md`.
**Scope.** Render `RailRow.status` dot on `OttyTabRow` (A20/O6) — spinner/check/dot/error/hand from `ClaudeStatus` + OSC-133 command state + caffeinate/sudo (via E4 processes). Show `#N` number badge + cwd subtitle + shell/process trailing label (A14). Add sidebar **tab search/filter** (call existing `RailRowsBuilder.filtered`). Implement **grouping** (None/By-Project via git-toplevel from E4/By-Date) and **sort** (Created/Updated/Manual) — make the hamburger mutate store order, not local `@State`.
**Est** L · **Pri** 2 · **dependsOn** E1, E3, E4
**Acceptance:** ES-E6-1…ES-E6-5.

## E7 — Settings sections parity + iOS settings + import/export surfacing
**Goal.** Expand the 5-tab settings to otty's section taxonomy, add the missing toggles, surface workspace import/export, and add an iOS settings sheet.
**ottyRefs.** `spec/customization__{advanced-settings,import-export}.md`, `spec/getting-started__first-launch.md`, `spec/terminal-features__{cursor-and-mouse,scroll,copy-and-paste,input,notifications}.md`.
...
**Goal.** Add the client-side selection/copy/scroll/input behaviors otty exposes as Controls toggles, all gated by E7 settings.
**ottyRefs.** `spec/terminal-features__{selection,copy-and-paste,scroll,input,cursor-and-mouse}.md`.
...
**ottyRefs.** `spec/user-interface__details-panel.md`, `spec/user-interface__outline.md`.
...
**ottyRefs.** `spec/user-interface__files-and-links.md`, `spec/user-interface__status-bar.md`, `spec/terminal-features__hint-mode.md`, `spec/user-interface__outline.md`.
...
**ottyRefs.** `spec/user-interface__open-quickly.md`, `spec/user-interface__outline.md`.
...
**ottyRefs.** `spec/agents__composer.md`, `spec/agents__prompt-queue.md`, `spec/terminal-features__input.md`.
...
**Goal.** Surface the complete host detection stack as otty's agent experience: install card, behavior toggles, tab badges, Send-to-Chat, History, Resume/Fork. Claude Code only.
**ottyRefs.** `spec/agents__{setup,supported-agents,agents-overview,send-to-chat,history,fork-branch-session,parallel-tasks}.md`, `spec/getting-started__first-launch.md`.
...
**ottyRefs.** `spec/terminal-features__{progress-state,notifications}.md`.
...
**Goal.** Add the theme editor, custom/imported themes, and the font-settings otty exposes.
**ottyRefs.** `spec/customization__{themes,fonts}.md`, `spec/terminal-features__unicode-and-text-styles.md`.
...
**ottyRefs.** `spec/customization__custom-commands.md`, `spec/user-interface__window-tab-split.md`.
**Scope.** Tree-path **Save Layout `⌘S`** (scope tab/window, content layout/commands) → `.ottyrecipe`-style TOML (A28/N7); Open Recipe restore via `reconcileTree`; portable-path vars; command-replay modes (Auto/Ask-Once/Manually/Skip) + trust-hash store; shell-handoff pause via OSC-133. **Snippets UI** (N8): editor (Name/Alias/Text + `{{clipboard}}`/`{{date}}`/`{{time}}`/`{{cursor}}`), alias expansion at prompt, palette entries — reuse `SnippetExpander`/`SendKeysParser`/CRUD store methods.
...
**ottyRefs.** `spec/terminal-features__{read-only-mode,vi-mode,input}.md`.
...
**ottyRefs.** `spec/user-interface__drag-and-drop.md`, `spec/user-interface__files-and-links.md`.
...
**Goal.** Extend the `aislopdesk` CLI to otty parity and add the first-launch flow.
**ottyRefs.** `spec/reference__cli.md`, `spec/terminal-features__progress-state.md`, `spec/getting-started__first-launch.md`.
**Scope.** Map otty subcommands onto `aislopdesk-ctl`/store ops: `open/view/edit`, `config get/set/reload`, `font/theme/keybind list`, `tab/pane/window` (`send-keys` already exists), `tab badge --kind`, `pane capture`, `watch <cmd>` (OSC 9;4 wrapper), `watch:claude <id>` (exit 0/4/9), `jump/learn/ignore` (frecency), `version`, `completions`, `state:`/`ipc` (done). Install-CLI flow + omit-prefix (O3). First-launch settings (On-Launch, default-terminal local-only, install hooks card) (O1/O2).
**Est** L · **Pri** 4 · **dependsOn** E4, E13
**Acceptance:** ES-E20-1…ES-E20-4.

## E21 — Remote-window extension first-class through clone surfaces
**Goal.** Ensure the user's remote-window feature flows through every clone surface as a first-class peer of terminal panes.
**ottyRefs.** (no otty analog — aislopdesk extension; design alongside `spec/user-interface__window-tab-split.md` and `spec/user-interface__open-quickly.md`.)
