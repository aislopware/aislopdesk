# Otty Clone — Acceptance User Stories (Self-Verification Checklist)
...
- **ES-E1-5** [E1] As a developer, every clone action (`commandPalette`, `cheatSheet`, `find`, `openQuickly`, `composer`, `promptQueue`, `sendToChat`, `reopenClosed`) is registered in the binding registry with a unique, override-resolvable chord and a `routeTree` case. — unit-testable
...
- **ES-E7-1** [E7] As a user, Settings shows General / Shell / Controls / Editor / Appearance / Agents / Keybindings / Advanced sections matching otty's taxonomy. — GUI-verifiable
...
- **ES-E15-2** [E15] As a user, dropping a `.ottytheme` (or importing iTerm2/Kitty/Alacritty/Ghostty) adds it to the theme list and I can activate it. — both
...
- **ES-E16-1** [E16] As a user, `⌘S` saves the current tab/window layout as a recipe (layout-only or include-commands) to a `.ottyrecipe`-style file. — both
...
- **ES-E21-4** [E21] As a developer, every clone surface treats `.remoteGUI` panes as first-class peers of terminal panes (no special-casing that drops them). — unit-testable
