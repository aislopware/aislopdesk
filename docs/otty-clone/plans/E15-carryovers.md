...
- **Vertical-tabs-only — the one that bites E15.** The theme editor exposes per-region chrome
  ...
  bar was deliberately dropped per the user). So when cloning otty's theme-editor chrome regions, do
  **NOT** introduce a "tab bar" region that implies a horizontal/top tab strip, and do not add a
  theme key or swatch whose only purpose is colouring a top tab bar. Theme the **sidebar/vertical
  rail** tab chrome instead. If otty's theme screenshots show a top-tab-bar region, map it to the
