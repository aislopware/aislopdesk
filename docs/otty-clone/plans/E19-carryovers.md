...
- The GENUINE gap for **ES-E19-3 (A32)** is the VIEW: `NavigatorColumn` renders ONLY
  ...
  Do **not** invent new session ops. Match otty's sidebar session affordance in
...
- **View → Pin Window** menu item: `Commands/WorkspaceCommands.swift` builds one `CommandMenu` per
  ...
  carries NO `.keyboardShortcut` — otty's Pin Window has no default chord anyway, so a menu Button is
  enough; if you want a chord, wire it focus-scoped locally (NOT in the global menu).
...
otty's `window-size` = `remember` (default) / `grid` (`window-cols`×`window-rows`, def 80×24) /
...
⌘⇧L override (otty's `auto` is "hidden when only one tab", a default state, not a lock).
