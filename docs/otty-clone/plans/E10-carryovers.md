...
- **Status-bar fields are CLIENT-side derivable — reuse existing engines, no wire change.** cwd via
  **OSC 7**, last command's exit via **OSC-133 D** (the OSC-133 / `TerminalBlockModel` stack already
  ...
  `ConnectionRegistry`. Honour the otty **`hideStatusBar`** config key (don't ship a status bar that
...
- **⌘-hold link underline / 2-letter hint mode are NOT the reverted titlebar hint-chips.** Memory
  ...
  legitimate otty features over the terminal grid — build them faithfully per `files-and-links.png`
...
`spec/terminal-features__hint-mode.md`, `spec/user-interface__outline.md` + their screenshots
(`files-and-links.png`, `links-schemes.png`, `status-bar.png`, `hint-mode.png`, `jump-to.png`) are
the 1:1 visual standard (status-strip layout, link underline, hint-label rendering, Jump-To panel).
Match them.
