...
- **No horizontal tab bar.** Do not add a horizontal/top tab-bar layout option anywhere in
  settings or UI. If otty exposes a "tab bar position" (top vs left) row, render ONLY the
...
5. **`⌘⇧W` is bound to Close Tab, but otty's `⌘⇧W` is Close Window.** Per the spec keybindings
   table (`user-interface__window-tab-split.md` lines 99/103/104): `⌘⇧W` = Close window, `⌘W` =
   close focused pane/tab/window cascade, `⌘⇧T` = reopen tab; otty has NO dedicated Close-Tab
   ...
   Close-Tab chord is still wanted, give it a non-otty chord or leave tab-close to the `⌘W`
   cascade. **Record the decision in `docs/DECISIONS.md`** as the E1 keymap fixes were.
...
7. **otty "New Window" working-directory policy is unwired (dead accessor).**
   ...
   kind. (Do not just drop the key — New Window cwd is a real otty settings feature.)
8. **Pane close is gated by the Tab close-confirmation policy on EVERY pane close, incl. non-last
   (diverges from otty).** In `WorkspaceStore.closeConfirmationNeeded(scope: .pane, …)`, apply the
