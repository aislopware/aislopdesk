...
- E6 polish2 already made the drag carry **tab IDENTITY (uuid), not rendered index** (a mid-drag
  ...
  and `OttyTabRow.swift:162` exposes the Manual sort row.
...
- **Split-L / Split-R** → the existing split machinery: `Chrome/OttyTitlebar.swift:177 split(_ axis:)`,
...
not resolve. otty is local so it injects verbatim — MATCH that (inject verbatim, optionally an advisory
toast), do NOT block the drop. If you want a host-side path-exists check, REUSE E4's metadata channel
(a stat-style verb) — do NOT add a fresh wire message just for this.
