...
- **No horizontal tab bar.** Do not add a horizontal/top tab-bar layout for the Details panel's four
  tabs (Info / Outline / Git / Files). Render the otty tab affordance only (segmented header / left
...
1. **Info tab omits otty's "Working Directory" section and the Copy-Path action E4 committed to ship.**
   ...
   `OttyKeyValueRow(label: "Dir")` with `.truncationMode(.head)` (~:279-280) buried in that Session
   ...
   absent from its required surface. **Acceptance:** add a "Working Directory" `OttySectionHeader` +
   ...
   status/host/ping block as the remote-specific addition, but restore the otty Working-Directory
