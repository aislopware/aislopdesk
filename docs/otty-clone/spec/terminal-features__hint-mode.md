...
No screenshots are provided on the otty docs page for this feature. The page
contains only prose and tables — no annotated screenshots of the label overlay
UI. The following is inferred from the behavioral description:
...
specified in the otty documentation and must be determined by inspecting the
live app or cross-referencing the otty theme/design system.

## Screenshots

(none — the otty docs page for this feature contains no screenshots)
...
### Mappings that require care or cannot map 1:1
...
- **Hint-to-open (file paths)**: On macOS client, opening a file path with
  ...
  - Flag: **cannot map 1:1 for file paths** — must route through the
    control channel to the host. URLs and IP addresses are host-agnostic and
...
- **Hint-to-reveal in Finder**: "Reveal in Finder" is inherently a HOST-side
  ...
  - Flag: **cannot map 1:1 on iOS** — suppress or remap on iOS client.
