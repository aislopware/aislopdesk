...
- **System notifications**: Fire when a task completes or when an agent is waiting for approval/input. Otty can also keep the Mac awake to prevent a long run stalling on sleep.
...
- **Prompt Queue (⌘⇧M)**: Queue commands while an agent is busy; Otty dispatches each queued command at the next idle prompt automatically.
...
- **Hook/plugin install**: On first agent run, Otty offers to install a small hook (Claude Code, Codex) or plugin (OpenCode) into that agent's config. Approve once; nothing else changes. The hook lets the agent report start/finish/waiting state to Otty.
- **Restart prompt**: After hook install, Otty may prompt to restart the agent in that tab. Once restarted, tab badges, notifications, history, and fork activate automatically.
...
- **History and Fork** are agent-generic at the Otty level; the conversation capture depends on the hook being installed.
...
- Left: `~/Workspace/otty:main` — current working directory + git branch, in small gray monospace text.
...
- **Current working directory in status bar** (`~/Workspace/otty:main`): CWD comes from OSC-7 (shell integration) emitted by the host PTY. This is already supported via the OSC-7 seam in aislopdesk's wire protocol, so the status bar display is achievable.
