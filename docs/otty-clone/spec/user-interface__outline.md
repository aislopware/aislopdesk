...
When Shell Integration is active, the shell emits OSC 133 marks around each prompt and command. Otty uses those marks to build a per-pane index of user commands with exit status. The Outline feature is surfaced in two places: (1) the Jump To panel (⌘J / ⌘⇧O), a floating quick-switcher overlay that also lists commands under a "Current" filter tab, and (2) the Outline sidebar panel shown in the Details Panel (right sidebar).

## Behaviors

- **Per-pane command index**: Otty builds a per-pane index of all user-entered commands by consuming OSC 133 marks (shell integration). The index is live — new commands appear as they are run.
...
- **No prompt modification required**: Otty injects shell integration via shell hooks; users do not need to modify their PS1/PROMPT. Integration is done entirely through shell hook injection.
...
- **Requirements**: A supported shell (bash/zsh/fish) with Otty's shell integration hooks installed and Shell Integration activated in Otty settings.
...
- Shows git branch "main", diff stats (+150 -403), repo URL "https://github.com/applemakers/otty".
...
- Bottom: a shell prompt `> Explain this codebase` and output `gpt-5.5 xhigh -- ~/Workspace/otty`, suggesting an active agent session.
...
- **Session header row**: shows "~/Workspace/otty" path and "4m ago" timestamp in small muted text.
...
- **Permission required** dialog block (distinct visual treatment, slightly highlighted background):
  - "◈ Access external directory ~/Workspace/otty/packages/app-macos/Sources/Packages/OpenQuickly"
  - "Patterns" section listing: "~/Users/abner/Workspace/otty/packages/app-macos/Sources/Packages/OpenQuickly/*"
...
- **Host-side CWD in outline rows**: Otty can show `~/Workspace/otty` as the session path because it runs locally. In Aislopdesk, the CWD comes from OSC 7 (`OSC 7 ; file://host/path`) emitted by the remote shell. The remote path is available over the wire but requires the remote shell to emit OSC 7; if it does not, the session path label will be absent or stale.
...
- **Timestamp accuracy**: Otty shows relative timestamps ("4m ago") per command. These timestamps are local. In Aislopdesk, the timestamp of a command's mark arriving at the client may differ from the actual shell execution time on the host by RTT. This is negligible for the UX but worth noting: use client-receive-time as the timestamp, not a host-provided clock.
- **Supported agent session detection**: Otty lists "history prompts" for "supported code agent sessions." In Aislopdesk, Claude Code agent sessions are auto-detected via `ClaudeStatus`/`ClaudePaneDetector`. The outline should check `PaneKind` to decide whether to show the "prompt history" view vs the plain command list view.
