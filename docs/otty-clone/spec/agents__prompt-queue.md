...
The screenshot shows a macOS window with a standard title bar (traffic-light buttons top-left, title "otty" centered). The window is divided into two columns by a thin vertical divider:
...
- **Left column — tabs/sidebar** (narrow, ~110 px wide): labeled "TABS" in small uppercase grey text at the top. Below are two rows:
  ...
  - Row 2: "otty" (selected/active tab, shown in slightly bolder text) with badge "⌘2" right-aligned.
...
Below it, a status row showing: `gpt-5.5 xhigh · ~/Workplace/otty` in small monospace text with subtle color — `gpt-5.5` in blue/teal, `xhigh` in muted text, path in grey.
