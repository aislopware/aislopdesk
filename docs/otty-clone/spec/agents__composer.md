...
The Composer is the primary input affordance for agent (code-agent) sessions in Otty. It is a multi-line text editor that floats at the bottom of the agent pane, providing cursor operations, undo/redo, rich paste, pinning (stay visible across tab switches), a floating Spotlight-style detached panel mode, and prompt-queue routing. It is also available in ordinary terminal panes as a GUI-quality input overlay. The key design principle is that Return alone never sends — accidental sends are impossible by design.
...
- **Agent-generic vs Claude-Code-specific:** All behaviors above are agent-generic (described for any code agent). No behavior on this page is exclusively Claude-Code-specific, though the page is documented in the context of Otty's code-agent integration (OpenCode shown in screenshots).
...
- Number callout overlays ①②③ appear as red filled circles with white numerals (added by the documentation, not native Otty UI).
...
**Overall context:**
A browser window showing the Otty docs site (localhost:5173/agents/composer) is in the background. In the foreground is the Otty app window (partially visible, showing the composer-in-pane view, darker, slightly scaled back). In the top-right foreground is the **floating Composer panel**.

**Floating Composer panel:**
- Title bar: "Otty Composer — OpenCode" in standard macOS window title bar style. Traffic-light buttons (red/yellow/green) on the left of the title bar. The window has the standard macOS floating panel appearance — no full window chrome, minimal footprint.
- **Window size:** Approximately 340×160pt. Compact, Spotlight-like proportions — wider than tall, like a command palette.
- **Content area:** Dark background matching the main Otty theme. Two lines of body text visible in the input area: "> The composer is the input affordance for agent sessions — multi-line, cursor operations, undo / redo, float on top..." (appears as a quoted/prefilled context line in lighter/muted style), followed by "Rephrase this|" with a text cursor (blinking bar), indicating the user's active draft.
...
- **Positioning:** The float panel sits above and to the right of the underlying Otty terminal window, demonstrating that it can be freely positioned over other app content. No shadow from underlying windows bleeds into it — it is a distinct system-level floating window.
...
The Otty docs page shows the Composer heading, a highlighted blue intro paragraph, and the composer-in-pane screenshot embedded in the page — demonstrating the documentation-in-browser + float-panel-in-use workflow.

## Screenshots

- `composer.png` — composer embedded in an agent pane, showing multi-line draft and the three toolbar buttons (Pin ①, Float ②, Queue ③)
- `composer-float.png` — floating Composer panel detached over another app (docs browser), titled "Otty Composer — OpenCode"
...
- **Float panel (macOS):** Otty's float panel uses `NSPanel` with `.floating` window level and does NOT activate the app or steal menu bar — this requires `NSPanel` with `becomesKeyOnlyIfNeeded` and `.nonactivatingPanel` style mask. On aislopdesk macOS client this is feasible but needs care: the panel must remain associated with the originating pane session so that `⌘↩` injects into the correct host's PTY. Panel identity must survive window-level changes.
...
- **"Stays on top without activating Otty":** The non-activating NSPanel behavior is macOS-only and is a specific Cocoa feature. On aislopdesk macOS this is achievable. On iOS there is no equivalent.
...
- **"Title: Otty Composer — OpenCode" in float panel:** The agent name ("OpenCode" in Otty's screenshots) comes from the agent integration layer. Aislopdesk currently integrates Claude Code; the float panel title would read "Aislopdesk Composer — Claude Code" or similar. The agent name must be tracked per-pane in `WorkspaceStore`.
