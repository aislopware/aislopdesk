...
- Config file location: `~/.config/otty/config.toml`
...
- `omit-otty-prefix` is live-toggleable at runtime (no restart required)
- `auto-theme-dark-mode = true` makes the terminal follow OS appearance — `theme` used for light, `theme-dark` for dark
- Custom theme `.toml` files dropped into `~/.config/otty/themes/` are auto-discovered
...
| `term` | `auto` | Value of `$TERM`. `auto` picks the best-supported value for Otty. |
...
Custom themes: drop `.toml` files into `~/.config/otty/themes/` or import from other terminals.
...
| Key | Default | Effect |
|-----|---------|--------|
| `link-open-with` | `browser` | Clicked URL destination: `browser` or `otty`. |
| `file-open-with` | `default-app` | Clicked file: `default-app` or `otty`. |
| `folder-open-with` | `default-app` | Clicked folder: `default-app` or `otty`. |
...
| `omit-otty-prefix` | `false` | Install edit/view/watch functions without the `otty` prefix. Live-toggleable. |
...
| Key | Default | Effect |
|-----|---------|--------|
| `notification-foreground` | `off` | Show banner while Otty is in the foreground. Values: `off`, `always`, `tab-unfocused`. |
...
| `recipe-replay-file` | `manually` | Command replay for external `.ottyrecipe` files. Same values as above. |
...
This reference page contains no screenshots. It is a pure text/table documentation page. The only image present in the HTML is the site logo (`/otty-icon.png`).

The page layout follows the standard Otty docs template:
...
- Footer: Copyright © 2026-present Otty, Previous/Next navigation links
...
- **`font-family`, `font-size`, `font-blending`, `font-ligatures`, `font-thicken`** — these map to libghostty's `TerminalConfigBuilder` font/rendering settings. Aislopdesk already routes theme colors through `resolveTerminalColors`; font config should follow the same path.
...
- **`theme`, `theme-dark`, `auto-theme-dark-mode`** — Aislopdesk already has `ThemeStore` and Monokai Pro as default. Map otty theme names to aislopdesk themes or ignore if using own theme system.
- **`ui-panel-*`, `ui-text-*`, `ui-hover`, `ui-active`, `ui-accent`** — already approximated by Aislopdesk's `OttyDesign` token system. These config keys would allow per-user override of design tokens.
- **`font-size`, `adjust-cell-height`** — map to libghostty cell sizing config. `adjust-cell-height` uses Ghostty-compatible syntax — directly passable.
- **`foreground`, `background`, `palette-0`–`palette-15`** — already handled via `resolveTerminalColors` → `TerminalConfigBuilder` in Aislopdesk.
- **`scrollback-lines`** — map to libghostty scrollback config.
- **`text-bold`, `text-italic`, `text-underline`, `text-blink`** — map to libghostty text rendering options.
- **`mouse-reporting`, `mouse-scroll-multiplier`, `macos-option-as-alt`** — map to libghostty / NSEvent input handling.
...
- **`shell-integration`** — Aislopdesk already has OSC 133 / CWD detection. This key controls whether the managed shell-rc block is installed.
- **`live-resize-sigwinch-delay-ms`** — Aislopdesk already implements `onResizeSettled` / deferred SIGWINCH. Map to that delay constant.
- **`terminal-scroll-smooth`** — Aislopdesk already has smooth scrollback; this is a user toggle.
- **`window-layout`** — Aislopdesk has sidebar-left layout currently. `tabs-top`/`tabs-bottom` are additional layout modes to implement.
- **`sidebar-visible`, `sidebar-width`, `details-panel-width`** — direct sidebar visibility/sizing config; Aislopdesk already has sidebar toggle.
- **`autocomplete-*`** — Aislopdesk uses a custom autocomplete; these config keys define behavior surface. `autocomplete-on-device-learning` maps to local history learning only.
- **`privilege-badge-*`**, **`privilege-notify-*`** — Aislopdesk's agent monitoring (`ClaudeStatus`/`ClaudePaneDetector`) produces these events. These keys control opt-in/opt-out per signal type.
- **`privilege-caffeinate-agent-processing`** — maps to `IOPMAssertion` / `caffeinate` while Claude is processing. Worth implementing.
- **`privilege-resume-agent-session`** — maps to `session-restore-multiplayer` + agent session state on restore.

### Cannot map 1:1 (remote architecture constraints)

- **`working-directory`, `window-working-directory`, `tab-working-directory`, `split-working-directory`** — in Aislopdesk the CWD is always on the **host machine** (remote). The client cannot set CWD to a local path. Only `inherit` semantics make sense client-side; the initial directory must be configured on the host. Flag in Settings as "host-side only".
- **`command`, `shell`** — the shell runs on the **host** (`aislopdesk-hostd`), not locally. This config key belongs in host configuration, not the client config file.
...
- **`ssh-integration`** — Aislopdesk IS the remote transport; SSH integration inside the terminal would be SSH-over-aislopdesk. Lower priority; flag as N/A unless the remote session itself then SSHes onward.
- **`login-greeting`** — host-side only (controls remote shell startup mode).
- **`quick-terminal-*`** — Quick Terminal is a macOS-global hotkey drop-down terminal. Maps well on macOS client. Does not apply to iOS client.
- **`dock-icon-*`** — macOS Dock only; not applicable on iOS client.
- **`auto-secure-input`, `secure-input-indication`** — macOS Secure Keyboard Entry applies locally on the client machine. Since the remote end is `aislopdesk-hostd` on a different Mac, "secure input" semantics are local to the client keyboard. The indication pill can still be shown on the macOS client. Not applicable on iOS (iOS has its own secure field handling).
...
- **`open-with-app`, `default-git-client`, `file-open-with`, `folder-open-with`** — "open in app" semantics require opening local files. On aislopdesk, files are remote. Only `link-open-with = browser` (opening URLs) maps cleanly since URLs are scheme-based and can be forwarded to the client browser.
- **`zoxide-enabled`, `zoxide-local-path`** — `zoxide` runs on the host. frecency sync with a local `zoxide` binary doesn't apply on the client. The aislopdesk client has its own frecency table (Open Quickly uses it per the existing implementation).
- **`session-log-mode = redacted`** — secret masking is a host-side concern since the raw PTY stream passes through `aislopdesk-hostd`. Client-side session log would only capture what arrives over the wire.
- **`ipc-allow-send-keys`, `ipc-allow-sensitive-sessions`** — IPC security in otty is for local IPC between the terminal and shell helpers. In aislopdesk the equivalent is the `aislopdesk-ctl` NDJSON control channel — map these concepts to `AISLOPDESK_AGENT_CONTROL` permission gating.
- **`recipe-replay-*`** — otty recipes (`.ottyrecipe` files) are a local file format. Aislopdesk can adopt a similar concept, but file paths would be local to the client machine, while execution happens on the host.
...
- **`session-restore-multiplayer` (tmux reattach)** — Aislopdesk has its own detach/reattach via `DetachedSessionStore`. tmux reattach in the remote shell is an orthogonal concern; can be supported via `AISLOPDESK_DETACH_ENABLED` + custom session restore logic.

### Implementation priority order (for clone)
...
7. `autocomplete-*` → aislopdesk autocomplete engine
