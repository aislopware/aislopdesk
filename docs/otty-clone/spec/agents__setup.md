...
Source: https://docs.otty.sh/agents/setup
Section: agents / slug: setup
Captured: 2026-06-25
...
Otty does NOT run agents itself — users continue using Claude Code, Codex, or OpenCode exactly as they normally would. The one-time setup described on this page teaches each agent to report its runtime state back to Otty, so Otty can show live tab badges, fire system notifications, resume sessions, and surface history. It is a single click per agent.

The integration is hook-based: Otty writes into the agent's own config file (e.g. `~/.claude/settings.json` for Claude Code) and the agent calls those hooks at lifecycle events. Otty never runs agents or intercepts their I/O; it only receives state signals.
...
- **Per-agent install card**: Each supported agent has its own card section. Clicking **Install** (or **Install Hooks** / **Install Plugin** depending on agent) writes Otty's config into the agent's config file. Only the Otty-specific entries are touched; the rest of the file is left untouched.
...
- **Uninstall**: Clicking **Uninstall** cleanly removes Otty's entries from the agent's config file and leaves the remainder untouched. The card reverts to the Install button.
- **Restart requirement**: The integration takes effect only after the agent restarts with the new config. If an agent was already running in a tab it must be restarted.
  ...
  - For **Codex**: if Otty or Codex was recently updated, approve the trust prompt when it restarts.
...
- A card row (white background, rounded border ~6px radius, subtle #E5E5E5 border) containing:
  - Left: bold label **"Install Hooks"** (~14pt, #1A1A1A), below it a description line in grey (#666, ~12pt): "Add Otty hooks to ~/.claude/settings.json for real-time state updates"
...
- Card: bold **"Install Hooks"**, description "Add Otty hooks to ~/.codex/hooks.json for real-time state updates".
...
- Card: bold **"Install Plugin"**, description "Add Otty plugin to ~/.config/opencode/plugins/ for real-time state updates".
...
6. **Agent Behavior toggles greyed out until install**: This UX gating is straightforward to implement — observe whether `~/.claude/settings.json` contains Otty's hooks and conditionally enable the toggles. The check should be re-run each time the Settings panel opens (not cached forever).
