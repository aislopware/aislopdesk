...
Four things worth setting up the first time you open Otty. All of them live in **Settings** (`⌘,`) — no config files required:

1. Set Launch and New Tab / Window Options
2. Set as Default Terminal
3. Install the Otty CLI (and skip the `otty` prefix)
4. Change Theme
...
- **System default** — registers Otty as the OS handler for `.command` / `.tool` / `.sh` file double-clicks in Finder, `open script.sh` from the command line, and `man://` / `ssh://` URL scheme opens.
  - Action: Settings → General → OS Integration → click **Set as Default Terminal**.
- **Editors & git GUIs** — most editors (VS Code, Cursor, Windsurf, Sublime Text, etc.) and git GUIs hardcode Terminal.app or iTerm.app and ignore the system handler. Otty rewrites their per-app config to launch Otty instead.
...
- **Finder Integration** — adds "Open in Otty" to Finder's right-click Services menu for folders. Enable or rebind in System Settings → Keyboard → Keyboard Shortcuts → Services. Button shown: **Open System Settings**.
- **Full Disk Access** — needed when commands run inside Otty need to read/write protected files (mounting DMGs, reading `~/Library`, scripting Mail/Messages, etc.). Otty itself works without it. Button shown: **Open System Settings**.

### 3. Install the Otty CLI

The `otty` command drives the app from any shell — open files in a pane, jump to recent folders, watch a long-running command, etc. It ships inside the app bundle but is not on `PATH` until installed.

Settings → Shell → Otty CLI:

- **Install CLI** — adds `/usr/local/bin/otty`; requests admin privileges once.
- **Omit `otty` Prefix** — toggle ON causes shells launched by Otty to get plain `edit`, `view`, `watch`, `jump`, and `learn` as shell functions, so you can type `edit foo.txt` instead of `otty edit foo.txt`.
- **Allow Overwrite** — leave OFF unless you already have your own `edit`/`view`/etc. shell functions and want Otty to replace them.

CLI quick usage:

```sh
otty jump Workspace   # jump to folder, like Zoxide or autojump
otty view readme.md   # view file directly in Otty window
otty watch build.sh   # run and watch script, show notification when done

# When "Omit otty Prefix" is enabled:
jump Workspace
view readme.md
watch build.sh
```
...
- For each agent you use, click **Install** in its card. This writes a small hook config:
  ...
  - The hook lets the agent stream its live state back to Otty.
...
|---|---|---|
| **On Launch** (Settings → General) | `New Window` (recommended: `Restore Last Session`) | What Otty does when the app opens: start fresh or restore last session with scrollback and agent state |
...
| **Install CLI** (Settings → Shell → Otty CLI) | Not installed | Adds `/usr/local/bin/otty` to PATH; one-time admin prompt |
| **Omit `otty` Prefix** (Settings → Shell → Otty CLI) | OFF | Exposes `edit`, `view`, `watch`, `jump`, `learn` as bare shell functions in Otty-launched shells |
| **Allow Overwrite** (Settings → Shell → Otty CLI) | OFF | Whether prefix-less functions overwrite existing user-defined functions of the same name |
| **Default Terminal** (Settings → General → OS Integration) | Not set | Registers Otty as the system URL/file handler for terminal launches |
| **Set as Default Terminal for Common Apps** (Settings → General → OS Integration) | Not configured | Rewrites per-app config for VS Code, Cursor, Windsurf, Sublime Text, etc. to open Otty |
| **Finder Integration** (Settings → General → OS Integration) | Inactive | Adds "Open in Otty" to Finder right-click Services menu |
| **Full Disk Access** (Settings → General → OS Integration) | Not granted | Grants Otty permission to read/write protected system files |
...
### first-launch.png — Initial Otty window on first launch
...
2. **Set as Default Terminal for Common Apps**
   ...
   - Description: "macOS has no real default-terminal concept — most editors and git GUIs hardcode Terminal.app. Rewrites each known third-party app's external-terminal setting so it opens Otty instead."
   - Button right: **"Configure…"** (rounded rectangle, light border)

3. **Finder Integration**
   - Label: "Finder Integration" (bold)
   - Description: "Adds 'Open in Otty' to Finder's right-click Services menu for folders. Enable or rebind in System Settings → Keyboard → Keyboard Shortcuts → Services."
   - Button right: **"Open System Settings"** (rounded rectangle, light border)

4. **Full Disk Access**
   - Label: "Full Disk Access" (bold)
   - Description: "Needed when commands run inside Otty have to read or write protected files — mounting DMGs, reading `~/Library`, scripting Mail/Messages, etc. Otty itself works without it."
   - Button right: **"Open System Settings"** (rounded rectangle, light border)
...
- `first-launch.png` — Otty main window on first launch (light theme, single tab, empty shell)
...
- **Default Terminal (OS-level handler)** — Otty registers itself as the system default for `.command`/`.sh`/`man://`/`ssh://`. Aislopdesk could do the same for local shell launches, but the remote session aspect means some of these (e.g., `ssh://` links) may open a remote connection rather than a local one. Flag for product decision: should clicking an `ssh://` link open a remote pane into that host via Aislopdesk's transport, or fall back to local Terminal?
...
- **Finder Integration ("Open in Otty")** — adds a Services menu item. Fully achievable for client-local Finder. The opened folder would need to map to either a local pane or a remote SSH pane; the routing decision is Aislopdesk-specific.
...
- **Install CLI (`/usr/local/bin/otty` equivalent)** — Aislopdesk has `aislopdesk-ctl` and related CLIs. A "Install CLI" button in Settings that symlinks `/usr/local/bin/aislopdesk` is directly applicable.
...
- **"Restore Last Session" with remote agent auto-resume** — Otty resumes local claude/codex processes. In Aislopdesk, the "process" runs on the remote host. Auto-resume means reconnecting the remote PTY and re-attaching to the detached session where the agent was running. This is architecturally different: the agent never paused — it kept running on the host. The UX of "auto-resume" is actually just "reconnect to the still-running detached session." This is possible via `DetachedSessionStore`, but the framing/UX should make clear the agent was NEVER paused.
