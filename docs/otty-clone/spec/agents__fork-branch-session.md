...
- **Agent scope note:** The fork/branch command is initiated from inside the agent's own chat interface (Claude Code's `/branch` slash-command). Otty detects the fork event and routes the new session to the chosen split/tab layout.
...
- **Status bar context:** The bottom status bar shows the current file context (`fork-branch-session.md 25.8K (13%)`), cost (`$0.12`), the palette shortcut hint (`ctrl+p commands`), working directory (`~/Workspace/otty:main`), and agent version (`OpenCode 1.16.2`).
...
- Right side: "ctrl+p commands" (palette hint), "~/Workspace/otty:main" (cwd+branch), "· OpenCode 1.16.2" (agent version)
...
| Fork detected by terminal, new session routed to split/tab | Agent-generic — Otty detects the agent's own command output |
...
5. **Session context sidebar (tokens, cost, LSP):** This data is surfaced by Otty from the agent's own output or API. In aislopdesk, this would require parsing Claude Code's structured output (e.g. JSON progress events, OSC-9;4 progress sequences, or a sidecar protocol). This is **not a 1:1 map** — aislopdesk has no existing token/cost parsing; this would need to be added to `BlockOutputView`/`ClaudeStatus` machinery.

6. **Status bar (cwd, branch, agent version):** The cwd (`~/Workspace/otty:main`) displayed in the status bar comes from OSC-7 (Current Working Directory) emitted by the remote shell. Aislopdesk already supports OSC-7 routing. The git branch suffix (`:main`) and agent version string would need to be parsed from the agent's prompt or output. Flagged as **partial map** — cwd is handled, branch+version require additional parsing.
