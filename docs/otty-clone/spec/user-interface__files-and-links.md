...
- Otty detects the following in live terminal output and scrollback:
...
|--------|-------|---------|----------|-------------|
| Path | nothing (prevents accidental opens) | Open in best handler — file pane for text, default app otherwise | Reveal in Finder | Context menu: Open With…, Copy Path, Reveal, Change Directory Here, Open in Otty |
...
- **Open in Otty** — submenu: open in current pane, new tab, or split.
...
- Otty can open and preview common file types and embeds a text editor.
- Ways to open a file:
  - Drag a file from Finder onto the window's "New Tab" drag zone → new file pane in a new tab.
  - Context menu → Open in Otty → opens view pane.
  - `otty view <file>` — open in preview/read-only mode.
  - `otty edit <file>` — open in edit mode.
...
- Files modified outside Otty trigger a banner with "Reload" / "Keep my changes" options.
...
- Open via: `otty view <dir>` or right-click a folder and select "Open in Otty".
...
- Otty includes a built-in web browser (native WKWebView), allowing a link from the terminal to open right next to work — in the current pane, a new tab, or a split.
- Default handler for `http(s)` links is the system browser; can be changed to Otty so `⌘click` opens links directly in the terminal.
...
These are set in the Otty config file (`~/.config/otty/config`) under the relevant section. See [Configuration Reference](https://docs.otty.sh/reference/configuration).
...
|---------|---------|--------|
| Open Links With | Browser | Where `⌘click` and right-click "Open Link" item open a URL. Options include Browser, Otty. |
| Open Files With | Otty | Where `⌘click` and right-click "Open File" item open a file path. |
| Open Folders With | Finder | Where `⌘click` and right-click "Open Folder" item open a folder path. Options: Finder, Otty. |
...
**State shown:** Read-only/saved state (toolbar shows `✓ Saved`). The file is `index.md` — a Markdown document showing the Otty readme with `# Otty` heading, paragraphs, `## Highlights`, and `## Contents` with bullet lists.
...
**Status bar at bottom:** A thin dark bar spanning the full window width. The left portion (dark background, ~30% width) shows the resolved absolute path `/Users/abner/Workplace/otty/CREDITS.md` in small white monospace text. The right portion shows standard terminal status info in lighter text: `files-and-links.md  20.7K (10%)  - $0.08  ctrl+p commands`. This status bar path display is the "hover shows full path" feature — it appears when the user presses `⌘` and hovers over a detected path in terminal output.
...
**Breadcrumb bar:** Below the icon row, a breadcrumb path `abner > Workplace > otty > docs` with `>` separators, each segment in blue clickable text.
...
The highlighted row (`open-quickly.md`) has a very light blue background fill spanning the full width. No checkboxes or badges on rows. The sidebar (TABS column at the left edge) is visible: `otty #1`, `OpenCode #2`, `docs #3`.
...
**Web content area:** Renders the Otty documentation site. Left nav sidebar is visible showing menu icon. The main content area displays the same Otty home page content (heading "Otty", paragraphs, "Highlights" section with bullet points). The content is rendered as native web — standard browser rendering, not Otty's own markdown renderer. Otty's own chrome (traffic lights, title bar, browser toolbar) is very minimal — the entire content area is the WKWebView.
...
- Session metadata header: a compact row showing the session ID hash (`b2d4f6a0-1c3e-4a5b-9d7f-2e4c0a80d11.json`), branch info (`~/Workspace/otty · ⎇ main · v2.1.159`).
- A timestamp entry: `● You · 19:30:21` with a gray dot badge.
- A file path highlighted in blue: `/Users/abner/Workplace/otty/docs/user/workspace/files-and-links.md`
...
- A list of "Sources verified" with blue hyperlinks: file paths like `link-open-with → browser | otty — packages/config/src/lib.rs:924,3484,5801`, `otty open <url> opens a URL pane — docs/...`, etc.
...
- **Open Files With** — "Where Cmd+click and the right-click 'Open File' item open a file path" — dropdown showing "Otty"
- **Open Folders With** — "Where Cmd+click and the right-click 'Open Folder' item open a folder path" — dropdown showing "Finder" with a dropdown open below it showing two options: "Finder" (top) and "Otty" (bottom), in a white popup menu with light shadow and rounded corners
- **Default Git Client** — "The git GUI used as the primary 'Open in App>' target on the Details › Git toolbar. Other installed clients and your custom Open With apps still appear in the dropdown." — dropdown partially obscured by the Finder/Otty popup
- **Custom Open With Apps** — "Add third-party apps (e.g. Fork, Typora) to the folder/file 'Open in…' submenus." — "Configure…" button
...
**Terminal content:** Standard dark terminal output (dark background) showing commands like `atty-docs-web@0.1.0 predev`, `npm run sync-docs`, sync log lines, vitepress v1.6.4 output, and sync lines. One line is highlighted/underscored in yellow: `Workspace/otty/packages/docs-ms (main x)●` — this is the detected current-working-directory path that has been identified as a clickable link.
...
2. **File pane for remote files:** `otty view`/`otty edit` open files on the LOCAL filesystem in Otty. In aislopdesk, the host terminal session's filesystem is REMOTE. To open a remote file in the client's file pane, aislopdesk needs an SFTP/SCP-style file transfer channel (not currently in the wire protocol). **Cannot map 1:1 without adding a file-transfer sub-protocol.**
...
8. **WKWebView browser pane:** Available on both macOS and iOS (WKWebView is cross-platform Apple). Maps 1:1 for both clients. The `link-open-with = browser | otty` config sets whether URLs open externally or in this pane.
...
- The "Open in Otty" context menu submenu (current pane / new tab / split) maps exactly onto aislopdesk's `PaneChooser` and `WorkspaceStore.reconcile()` infrastructure.
