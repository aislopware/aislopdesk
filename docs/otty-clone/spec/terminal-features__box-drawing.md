...
- Arrow glyphs (← → ↑ ↓, U+2190–U+2193) and triangle glyphs (◀ ▶ ▲ ▼) are treated as box-drawing participants: when such a glyph is adjacent to a box-drawing line, Otty extends the stem of the arrow/triangle to meet the rule so there is no gap.
...
### box-drawing-otty.png — Otty: arrows join the rules
...
**Typography:** Same monospace font style, same green prompt color. Box border lines appear slightly thinner/lighter than Otty (possibly slightly different stroke weight from font-rasterized vs analytical rendering).

**Purpose in documentation:** This screenshot demonstrates the BEFORE state — standard (gap-present) rendering. The gap is clearly visible compared to the Otty screenshot.
...
**Background color:** Noticeably darker gray background compared to the creamy/light backgrounds of Otty and Ghostty screenshots.
...
- `box-drawing-otty.png` — Otty rendering with arrow/triangle joining enabled (arrows join the rules)
...
Aislopdesk uses libghostty (behind a `TerminalSurface` seam / `AislopdeskTerminal` + `TerminalRenderingView`) for terminal rendering. The relevant mapping considerations are:

1. **Analytical box-drawing rendering — partial mapping.** libghostty already performs analytical/pixel-perfect rendering of Unicode box-drawing characters (that is Ghostty's own feature). The comparison screenshot confirms that Ghostty renders box-drawing analytically (sharp corners, clean lines) but WITHOUT the arrow/triangle stem-extension behavior. Aislopdesk inherits Ghostty's box-drawing quality for free through libghostty.

2. **Arrow/triangle stem joining — NOT available via libghostty.** The distinguishing Otty feature — extending arrow/triangle stems to meet adjacent box-drawing rules — is NOT a libghostty/Ghostty feature. This is Otty-specific analytical glyph composition. Implementing it in aislopdesk would require either:
   ...
   **Verdict:** Mark this feature as NON-1:1 for aislopdesk's initial clone. Ship with libghostty's existing box-drawing (Ghostty-equivalent quality), without the stem-extension join. Note in settings that "Join arrows & triangles to box-drawing rules" is not available.

3. **Braille and Powerline glyphs.** libghostty already handles these analytically. No additional work needed — these map 1:1.

4. **Block elements.** libghostty renders block elements analytically. Maps 1:1.

5. **Config key exposure.** The "Join arrows & triangles to box-drawing rules" setting cannot be wired to any libghostty behavior because libghostty does not implement this. If/when aislopdesk adds its own glyph-join layer, this setting can be added to `PreferencesStore` via the `Defaults` product (as established in the codebase). Default should remain `true` to match Otty's default, but the implementation will be a no-op stub until the join logic is built.
