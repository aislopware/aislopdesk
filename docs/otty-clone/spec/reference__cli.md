...
- **`otty config get/set/unset/...`** → Maps to aislopdesk's `EnvConfig` + `PreferencesStore` layer. `--transient` flag (running app only, no persist) is equivalent to writing to `EnvConfig` without touching `PreferencesStore`.
