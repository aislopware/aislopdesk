...
Otty stores all settings in `~/.config/otty/config.toml` — a lenient TOML file (flat `key = value` pairs, one per line, `#` comments). The Settings panel writes this same file, so panel edits and manual edits are always in sync. Editing the file directly is useful for version-control, cross-machine copying, commenting choices, or setting keys that only exist in the file and not in the GUI. The file is optional; Otty runs on compiled defaults until it is created. Saving the file triggers an immediate hot-reload across every open window with no restart required.

---

## Behaviors

- **File location:** `~/.config/otty/config.toml`. The file need not exist; Otty runs on defaults until created.
...
- **Hot-reload:** Otty watches the file with a file-system observer. Saving the file applies changes immediately to every open window — no restart, no SIGHUP.
- **Force reload:** After editing an `include`d file (which Otty may not watch directly), use Settings → Advanced → Config File → Reload Config, or run the **Reload Config** command from the Command Palette.
...
The full configuration reference lives at `/reference/configuration`. Below is every key documented across the Otty reference, with its default and effect.
...
| `omit-otty-prefix` | `false` | Install CLI functions without `otty_` prefix |
...
| `recipe-replay-file` | `manually` | Command replay behavior for `.ottyrecipe` files |
...
Aislopdesk is a remote coding tool: a Swift macOS host runs `aislopdesk-hostd` (PTY, screen capture, video encode/FEC), and macOS/iOS clients render the remote terminal via libghostty behind a `TerminalSurface` seam. The config file being cloned here is the CLIENT-SIDE UX layer.

### 1:1 mappings (straightforward)

| Otty key / behavior | Aislopdesk mapping |
|---|---|
...
| `theme` / `theme-dark` / `auto-theme-dark-mode` | `ThemeStore` already drives `MonokaiSeed` selection; map Otty theme names to aislopdesk themes |
...
| `window-layout`, `sidebar-visible`, `sidebar-width` | `WorkspaceStore` / `OttyDesign` sidebar tokens |
| `keybind` | `WorkspaceBindingRegistry` — client-side key dispatch, already partially wired |
| `autocomplete-*` | Client-side inline suggestion UI |
| `clipboard-*` | Client-side clipboard handling via OSC 52 |
| `ui-*` colors | `OttyDesign` token overrides / `ThemeStore` |
| Config hot-reload | `PreferencesStore` + `Defaults` product already used; add a file-watcher on `~/.config/aislopdesk/config.toml` |
| `include = <path>` | Implement in config parser (not yet present) |
| `$VAR` and `~` expansion | Implement in string-value parser |

### Partial / adapted mappings

| Otty key / behavior | Issue |
|---|---|
| `working-directory` | CWD is the HOST's filesystem, not the client's. `working-directory = ~/projects` must be resolved on the host and communicated at session open via the wire protocol (already part of session init). The value lives in client config but applies on host. |
| `tab-working-directory` / `split-working-directory` | Same HOST-side resolution issue. On new tab/split, the CWD is inherited from the host-side PTY, not from the client's local filesystem. |
| `window-working-directory` | `home` default means the HOST's home directory. |
| `command` | The command runs on the HOST (in `aislopdesk-hostd`). Client config can store a preferred command that is sent over the wire at session-open, but the client cannot enforce it after launch. |
| `env` | Environment variables apply on the HOST side in the PTY. Client stores them and sends at session open. |
| `ssh-integration` | Already remote by definition — aislopdesk IS the SSH-like transport. Map to "integration forwarding" over the aislopdesk mux channel, not a secondary SSH hop. |
| `session-restore-*` | Session restore in aislopdesk uses `DetachedSessionStore` (`AISLOPDESK_DETACH_ENABLED`). Align restore behavior with the host-side detach mechanism rather than re-implementing otty's client-side session log. |
| `shell-integration` | Shell integration RC block must be injected on the HOST shell (not client). Host already manages this; client config controls whether to request it at session open. |

### Cannot map 1:1 — flag for future work

| Otty feature | Why it cannot map 1:1 |
|---|---|
| `quick-terminal-*` (Quick Terminal) | Otty's Quick Terminal is a system-wide dropdown (like Visor). Aislopdesk's client is a full windowed app; there is no OS-level overlay mechanism to implement an equivalent without a separate helper app or Accessibility API. Not feasible in Phase 1. |
| `dock-icon-*` | Dock icon badges and animations are macOS-only and client-local. They can be implemented on the macOS client (`AislopdeskClientUI`) but not on iOS. Mark as macOS-only. |
| `auto-secure-input` / `secure-input-indication` | Secure input (IOHIDSetSecureInput) applies to the CLIENT's event stream. However, since aislopdesk injects input events via the host, "secure input" on the client does not protect the host shell from interception. The semantics differ fundamentally — flag as N/A for remote sessions. |
| `freeze-inactive-tab` | In otty this releases a local GPU Metal surface. In aislopdesk the video decode surface is always "remote" — inactive panes can suspend the video stream (already possible via the FEC/packetizer path) but the libghostty surface management is different. Needs host-side stream pause integration. |
...
| `progress-bar-commands` | Progress detection requires parsing shell output locally in otty. In aislopdesk the shell runs on the host; OSC 133 (already tracked by aislopdesk's `Blocks/OSC-133` integration) is the correct seam for progress signaling. |
| `window-opacity` (window-level blur/transparency) | macOS vibrancy/window translucency is client-local and feasible on macOS. On iOS, background blur behind the terminal view requires different API. Mark as platform-specific. |
| `notification-foreground`, `privilege-*` (notifications) | Notifications are delivered to the CLIENT device. For iOS remote clients, notifications require APNS push tokens — not just local `UNUserNotificationCenter`. Flag as iOS-specific complexity. |
| `ipc-allow-send-keys` / `ipc-allow-sensitive-sessions` | IPC in otty refers to a local Unix-socket IPC between processes on the same Mac. In aislopdesk, the equivalent is the host-side agent-control socket (`AISLOPDESK_AGENT_CONTROL=1`, AF_UNIX NDJSON). The client config key guards client exposure; map to `AgentControlListener` access policy on the host. |
| Config file format — `include` | Not yet implemented in aislopdesk's config stack; add to parser backlog. |

### Implementation order recommendation

1. **Config file path:** `~/.config/aislopdesk/config.toml` (matching otty convention).
...
5. **Settings panel bridge:** Advanced tab showing path + "Open Config File" + "Reload Config" — mirrors otty exactly.
