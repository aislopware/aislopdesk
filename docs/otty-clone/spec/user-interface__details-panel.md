...
- Path value: "~/Workplace/otty" in regular weight dark text (~13pt).
...
**Status bar (bottom of terminal pane, not part of sidebar):** Shows "~/Workplace/otty:main" on the left and "1.15.13" on the right in small gray text on a slightly darker bar.
...
**Outline tab content:**
- Top entry: "~/Workplacepotty! / 4m ago" — path and relative timestamp in small gray text. Below it: "opencode" in regular dark text (the pane/process name).
...
- Remote URL: "https://github.com/azpomakes/otty" in small gray text below the branch name.
