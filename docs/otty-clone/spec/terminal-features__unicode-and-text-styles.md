...
Otty renders text via GPU (Metal on macOS, DirectX 12 on Windows) with glyph caching. It supports
...
1. Command line: `~ ▷  otty features emoji` — prompt + command in the default foreground.
...
Content rows:
1. `~ ▷  otty features ligature` — command line.
...
### Cannot map 1:1 / requires attention

- **Settings → Appearance → Text (ligature level UI)**: Otty exposes a per-level ligature
  selector in Settings. Aislopdesk's preferences UI needs to expose an equivalent setting that
  round-trips to a libghostty config key. The exact Ghostty config key is `font-feature` or
  `grapheme-width-method`; verify against libghostty's public config API before wiring.
...
- **Settings → Advanced → Widen East-Asian-Ambiguous Blocks (per-block selector)**: Ghostty has
  ...
  The per-block granularity otty exposes may not be achievable 1:1 with libghostty; a coarser
  "widen all ambiguous" toggle is the achievable approximation. Flag this as a fidelity gap.
