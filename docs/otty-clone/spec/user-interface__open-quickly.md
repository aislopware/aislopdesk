...
### Filter: Folders (`⌘Z`)
- Frequently-used folders ranked by Otty's built-in frecency database.
...
- Second result: "abner@MacBook-Pro: ~/Workplace/otty" with "16 tabs" and "Window" badge.
- Then a "TABS" section header followed by individual tab rows, each with a tab icon,
  title (e.g. "OpenCode", "Yazi: otty", "index.md"), trailing CWD path, and "Tab" badge.
...
- Left sidebar: vertical list of tab titles ("OpenCode", "Yazi: otty", "index.md",
...
- Titlebar: "abner@MacBook-Pro: ~/Workplace/otty" centered. Traffic-light buttons top-left.
...
- **Current filter — URLs/files/folders:** `Files and Links` detection from terminal
  output — already in scope (see otty Files and Links spec). 1:1 feasible.
...
- **Remote SSH badge on the result row:** Otty's docs do not mention it, but in
