...
Otty can read configs from other terminals (Ghostty, Kitty, Alacritty) and write the current Otty config back out in any of those formats. Useful when trying Otty alongside an existing setup, or when sharing a configuration with someone who has not switched. Access is via Settings → Advanced → Config File (two action rows) or via the CLI (`otty import` / `otty export`).

## Behaviors

- A classification engine categorises every line in the source config into one of four buckets:
  - **Supported** — Otty has the same key; value written as-is or value-translated automatically.
  - **Conflict** — Same key exists in Otty but the current value differs; user decides per-row whether to overwrite or keep.
  - **Similar** — Otty has a close analog but it is not auto-imported; user decides manually.
  - **Source-only** — Source terminal feature with no Otty equivalent; docs links are provided.
...
- On export, the output file is a complete drop-in for the target terminal — Otty fills every key the target supports; unsupported Otty-only keys go into a "dropped" list (printed to stderr in text mode, returned as JSON in `--json` mode).
- Kitty format uses space-separated `key value` with underscores; the adapter converts to Otty's hyphenated format on import and back on export.
- Alacritty format is TOML with nested sections flattened to dotted paths; color sections are transformed to the Otty palette format.
- Value translations are performed automatically where semantics differ (e.g. `mouse_hide_wait` seconds → boolean; `cursor_shape beam` → `cursor-style bar`; Alacritty startup modes → Otty window sizing keywords).
- Otty-only keys (font fallbacks, font rendering, ligatures, sidebar/panel controls, SSH integration, autocomplete, privilege/notification settings, quick terminal, recipes, editor settings, etc.) are silently dropped on export to any external terminal.
...
The import/export feature does not expose runtime config keys of its own. The table below captures the keys relevant to interoperability — specifically the Otty keys that serve as the mapping targets.
...
- Below the header, a white card/list group with individual rows separated by hairline dividers:
  - **Path** row: label "Path" left-aligned; value `~/.config/otty/config.toml` right-aligned in gray text.
...
- **CLI commands:** `otty import` / `otty export` map to aislopdesk CLI subcommands with the same flags (`--overwrite`, `--keep`, `--json`, `-o`). The `--json` mode maps to aislopdesk's existing structured output conventions.
...
- **Config file location:** Aislopdesk's own config file is at `~/.config/otty/config.toml` (per the screenshot). This lives on the CLIENT machine. The Settings → Advanced → Config File section with Path, Open Config File, Reload Config rows is part of this same screen.
- **No SSH badge / remote indicator:** Otty runs locally; aislopdesk is remote. The Import/Export UI itself needs no remote-specific badge, but error states (e.g. "command path not found on host") should distinguish client-side vs host-side resolution failures.
