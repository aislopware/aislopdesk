...
*Otty documents no hard-coded default key chord for toggling read-only. The feature is reachable via menu bar (Shell → Read Only) and command palette.*
...
| Otty behavior | Aislopdesk equivalent | Notes |
|---------------|----------------------|-------|
...
2. **Host-side enforcement:** Otty can additionally enforce read-only at the PTY/shell level if it chooses. In aislopdesk the lock is client-side only — a sufficiently motivated remote agent on the host can still write to its own PTY. This is acceptable for the stated use case (preventing stray keystrokes from the viewer), but is not a security boundary.
3. **`tail -f` title display:** Otty auto-names the titlebar from the running process (`tail -f`). Aislopdesk must relay pane titles over the wire (OSC 0/2 or shell integration) for the client to display them — existing mechanism, just needs to be wired to the pane header.
