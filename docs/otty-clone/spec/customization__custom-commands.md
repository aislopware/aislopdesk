...
> The `/customization/custom-commands` page in otty docs is a stub that delegates
> entirely to `/workflows/recipes`. All substantive content is reproduced here
> from that page. The canonical otty name for this feature set is **Recipes**.
...
`.ottyrecipe` TOML file that can be re-opened later or shared with a teammate.

Recipes are stored in `~/.config/otty/recipes/` and surface in:
- **Settings → Recipes**
- **File → Recipe** menu
- The **command palette**
...
| `{{recipe_location}}` | Folder containing the `.ottyrecipe` file |
...
- A third content level, **Include Scrollback**, stores terminal output but is
  only available for internally-saved recipes (not exported `.ottyrecipe` files).
...
| Method | Result |
|--------|--------|
| Double-click `.ottyrecipe` in Finder | Opens in a new window |
| `otty open foo.ottyrecipe` (CLI) | Same result |
| **File → Open Recipe** menu | Pick from the internal recipe database |

### The `.ottyrecipe` File Format
...
- Opening an unfamiliar `.ottyrecipe` displays all commands before execution with
  three choices:
...
- Trusted-file records live in `~/Library/Application Support/Otty/trusted_recipes.json`.
- Self-saved recipes bypass the trust dialog entirely.

### Shell Handoff Recognition

- Otty recognizes interactive programs (`ssh`, `tmux attach`, `docker exec -it`,
...
| Key | Default | Effect |
|-----|---------|--------|
| `scope` (in `.ottyrecipe`) | — | `"tab"` \| `"window"` \| `"commands"` — determines what is saved/replayed |
| `split` (per pane in `.ottyrecipe`) | — | `"right"` (or other direction) — split direction relative to previous pane |
| `size` (per pane in `.ottyrecipe`) | — | `0.0`–`1.0` fraction of parent container |
| `version` (in `.ottyrecipe`) | `1` | Recipe format version |
| Command Replay — Saved Recipes | `Auto` | Replay mode for internally-saved recipes |
| Command Replay — Recipe Files | `Ask Once` | Replay mode for externally-opened `.ottyrecipe` files |

> Runtime configuration paths:
> - Recipes directory: `~/.config/otty/recipes/`
> - Trusted-recipe hashes: `~/Library/Application Support/Otty/trusted_recipes.json`
...
- **Recipe file format (`.ottyrecipe` TOML):** Fully client-side concern. The
...
- **File system access for `.ottyrecipe` opening:** On iOS, Finder-style
  double-click is unavailable. The iOS client would use a `UIDocumentPickerViewController`
  or the share sheet to receive `.ottyrecipe` files and register a UTI for the
  extension.

- **`otty open foo.ottyrecipe` CLI:** Maps to the existing `aislopdesk-client`
