...
This page documents the syntax of `~/.config/otty/config.toml` — the TOML-based flat config format used by Otty. It is a pure syntax/format reference, not a key inventory; the key inventory lives in the Configuration Reference page. The format is deliberately simple: a flat list of `key = value` pairs with lenient parsing, hot reload, environment expansion, and include directives.

## Behaviors

- **File location**: `~/.config/otty/config.toml`
- **Format**: TOML — a flat list of `key = value` pairs, one per line, with `#` for comments
- **Lenient reader**: Otty's parser is lenient — quotes around simple string values are optional
- **Unknown keys are silently ignored**: typos won't error; a config written for a newer Otty stays loadable on an older build
...
| Key | Default | Effect |
|-----|---------|--------|
| `link-open-with` | `browser` | Where to open a clicked URL. Values: `browser`, `otty`. |
| `file-open-with` | `default-app` | Where to open a clicked file path. Values: `default-app`, `otty`. |
| `folder-open-with` | `default-app` | Where to open a clicked folder path. Values: `default-app`, `otty`. |
...
| `ssh-integration` | `true` | Forward Otty's shell integration over SSH to remote hosts. |
| `omit-otty-prefix` | `false` | Install edit/view/watch shell functions so the `otty` prefix can be dropped. Live-toggleable. |
| `cli-allow-overwrite` | `false` | Let omit-prefix/custom-alias wrappers replace names the user already defined. |
| `cli-alias` | (none) | User CLI alias: installs a shell function `<name>` running `otty <command>`. Repeatable. Settings-managed. |
...
| `quit-after-last-window-closed` | `false` | Quit Otty when the last window closes. |
...
| Key | Default | Effect |
|-----|---------|--------|
| `notification-foreground` | `off` | Banner behavior while Otty is foreground. Values: `off`, `always`, `tab-unfocused`. |
...
| `recipe-replay-file` | `manually` | Command replay for external `.ottyrecipe` files. Same values. |
...
| `frecency-auto-record` | `true` | Record every CWD change into the frecency table (powers the Folders tab and `otty jump`). |
...
The config-file-format page is a pure text documentation page in the Otty docs site (VitePress). It contains no content screenshots — only the site logo appears as the sole image.

**Otty logo (otty-icon.png)**
...
The Otty docs use VitePress v1.6.4. Layout: fixed sidebar navigation on the left (~260px), main content area in the center with comfortable line-width (~740px max), and an on-page TOC on the right. Background is white in light mode. The content area uses a clean sans-serif body font (Inter) for prose and a monospace font for code snippets. Code blocks have a light grey background with syntax highlighting. Tables use subtle row borders. Navigation items are flat text links with hover states. The page footer reads "Copyright © 2026-present Otty".

## Screenshots

- `otty-icon.png` — Otty app icon (navigation logo only; no content screenshots on this page)
...
- **`ui-panel-*` / `ui-text-*` / `ui-hover` / `ui-active` / `ui-accent`**: Maps to Aislopdesk's `OttyDesign` token system. These are the exact tokens already in the design system. Auto-derivation from terminal fg/bg is what Aislopdesk's `ThemeStore` already computes via `resolveTerminalColors`.
...
### Cannot map 1:1 / requires special handling
...
- **`quick-terminal-persist-session` / `quick-terminal-cwd`**: Otty's Quick Terminal is a system-wide hotkey drop-down terminal (similar to Quake-style terminals). Aislopdesk does not have this concept currently; it would need a new system-wide hotkey handler and separate window type.
...
- **`link-open-with = otty`**: Opening a URL in Otty's built-in browser pane. Aislopdesk has a `web-browser` pane type (`web-broswer.png` screenshot already captured). This is possible but requires the `WebPane` or equivalent to be wired up.

- **`file-open-with = otty` / `folder-open-with = otty`**: Opening files/folders in Otty's editor/file pane. Aislopdesk has `editor-pane`, `file-panel`, `folder-pane` concepts (screenshots show these pane types). Achievable but requires pane-type routing logic.
...
- **`autocomplete-*`**: Aislopdesk does not currently implement shell autocomplete. This is a significant feature gap relative to Otty. The `autocomplete-on-device-learning` privacy gate (everything stays local) is a design constraint to respect when implementing.

- **`recipes`**: Otty's recipe system (`.ottyrecipe` files, command replay). Aislopdesk has no equivalent; this would be a new feature area.
