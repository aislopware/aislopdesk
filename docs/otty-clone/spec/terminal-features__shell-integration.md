...
Otty injects small shell hooks that emit OSC 133 (FTCS) prompt marks. These power command outlines, working-directory tracking, and exit-status indicators. The feature is enabled by default and is recommended to keep on. Supported shells are **zsh**, **bash**, and **fish**, auto-detected from `$SHELL`. Other shells work as plain terminals but receive none of the integration features.
...
- **Auto-record visited folders** — database for Frequent Folders (`otty jump` and Open Quickly's Folders tab)
...
- `edit` / `view` / `jump` / `learn` shell functions (via _Omit `otty` prefix_ option) and custom aliases
...
- Otty still saves the setting but pops a warning that it won't do anything until shell integration is re-enabled.
- Every affected Settings toggle in the UI is individually flagged when integration is off.

### How integration loads per shell

**zsh:** Otty points `ZDOTDIR` at its bundled `zsh/` directory for the launched session. That `.zshenv` immediately restores the user's real `ZDOTDIR`, runs their own `.zshenv`, then loads Otty's payload on the first prompt (so its marks win over plugin managers). `~/.zshrc` and other dotfiles are **not** edited.

**fish:** Otty prepends its bundled directory to `XDG_DATA_DIRS`, so fish auto-loads the `vendor_conf.d` entry, which then removes that dir again so child processes do not inherit it. `config.fish` is **not** edited.

**bash:** bash has no clean per-spawn auto-load, so Otty adds a small, clearly-marked block to `~/.bashrc` (and a `~/.bash_profile` shim) that sources the bundled payload only when launched by Otty. The block is inert in other terminals and is removed when the toggle is turned off.

**tmux exception:** A tmux server captures its environment once at start, so the `ZDOTDIR` / `XDG_DATA_DIRS` injection cannot reach panes spawned later. When tmux is installed, Otty additionally adds the same guarded managed block to the user's zsh/fish rc so tmux panes still get the integration. Turning the toggle off removes every block.

### Integration scripts are readable and code-signed

Scripts ship inside the app bundle at:
```
Otty.app/Contents/Resources/shell-integration/
├── otty-integration.zsh
├── otty-integration.bash
├── otty-integration.fish
├── zsh/.zshenv
└── fish/vendor_conf.d/otty-shell-integration.fish
```
Nothing is synthesized at runtime or written to a hidden temp file.

### Opting out — everywhere

Open **Settings → Shell → Shell Integration** and turn off **Provide Shell Integration**. A confirmation dialog (_Turn off shell integration?_) appears. On confirm, Otty removes the bash block and any tmux managed blocks from shell startup files and stops the per-session `ZDOTDIR` / `XDG_DATA_DIRS` injection for zsh and fish. Toggling back on reinstalls everything.

### Opting out — per-shell

Leave integration on globally but skip it for one shell by exporting before Otty's payload loads:
```bash
...
| `OTTY_DISABLE_INTEGRATION` (env var) | unset | Set to `1` in a shell rc (before Otty's payload) to skip integration for that shell instance only, without changing the global setting. |
...
- When a dependent setting (e.g. _Notify on Command Finish_) is turned on while the master toggle is off, Otty shows an inline warning within Settings indicating the setting will have no effect until integration is re-enabled.
...
### Direct mappings (feasible 1:1)

- **OSC 133 A/B/C/D parsing** — libghostty already parses OSC 133 (FTCS) sequences (it's a ghostty feature); aislopdesk can consume the parsed mark events from the ghostty surface via the `TerminalSurface` seam to drive identical features.
- **OSC 7 CWD tracking** — ghostty parses OSC 7; the cwd can be read from the terminal surface and surfaced as pane metadata. On the macOS client this is used to inherit cwd for new tabs/splits — maps directly since the client owns the PTY via the host daemon.
- **Exit-status gutter dots** — rendered in the client UI overlay layer beside command output blocks; feasible using the OSC 133 D exit code passed from the host via the terminal mux wire.
...
- **`OTTY_DISABLE_INTEGRATION` env var pattern** — aislopdesk can expose an equivalent `AISLOPDESK_DISABLE_INTEGRATION=1` env var read in the integration scripts.
- **Opt-out per-window** — `aislopdesk open --no-integration` maps to the existing `AISLOPDESK_*` env-flag pattern at session launch.

### Partial mappings (feasible with caveats)

- **Shell integration script injection (zsh ZDOTDIR / fish XDG_DATA_DIRS)** — the host daemon (`aislopdesk-hostd`) spawns the PTY and controls the launch environment, so it can inject `ZDOTDIR` / `XDG_DATA_DIRS` exactly as otty does. The integration scripts themselves live on the **host** machine (not the client), so they must be bundled with the host daemon, not the client app. This is a clean architecture fit.
...
- **Code-signed, readable bundle scripts** — aislopdesk's host daemon is a CLI binary, not a macOS `.app` bundle. The integration scripts cannot live at `Hostd.app/Contents/Resources/shell-integration/`. Instead, they should be embedded in the host binary as resources or installed to a well-known path (e.g. `~/.aislopdesk/shell-integration/`) at first launch. The user-auditability story is weakened unless the installed path is documented prominently.
...
- **Settings confirmation dialog ("Turn off shell integration?")** — aislopdesk's settings UI is in the macOS client app but the actual effect (removing bashrc blocks) must happen on the host. The client must send a control message to the host daemon to perform the uninstall. This adds an async round-trip that otty (running locally) does synchronously. Handle with a loading/spinner state in the settings toggle.
- **On-device autocomplete learning** — otty's autocomplete is local. In aislopdesk, the host knows the exit code (OSC 133 D) and can maintain a host-side command history database; the client can query it. Alternatively the client can maintain a per-host learned history received over the wire. Flag as a separate design decision.
- **Frequent Folders database (`otty jump`)** — this is a host-side database of visited CWDs (from OSC 7). Map to a host-side store queried via `aislopdesk-ctl jump` or similar. The Open Quickly "Folders" tab on the client must fetch from the host over IPC.
