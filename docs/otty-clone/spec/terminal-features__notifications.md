...
- Otty does **not** implement a visual/flash bell — audio only.
- When a notification fires and Otty is not the active application, the Dock icon bounces via `NSApplication.requestUserAttention`. Controlled by **Bounce Dock Icon** (Settings → Shell, on by default).
- Otty drives the dock bounce from notification OSCs (9 / 777 / 99), **not** from the bell — unlike Ghostty.
- By default macOS suppresses notification banners while Otty is the foreground app. **Notify While Foreground** overrides this with three states: `off` (default, system suppresses), `always` (always show banners even when frontmost), `tab-unfocused` (show banner only when notification comes from a tab that is not the active one).
...
- A **System Permission** status row (green dot = allowed, amber/red = blocked) is shown at the top of Settings → Shell → Notification section, with an **Open System Settings** button that deep-links to System Settings → Notifications → Otty.
...
- **Notify on Watch Finish** (on by default): fires when an `otty watch`-wrapped command finishes.
...
| Notify on Watch Finish | on | Notify when an `otty watch`-wrapped command finishes |
| Notify While Foreground | off / `tab-unfocused` | Banner policy while Otty is frontmost: `off` (suppress), `always`, `tab-unfocused` |
| Bounce Dock Icon | on | Bounce Dock icon when notification arrives and Otty is not focused |
...
- **App icon** (left-aligned, vertically centred): the Otty app icon — a dark near-black circle (~50×50 pt) containing a `>_` prompt glyph in white/light grey. Positioned with ~16 pt leading inset from the card edge.
...
The notification is entirely OS-rendered; the app supplies only the title string, body string, and app icon. No custom UI from Otty.
...
4. **Notify on Watch Finish** — toggle ON (green). Subtitle: "Notify when an `otty watch`-wrapped command finishes".
5. **Notify While Foreground** — no toggle; instead a dropdown/segmented control on the right showing "Only when source tab is unfocused ∨" (with a downward chevron). Subtitle: "Banner behavior while otty is the foreground app". The dropdown has a light rounded-rectangle border (~1 pt stroke, ~#CCCCCC), white fill, system-default font ~13 pt.
6. **Bounce Dock Icon** — toggle ON (green). Subtitle: "Bounce the Dock icon when a notification arrives and Otty isn't focused".
...
The secure-input indicator is entirely within Otty's window frame — it is NOT a system-level overlay. The active (all-green) traffic-light buttons confirm the window is focused.
...
- **Auto Secure Input** (canonical-no-echo detection): Requires detecting when the remote PTY enters canonical no-echo mode. The host PTY layer can observe termios `c_lflag & ECHO == 0 && c_lflag & ICANON != 0` and send a control message to the client; the client then calls `enableSecureEventInput()`. **Partial mapping**: the host needs to signal this state over the control channel (a new control message type), and the client must call the macOS API. Not blocked, but requires a new wire event.
- **Clipboard — Shell Controlled / Clipboard Read / Write** (`OSC 52`): The remote host may generate `OSC 52` sequences. Because the clipboard lives on the client side, the client intercepts `OSC 52` from the terminal stream and applies the read/write policy locally. `ask` mode requires a client-side permission dialog. **Full mapping possible.**
...
### What cannot map 1:1 or needs caveats

- **Code Agent notifications via IPC** (Claude Code / Codex / OpenCode): In otty, the agent runs locally in the same process/machine and reports state over a local IPC socket. In aislopdesk, the agent runs on the **remote host**. The `ClaudeStatus` / `ClaudePaneDetector` / `AgentControlListener` infrastructure already exists in aislopdesk (per memory note 2026-06-21). The IPC path must be tunnelled over the existing control channel (TCP control connection) from host to client. This is feasible but is not a trivial 1:1 mapping — it requires the host agent-detector to forward state events over the wire, and the client to fire local macOS notifications based on those events. **Flag: agent IPC state must transit the control channel.**
...
- **IPC Allow Send Keys / IPC Allow Sensitive Sessions**: In otty, IPC clients are local processes. In aislopdesk, "IPC" from the client side travels over the authenticated control channel. The security model is different (WireGuard mesh = trusted boundary per CLAUDE.md design). These toggles may be mapped as host-side guards on the agent-control socket (`AISLOPDESK_AGENT_CONTROL=1` path), but the semantics differ: there is no local-process IPC threat model on the host in the same way. **Flag: semantics differ; map as host-side agent-control guards, not local IPC guards.**
...
- **`otty watch` / Notify on Watch Finish**: `otty watch` is an otty-specific CLI wrapper. Aislopdesk has no equivalent. This toggle can be implemented if aislopdesk ships an `aislopdesk watch` host-side wrapper command; otherwise skip or stub. **Flag: requires aislopdesk-side watch command.**
- **Configuration Reference key names** (`privilege-*`, `notification-*`, `title-report`, `clipboard-read`, `clipboard-write`, `auto-secure-input`): These are otty's config file key names. Aislopdesk uses `AISLOPDESK_*` env vars and `PreferencesStore` / `SettingsKey` (`Defaults` product). Map each to a `SettingsKey` in `PreferencesStore`. No config-file parser needed (per aislopdesk architecture).
