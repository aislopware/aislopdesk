...
Otty surfaces the live state of long-running commands and tasks visually — a spinner or badge on the tab, an animated Dock icon, and (on supported platforms) a taskbar-style progress bar. The state is driven by the **ConEmu `OSC 9;4` progress protocol**, fed by three mechanisms:
...
3. **`otty watch`** wraps any command to give it a spinner-to-finish badge.
...
- A program emits `OSC 9;4;<state>[;<pct>]` to report progress. The four canonical states are: `0` = clear, `1;<pct>` = in-progress (0–100), `2[;<pct>]` = error, `3` = indeterminate/spinner. State `4` (paused/warning) is recognized by the spec but ignored by Otty. State `5` is an Otty extension: `OSC 9;4;5;<exit>[;watch]` = finished with exit code; the `watch` suffix routes to a separate notification toggle.
- Example sequences:
...
- Scripts can set a badge directly via the CLI: `otty tab badge --kind running` (kinds: running, completed, finished, unread, error, awaiting-input) or `otty tab badge --clear`. The caffeinate/sudo badges are intentionally not settable via CLI.
...
- `otty watch:<agent>` blocks a script until an agent session is idle: `otty watch:claude <id>`, `otty watch:codex <id> --timeout-secs 600`, `otty watch:opencode <id> --interval-ms 2000 -v`. Exit codes: `0` = idle/closed, `4` = session ID never seen, `9` = timeout.
...
**Window chrome:** Top-left has the standard macOS traffic-light buttons (close/minimize/zoom) rendered as small gray circles (no color fill visible, suggesting an inactive or non-hover state). The window title bar at the very top center reads "abner@MacBook-AB: ~/Workplace/otty" in a small gray system font.
...
- The shell prompt line is visible at the top of the content area: `~/Workplace/otty (main ✗)` followed by two small colored icons — a red/orange asterisk-like asterisk glyph and a green right-triangle play icon. This is a zsh prompt with git status indicators and likely an otty-specific run/watch indicator.
  - `~/Workplace/otty` — rendered in cyan/teal monospace.
  ...
  - The triangle play button: green (~#2E7D32), possibly an otty watch or agent indicator.
...
**Key visual takeaways for clone implementation:**
...
- **`otty watch` equivalent:** Could be implemented as a host-side CLI wrapper (`aislopdesk watch`) that emits `OSC 9;4` sequences into the PTY. Not blocked by remote architecture.
- **Agent IPC badges (Claude Code specifically):** Aislopdesk already has `ClaudeStatus`/`ClaudePaneDetector`/`AgentControlListener` (per MEMORY.md). The processing/idle/awaiting-input states from the agent IPC can drive the same badge types as otty's agent badges. This is a direct mapping.
- **`otty watch:<agent>` blocking:** Can be implemented as a host-side CLI tool that polls the agent control socket and exits with the same codes (0/4/9).

### What cannot map 1:1 / caveats
...
- **Taskbar-style progress bar (macOS/Windows taskbar):** Otty surfaces a taskbar-style progress bar on supported platforms. On macOS this would be the Dock tile progress indicator (`NSDockTile.badgeLabel` or custom drawing). This is implementable on macOS only; iOS has no equivalent.
...
- **Settings UI location:** Otty's settings are under "Settings → Advanced / Appearance / Shell". Aislopdesk uses `PreferencesStore` + `EnvConfig`. The config keys should be added to `PreferencesStore` with the defaults documented above. The Settings UI pane in the client needs sections for these toggles.
