...
- `otty watch <COMMAND>` — wraps a command and drives spinner/checkmark/error badge based on its exit status.
...
- **Drag and drop** rearranges panes: drop a pane tab onto any edge of another pane to split above/below/left/right. Otty's drag-and-drop is described as more flexible than Ghostty's.
...
- Tab labels use the OSC-set title or a path fragment (e.g. `~/Workplace/Otty`, `abner@MacBook-AB:...`, `CREDITS.md`).
...
**Text field** — below the toggle, a single-line text input pre-filled with the current name (e.g. "Otty Workspace"), with the text selected/highlighted in blue.

**WORKING DIRECTORY** subsection — shows the current directory path (e.g. `~/Workplace/otty/`).
...
- **Vertical sidebar tab list**: the current otty-based design system already implements a left sidebar. The tab row visual (label + badge + optional card highlight for active) maps to the existing `TabRow` / `SidebarView` components.
- **Tab badges**: the badge types (spinner, checkmark, dot, error, hand, shield, SSH, coffee) map to detectable shell/process states aislopdesk already tracks (OSC-133 command tracking, Claude agent state, `sudo` detection, SSH session detection via `AISLOPDESK_SYSTEM_DIALOG_PANES`). Implement as a `TabBadge` enum in `WorkspaceStore`.
- **Split panes**: `NSSplitView`-based live-resize already exists in aislopdesk; equalize (`⌘⌃=`) and directional focus (`⌘⌃←/→/↑/↓`) need wiring.
- **Tab grouping (By Project / By Date)**: the git-repo grouping maps naturally since aislopdesk tracks `cwd` per pane and can call `git rev-parse --show-toplevel` to derive a project key. Date grouping is trivial.
...
- **Config keys** (`window-size`, `window-cols`, `window-rows`, etc.): map to `PreferencesStore` / `Defaults`-backed `SettingsKey` values.
...
- **`otty watch <CMD>` badge driver**: wrap a command and emit exit-status badge via OSC or a sentinel stdout sequence. Map to aislopdesk's existing OSC-133 / block-output detection. Medium effort.

### Cannot map 1:1

- **Picture in Picture (PiP)**: macOS PiP is a system AVFoundation/PlayerLayer feature used for video. Otty uses it to float a terminal pane above other apps. On aislopdesk, the client sees a **decoded video stream** (HEVC from host), so true PiP would need to pipe that stream into an `AVPictureInPictureController`. Possible in principle but requires careful integration with `AislopdeskVideoClient`'s decoder output. Flag as **not in scope for initial clone**; a simpler "always-on-top window" via `NSWindow.level = .floating` is a viable approximation for the Current Pane mode.
- **Host-side cwd detection**: otty runs locally and reads the shell's `cwd` directly via `lsof`/`proc_pidvnodepathinfo`. Aislopdesk's terminal runs on a **remote host**; `cwd` must be read via the existing `AISLOPDESK_AGENT_CONTROL` NDJSON channel or an OSC 7 (`file://host/path`) sequence emitted by the shell. OSC 7 is the standard approach — add it to the shell integration shim.
- **SSH badge (remote session indicator)**: in otty (local), detecting an SSH child process is a local `ps` query. In aislopdesk every session IS remote; the "remote" badge would need a different semantic — e.g., badge for a *nested* SSH from within the remote shell. Map as: detect `ssh` child of the PTY via the NDJSON control channel or OSC-133 command prefix.
