...
The VSCode-style command palette for _running an action_. Counterpart to Open Quickly, which is for _jumping to a thing_. Opened with ⌘⇧P from anywhere, it exposes every Otty action — including ones without keyboard shortcuts — in a searchable, categorised list. Actions are tagged with their scope (pane, window, or app level).
...
- The palette shows context-aware content at the top: under "WORKING DIRECTORY" it shows the focused pane's current working directory as a tinted badge (folder icon + path like `~/Workplace/otty/`). This badge appears in the top-right of the section header row.
- Actions cover all scope levels: pane-scoped (e.g. Find, Toggle Bold Brightens), window-scoped (e.g. Close Window, Toggle Tabs Panel), and app-scoped (e.g. About Otty). Each action is tagged with its scope internally (the screenshot shows this through grouping/section headers).
...
- May carry a contextual badge on the right side: in the "WORKING DIRECTORY" section, a pill-shaped badge shows a folder icon (dark gray, ~12 pt) followed by the cwd path string (e.g. `~/Workplace/otty/`) in dark gray text, on a slightly darker gray pill background. The badge sits flush right inside the row.
...
- **"Split Right" (planned in otty):** Aislopdesk has pane splitting in `WorkspaceStore`. Exposing it in the palette is straightforward.
