...
- **Cursor Click-to-Move** (toggle, default ON — green): Click in the prompt to move the shell cursor there — Otty sends the right number of arrow keys, even across soft-wrapped rows. Subtext: "Click in the prompt to move the shell cursor — sends arrow keys across soft-wrapped rows."
...
- When **Allow Mouse Capture** is on and a program enables reporting (vim, htop, tmux), clicks, drags, and scroll wheel are forwarded to the program instead of Otty's selection engine.
...
- Full-screen TUIs (yazi, btop, mc, ranger) can tell Otty what pointer shape to show over the pane via `OSC 22`.
- Examples: pointer over clickable buttons, crosshair over selectable cells, no-entry over read-only regions.
- Otty maps standard CSS cursor names to the closest native macOS system pointer.
...
All settings are GUI-only (Settings panel). The equivalent config-file keys are listed below based on the settings labels and otty's documented config format.
...
3. **Open in Otty** — regular weight (with right-pointing disclosure arrow ▶ indicating submenu)
...
**Title bar:** macOS window chrome at top — "abner@MacBook-Pro: ~/Workplace/otty" in center as window title, traffic light buttons (red/yellow/green) at top-left.
...
- **Cursor style (Block/Bar/Underline/hollow Block)**: libghostty exposes DECSCUSR and cursor style via `ghostty_config_t`. Aislopdesk's `TerminalConfigBuilder` can pass these through; the client UI settings panel mirrors the four options.
...
- **Right-Click Action (Context Menu / Copy / Paste / Copy or Paste / Ignore)**: client-side gesture handler in `TerminalRenderingView`. All options are local; Otty's "Composer" and "Send to Chat" items in the context menu are agent-integration features (map to Aislopdesk's agent control if present).
...
- **Mouse reporting modes (DECSET ?1000/?1002/?1003) + SGR encoding (?1006)**: libghostty implements these; Aislopdesk must forward the resulting input bytes from the client's pointer events via the terminal data channel. Already handled if the input path passes raw mouse events to libghostty for encoding.
- **Motion/drag de-duplication at cell boundaries**: libghostty handles this; Aislopdesk passes pointer position and libghostty decides whether to emit a report.
- **Modifier state on mouse reports (Shift/Option/Control)**: libghostty encodes these from the event's `modifierFlags`.
- **OSC 22 (pointer shape)**: libghostty parses OSC 22 and exposes a callback. The Aislopdesk client side responds by setting `NSCursor` to the nearest system cursor matching the CSS name. Standard mapping: `default` → arrow, `pointer` → pointingHand, `text` → iBeam, `crosshair` → crosshair, `not-allowed` → operationNotAllowed, `grab`/`grabbing` → openHand/closedHand, `wait` → spinning.

### Partial / needs care

- **Mouse Over to Focus (hover-to-focus panes)**: Aislopdesk supports multiple panes in a split view. Hover-to-focus works client-side via `NSTrackingArea` on each pane's view. The complication is that focus must be relayed to the correct pane's PTY channel on the host — the focus event triggers a `FOCUS_IN` terminal sequence sent to the active pane's PTY. Straightforward, but ensure focus change doesn't cause visible flicker in the title bar.
...
- **Allow Mouse Capture toggle**: This is a runtime-switchable gate. When turned off mid-session, Aislopdesk must send DECRST ?1000/?1002/?1003 to the program — or simply suppress forwarding. libghostty may handle this if the config key is hot-reloadable; otherwise inject disable sequences manually.

### Cannot map 1:1 — requires design decision

- **"Open in Otty" context menu item (submenu)**: In the right-click screenshot Otty shows "Open in Otty" with a submenu, presumably for opening a selected path or link in an Otty split/tab. Aislopdesk's equivalent would be "Open in Aislopdesk" or "Open in New Pane" — semantics differ because Aislopdesk is a remote tool; a path on the remote host cannot be opened locally in a native viewer without a protocol extension (e.g., file fetch over the inspector path).
- **"Composer" and "Send to Chat..." context menu items**: These are Otty's agent-integration items (feed selected text to an AI composer or chat). Aislopdesk has its own agent-control channel (`AISLOPDESK_AGENT_CONTROL`). Map to agent pane send if applicable, or omit initially.
- **Remote SSH badge in title ("abner@MacBook-Pro: ~/Workplace/otty")**: Otty shows the shell's cwd/hostname in the window title. Aislopdesk already receives OSC 7 (current working directory) from the host PTY via the data channel; the client can display `user@host: cwd` in the pane title bar. This works for the macOS client. For iOS, the title bar is more constrained — show abbreviated form.
- **"Split Pane" and "Switch to View" context menu submenus**: Aislopdesk has split-pane support (`WorkspaceStore`). These context menu items can open the pane chooser or create a new split. Implement as part of the right-click menu builder.
- **Cursor opacity**: libghostty may not expose a cursor opacity config independently of the terminal background alpha. Verify `ghostty_config_t` fields before exposing the setting; if unsupported, omit or gray out the control in the Aislopdesk settings panel.
