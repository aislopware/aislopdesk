...
How to pick, switch, and write themes. Otty supports a light/dark theme pair that follows the OS appearance, custom `.ottytheme` TOML files, and import from five terminal formats (Otty, iTerm2, Kitty, Alacritty, Ghostty). Theme switching is live — no restart required. Built-in themes include Paper (light default) and Nord (dark default).

## Behaviors

- Theme can be switched from: Main menu → View → Themes; Command Palette (type "Theme" or filter by theme name directly); Settings Panel → Appearance → Themes; CLI (`otty theme list` / `otty theme set <theme>`); direct `config.toml` edit (`theme = <theme>` row).
...
- Custom themes are added by placing an `.ottytheme` file in `~/.config/otty/themes/` (macOS/Linux) or `%APPDATA%\otty\themes\` (Windows) — the same folder Otty writes to when editing colors in the Settings Panel.
- The file name is the theme slug: display name lowercased, non-alphanumeric characters become `-`. Example: "My Cool Theme" → `my-cool-theme.ottytheme`. Human-readable name goes in `[meta] name`.
- New theme files appear next time the theme list opens; relaunch Otty if not visible. No build step required.
...
- Import a theme file directly — Otty drops it in the themes folder, converting from other formats as needed.
- Supported import formats: Otty `.ottytheme` (chrome styling preserved), iTerm2 `.itermcolors` (covers nearly every scheme online), Kitty color `.conf` (`foreground #fff` / `color0 #000`), Alacritty `[colors.*]` `.toml`, Ghostty theme files (`foreground = #fff` / `palette = 0=#000`).
...
- From Finder: double-click `.ottytheme` or run `open Nord.ottytheme`. Otty prompts: "Import" (add only) or "Import & Apply" (add and switch).
- From CLI: format auto-detected; accepts local path or `http(s)` URL. Can skip `theme` subcommand for `.ottytheme` or `.itermcolors`.
  - `otty import ~/Downloads/Nord.ottytheme` — add to themes
  - `otty import ~/Downloads/Nord.ottytheme --activate` — add and switch
  - `otty theme import ~/Downloads/Dracula.itermcolors` — explicit form, any supported format
  - `otty theme import https://themes.otty.sh/nord.toml` — import from web
  - `otty theme import https://themes.otty.sh/nord.toml --overwrite` — update existing
- Imported theme lands at `~/.config/otty/themes/<slug>.ottytheme`. If slug exists, Otty appends `-1`, `-2`, etc. Use `--overwrite` (CLI) to update in place.
...
- Top strip: partial terminal preview at top showing `ls -l` output with colored columns (file permissions in gray, sizes in orange/red, user "otty" in green, date in blue, filename in light gray). This is a live mini-preview of the selected theme applied to a terminal surface.
...
- Theme file discovery from `~/.config/otty/themes/` has no direct aislopdesk equivalent but the concept maps to a user themes directory that `PreferencesStore` / `ThemeStore` can scan. Can be implemented as `~/.config/aislopdesk/themes/` on the CLIENT (macOS) side.
...
- **CLI import (`otty import`, `otty theme import`)**: aislopdesk has no `otty`-equivalent CLI for themes yet. The host-side `aislopdesk-ctl` could be extended, but theme management is purely client-side in aislopdesk's architecture. No host involvement needed.
- **Import from URL (`https://themes.otty.sh/...`)**: requires client-side networking. Feasible but needs a dedicated UI affordance; the `themes.otty.sh` URL is otty-specific — aislopdesk would need its own theme gallery or accept the same `.ottytheme` / `.itermcolors` URL pattern from arbitrary URLs.
...
- **`[token] font-ui`** (window-chrome font): in aislopdesk the chrome is SwiftUI; font is controlled via SwiftUI `.font()` modifiers. This can be threaded through `OttyDesign` tokens but requires care to not break layout.
- **Theme slug collision handling** (append `-1`, `-2`): small implementation detail, no architecture blocker.
- **Finder double-click / `open` integration** (`.ottytheme` UTI registration): requires a macOS UTI declaration in the app bundle. Low priority; CLI or Settings Panel import is sufficient for v1.
