...
No content screenshots are present on this page. The page is purely prose + tables — no annotated UI images.

The site chrome visible from the favicon/logo fetch:
- `/otty-icon.png` — the Otty product icon (used in `<link rel="icon">`).
- `/otty-face.svg` — the Otty logotype/face mark used in the top navigation bar.
...
### What does NOT map 1:1 / requires special handling
