...
This page covers every dimension of font configuration in otty: font installation without system Font Book, font family selection (global vs per-theme vs fallback), font size, line height (cell height), ligatures, bold/italic/underline/blink rendering modes, and sub-pixel glyph blending. The bundled default is JetBrains Mono. Fonts are loaded from `~/.config/otty/fonts/` via CoreText at launch — no system-wide install required. Settings are scoped to Computed / Global / Light Theme / Dark Theme / Fallback tabs, and can be written to either global `config.toml` or per-theme TOML files.

## Behaviors

- At launch, otty registers all font files found in `~/.config/otty/fonts/` with CoreText so they are immediately available to the Font Family picker and `font-family` config key — no Font Book or system installation needed.
- Accepted font file extensions: `.ttf`, `.otf`, `.ttc`, `.otc` (case-insensitive; all other extensions ignored).
- Font installation shortcut: Settings → Appearance → Font Family → "Open font folder" button opens `~/.config/otty/fonts/` in Finder. Alternatively, `open ~/.config/otty/fonts` from a shell.
- CLI font import: `otty font import <font file path>`.
...
- Font Family setting has three scope tabs: **Global** (writes to `~/.config/otty/config.toml`, applies everywhere), **Light Theme** / **Dark Theme** (writes to the active theme TOML, travels with the theme), and **Fallback** (comma-separated list of fonts used when the primary font lacks a glyph, e.g. for CJK or icon characters).
- When "Auto-match weight & style" toggle is ON (default), otty automatically selects the real bold, italic, and bold-italic faces of the chosen font family without user intervention.
- When "Auto-match weight & style" toggle is OFF, four separate pickers appear for Font Family (regular), Font Family (Bold), Font Family (Italic), and Font Family (Bold Italic), each populated with all available faces from all installed and `~/.config/otty/fonts/` fonts.
...
### Font installation
- **Cannot map 1:1.** Otty uses `~/.config/otty/fonts/` with CoreText registration at launch. Aislopdesk runs a remote macOS host; the terminal rendering (libghostty) executes on the HOST. Font installation must happen on the host machine, not the client. The client Settings UI can only configure font names, not install font files to the host. The `otty font import` CLI command equivalent would need to be a host-side operation (e.g. via the aislopdesk-ctl agent socket or a dedicated protocol message). For the initial client, expose the font family field as a text entry (no local discovery); show a note that fonts must be installed on the host.
...
- **Partial map.** Aislopdesk has a ThemeStore with light/dark variants. The per-theme font storage mirrors otty's Light Theme / Dark Theme tab behavior. Global writes to the top-level PreferencesStore/config. Implement as: a scope selector (Global / Light Theme / Dark Theme / Fallback) above the Font Family combobox in Appearance settings, writing to the appropriate config layer.
...
### Font blending
- **Maps to Ghostty's `font-thicken` / blending config.** Ghostty has `font-thicken` (a boolean for macOS-style subpixel thickening) and its own blending/anti-aliasing pipeline. The five otty blending modes do not map 1:1 to Ghostty's options. **Cannot clone all five modes.** Recommended mapping: `Default` → Ghostty default, `macos-like` → enable `font-thicken`, others as best-effort. Document this as a known deviation; expose only Default/macOS-like in the initial client.
