...
The Advanced tab in Otty Settings is the single-pane escape hatch for every configuration key
...
Opening the Advanced tab: Settings (`⌘,` or Otty menu → Settings…) → click the **Advanced** tab
...
- The All Settings list is **complete**: it contains every Otty configuration key, including
...
[Otty Configuration Reference](https://docs.otty.sh/reference/configuration) for the full
...
- `all-settings.png` (1624×1144 px, sourced from `https://docs.otty.sh/screenshots/all-settings.png`)

---

## Aislopdesk mapping notes

**Architecture context:** Aislopdesk runs a macOS host (aislopdesk-hostd) and macOS/iOS clients.
...
- **Settings window with sidebar tab list** — fully implementable on macOS as a standard
  `NSWindowController` + `NSSplitView` (sidebar + content). Aislopdesk already has a settings
...
- **Config File section** — the host-side `~/.config/otty/config.toml` maps to aislopdesk's
  config file path; an "Open Config File" button can use `NSWorkspace.open(_:)`.
- **Debug logging toggle** — maps to `AISLOPDESK_VIDEO_DEBUG` / existing debug flags; the log
...
### What cannot map 1:1 or requires adaptation

- **`~/.config/otty/config.toml` path** — aislopdesk uses its own config path conventions
  (not otty's). The "Config File" section should reference aislopdesk's actual config location.
...
- **"Learn more →" links into Configuration Reference** — links to `https://docs.otty.sh/reference/configuration`. Aislopdesk would link to its own internal docs or could open a web view to aislopdesk documentation; there is no 1:1 external reference to link to today.
- **Remote host config keys** — some aislopdesk config keys (e.g. `AISLOPDESK_FEC_M`,
  `AISLOPDESK_VIDEO_DEBUG`) are host-side env vars, not client-side TOML keys. The Advanced
...
- **Reload Config** — aislopdesk's `PreferencesStore` does not have a concept of a TOML file
