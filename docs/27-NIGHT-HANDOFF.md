...
| 3 | GUI video (remote desktop) | ✅ renders end-to-end | RworkVideoHost/Client + §A optimizations |
...
- **Transport-level connect timeout** (`Sources/RworkTransport/Mux/NWMuxByteLink.swift` → `withMuxConnectTimeout`, ~10s) so a dead/unreachable host **fails fast instead of hanging in "connecting" forever**. Implemented without wrapping `client.connect` in a child task group (that path deadlocks).
...
### #4 Modern client UX
- **Per-pane status dots** in the sidebar + pane header (`Sources/RworkClientUI/Workspace/Views/PaneStatusIndicator.swift`), surfacing the #2 reconnect/timeout states.
...
- **Fix:** replaced `posix_spawn` with `fork()` (resolved via `dlsym`, since Swift marks `fork` unavailable) → child runs only raw async-signal-safe syscalls: `login_tty(slave)` (= `setsid` + `ioctl(TIOCSCTTY)` + `dup2`) → `close(master)` → `execve` (argv/envp/path built in the parent pre-fork). `Sources/RworkHost/PTYProcess.swift` (heavily doc-commented).
...
- HW sanity: host with an active live session, SIGINT → **exits promptly** (was: would hang). Full `RworkHostTests` 80/80.
...
swift build -c release --product rwork-hostd        # → .build/release/rwork-hostd  (terminal host; --port, default 7420)
swift build -c release --product rwork-videohostd   # → GUI-video host
swift build -c release --product rwork-client       # → CLI terminal client

# Client GUI app (Rwork.app) — needs the libghostty-wired project.yml (renderer-ENABLED, intentional)
...
# → .work/macos-verify/DerivedData/Build/Products/Debug/Rwork.app

# Host menu-bar GUI app (RworkHost.app) — NEW this session
...
# → RworkHost.app  (menu-bar agent; Start/Stop host + TCC checklist)

# Run (terminal): start host, then launch the client GUI binary DIRECTLY (not `open` — it drops env vars)
./.build/release/rwork-hostd --port 7779 &
RWORK_AUTOCONNECT_HOST=127.0.0.1 RWORK_AUTOCONNECT_PORT=7779 \
  .work/macos-verify/DerivedData/Build/Products/Debug/Rwork.app/Contents/MacOS/Rwork

# Video: needs Screen Recording TCC granted + a real unlocked Aqua session (else CGS_REQUIRE_INIT).
#   Seams: RWORK_VIDEO_AUTOCONNECT=1, RWORK_VIDEO_DEBUG=1
```

Debug seams left ON: `RWORK_RENDER_DEBUG=1` (terminal render log), `RWORK_VIDEO_DEBUG=1`, `RWORK_SHELL_INTEGRATION=0` (opt out of the zsh shim).

## Tests
674 across 6 filtered suites, 0 failures: `RworkClientUITests` 294, `RworkHostTests.PTYProcessTests` 13, `ShellIntegrationTests` 11, `RworkVideoProtocolTests` 86, `RworkVideoHostTests` 161, `RworkVideoClientTests` 109. (Full `RworkHostTests` is 80.) Always `swift test --filter <Specific>`; nothing that constructs a `HostServer`.

---

## Pre-commit checklist (when you decide to commit)

1. **Debug logging** — `RWORK_RENDER_DEBUG`/`rdbg` (GhosttyTerminalView.swift etc.) and `RWORK_VIDEO_DEBUG` are still wired. They're env-gated (silent by default) but you may want to gate/trim the noisier `[RDBG]`/`[CONN]` lines before committing.
...
3. **WF4 `ShellIntegration` shim** — now somewhat redundant given the ctty fix (default zsh redraws on SIGWINCH once the ctty is correct), but it's **harmless belt-and-suspenders** (its `TRAPWINCH` forces `zle reset-prompt`; for starship which has no own TRAPWINCH, it guarantees the reprint). Keep, or drop if you prefer minimalism (opt-out is `RWORK_SHELL_INTEGRATION=0`).
...
5. **Branch** — `fix/terminal-render-connect-once` carries ~20+ changed/new files across RworkHost, RworkTransport, RworkClient(UI), RworkVideoHost, and the two app targets. It's a large but coherent diff; consider splitting into reviewable commits per issue.
...
---
*Generated autonomously. Verification evidence (screenshots, logs) under `/tmp/rwork-hw/`, `/tmp/wf*-*.png`, `.work/video-verify/`. Memory: `memory/rwork-hw-validation-2026-06-05.md`.*
