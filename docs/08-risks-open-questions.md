...
| R2 | 🔴 | *(GUI only)* Cooperative activation refuses activation from network/timer events — the remote-control case. | `AXRaise` (reorders even when app activation is throttled) + try deprecated `activateIgnoringOtherApps:`. Measure on the macOS 26 target. |
| R3 | 🟡 | *(GUI only)* Bursty Wi-Fi loss/jitter breaks the 30–50 ms target. | Plain UDP on a trusted private network; the video path carries FEC + adaptive bitrate / congestion control (Rust core). |
| R4 | 🟡 | Keyboard layout / dead keys (´+e→é) are host-dependent. | Layout-independent Unicode injection for text; keycodes only for shortcuts (games that ignore Unicode → keycode fallback). |
...
| R9 | 🟢 | The mesh may fall back to relay (>80 ms) on hard NAT. | Design for direct P2P; if relayed, surface + warn only. The video path already has FEC + ABR for loss. See [13](13-network-transport.md). |
| R10 | 🟡 | macOS 26 decode/present latency regression (Moonlight #1696: CAMetalDisplayLink ~20–80 ms; Metal off → ~2.7 ms). | Test on the macOS 26 target; a fallback present path may be needed. |
...
> 📋 Many of these are resolved in [11-absolute-latency.md](11-absolute-latency.md) §"Verified API claims" (corrections + the Phase-0 checklist).
...
| Q4 | Auth/pairing? | None at the app layer — the security boundary is the network (a WireGuard mesh provides encryption + node auth + ACLs) |
...
- `kAXPositionAttribute` (AX) == `kCGWindowBounds` (CG), both top-left/points — true in practice but no Apple page states it; verify with runtime prints before trusting it for pixel-accurate clicks.
