...
> **STATUS: SUPERSEDED.** This is the old *video-first* roadmap. The shipped plan reorders around the hybrid architecture: the **terminal PTY text-path is Phase 1** (simpler, higher value, sidesteps input injection); the **GUI video path moves to Phase 4**. See [00-overview.md](00-overview.md) · [12 §"Revised phased roadmap"](12-coding-profile.md) · [DECISIONS.md](DECISIONS.md). Content below is kept only as a detailed reference for the GUI video-path.
