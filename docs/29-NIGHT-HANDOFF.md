...
**Fix** (`ThirdParty/ghostty/patches/0002-rwork-fix-sizereport-recursive-lock.patch`, tracked): added a
...
## ✅ Host TERM/terminfo bootstrap (`Sources/RworkHost/TerminfoResolver.swift` + HostServer)
...
`RworkClientUITests` 306/0. The fix restores known-working code + repaint-safety reviewed → HIGH
...
- 🟥 **HIGH regression (caught + fixed)**: enabling `supports_selection_clipboard = true` + a
  ...
  `location` — SELECTION → a private `NSPasteboard(name: "com.rwork.terminal.selection")` (preserves
...
- 🟧 **DOCUMENTED (fix needs iOS runtime verification — touches the just-restored keyboard path)**: on iOS a
  pane has **TWO independent OUT drains** to the same `RworkClient` actor — the keyboard
...
1. **iOS keyboard runtime verification** — the headless verification is strong; the SIM runtime was
   ...
   - The sim app did NOT connect to a loopback `rwork-hostd`, and screenshots showed it not frontmost
...
Ran a 6-dimension ultracode audit comparing rwork's terminal subsystems against canonical OSS
...
- **#1/#2 (HIGH) clipboard recursion → stack-overflow crash.** `confirm_read_clipboard_cb` re-completed
  ...
  AUTO-APPROVES (`confirmed:true`) since rwork has no dialog (the canonical embedder pattern — the confirm
...
- **#3 (HIGH) reconnect silently drops the fresh shell's output.** `MuxClientTransport.returningClient`
  ...
  "duplicate". FIX: `RworkClient.connect` now resets `highestSeqFed`/`highestContiguousSeq`/`ackPending`
  ...
  `RworkClientDedupTests.testReconnectResetsDedupSoFreshShellOutputIsNotDropped`.
...
- **#16 (doc) over-promised "byte-exact resume".** `RworkClient`/`ReconnectManager` doc-comments claimed
...
Code/tmux), grounded in the SwiftUI code + live screenshots (`/tmp/rwork-hw/ux-*.png`): **23 confirmed (2
...
- **Every pane titled "Terminal" (navigation).** FIX: `PaneChromeView` now prefers the LIVE OSC 0/2 title
  ...
  header showed the shell's cwd-based title ("/V/L/W/o/rworkspace"), not "Terminal".
...
- **Two HIGH `ConnectionRegistry` reconnect-storm races** (race; user-visible). On a shared-link drop with
  ...
  SECOND one and orphan it. (`Sources/RworkTransport/Mux/ConnectionRegistry.swift`.) Test:
...
- RworkClient: `RworkClient.swift` (round-2 #3 reconnect dedup reset + doc), `ReconnectManager.swift`
  (round-2 #16 doc).
- RworkTransport: `Mux/ConnectionRegistry.swift` (round-4 identity-gated eviction/release races).
- RworkHost: `PTYProcess.swift` (round-4 reaper exit-0 sentinel).
- RworkClientUI: `Terminal/TerminalScreenView.swift`, `Terminal/TerminalViewModel.swift` (round-2 #9
...
- RworkHost: `HostServer.swift`, **new** `TerminfoResolver.swift`, **new**
  `Tests/RworkHostTests/TerminfoResolverTests.swift`.
- ThirdParty/ghostty: `build-libghostty.sh`, `integration/GhosttySurface/GhosttySurface.swift` (round-2 #1/#2
  ...
  #4 iOS pan cursor-pos), **new** `patches/0002-rwork-fix-sizereport-recursive-lock.patch`.
- Tests: `RworkClientTests/RworkClientDedupTests.swift` (round-2 #3 reconnect-reset regression),
  `RworkClientUITests/TerminalViewModelTests.swift` (round-2 #9 fresh-session wipe + round-3 exit/disconnect
  clears-running), `RworkTransportTests/ConnectionRegistryTests.swift` (round-4 reconnect-storm).
- `README.md`.

Round-2 verification: headless suites green (RworkClientUITests 308/0, RworkClientDedupTests 2/0,
TerminfoResolver 15/0, RworkHostSmoke 16/0, ReplayBuffer 19/0, RworkClientSmoke 4/0); macOS renderer app +
rwork-hostd/rwork-client build clean; iOS-triple build clean. HW evidence: `/tmp/rwork-hw/cb-*.png`,
`recon-*.png`.
...
1. **HIGH zombie-transport reconnect race** (`RworkClient.connect`) — actor reentrant across `await
   ...
   throws — a throw makes `ReconnectManager` retry & fight the winner). Test: `RworkClientReconnectRaceTests`
   (3 tests × 200-loop).
...
R5 verification totals (direct binary): `RworkClientReconnectRaceTests` 3/0, `FrameDecoderCursorTests` +
...
R5 files changed: `Sources/RworkClient/RworkClient.swift`, `Sources/RworkProtocol/{FrameDecoder,
WireMessage+Encode}.swift`, `Sources/RworkProtocol/Mux/{MuxEnvelope,MuxFrameDecoder}.swift`,
`Sources/RworkTransport/ReplayBuffer.swift`, `Sources/RworkTransport/Mux/{MuxSubChannel,MuxNWConnection}.swift`,
`Sources/RworkHost/{HostServer,MuxChannelSession,PausableQueueGate}.swift`. New tests:
`RworkClientReconnectRaceTests`, `FrameDecoderCursorTests`, `MuxConnectionLifecycleTests`,
...
- **(#1, HIGH) `ConnectionViewModel.connect()`/`resume()` had no supersede guard** — the VM analogue of
  R5 rank-1. Since `RworkClient.connect` now RETURNS (not throws) on supersede, the VM's success branch
...
`Sources/RworkClientUI/Connection/ConnectionViewModel.swift`, `Sources/RworkProtocol/Mux/{ChannelTable,
FlowCreditPolicy,MuxFlowControl}.swift`, `Sources/RworkTransport/Mux/MuxNWConnection.swift`,
`Sources/RworkInspector/{InspectorReplayLog,NWByteChannel}.swift`. New test:
...
`Sources/RworkInspector/InspectorReplayLog.swift`, `Sources/RworkTransport/Mux/MuxNWConnection.swift`,
`Sources/RworkProtocol/Mux/ChannelTable.swift`, `Sources/RworkClientUI/RworkClientApp.swift`,
`Sources/RworkVideoProtocol/FrameReassembler.swift`, `Sources/RworkVideoClient/RworkVideoClientSession.swift`.
...
- **(#2, HIGH) host advertised `TERM=xterm-ghostty` the child couldn't resolve** — the terminfo PROBE
  ...
  `RworkHostSmokeTests.testCuratedEnvironmentForwardsTerminfoSearchPath`.
- **(#3, HIGH) per-session ZDOTDIR shim dir never deleted** — one `rwork-zdotdir-*` dir + 4 files leaked
...
UI 311/0; iOS build SUCCEEDED. R8 files changed: `Sources/RworkTransport/Mux/{ConnectionRegistry,
MuxNWConnection}.swift`, `Sources/RworkHost/{HostEnvironment,ClaudeCodeProfile,MuxChannelSession,HostServer}.swift`,
`Sources/RworkProtocol/Mux/ChannelTable.swift` (stateCount seam). New tests across `ConnectionRegistryTests`,
`MuxConnectionLifecycleTests`, `RworkHostSmokeTests`, `MuxChannelSessionBackpressureTests`.
...
Also fixed (R9 #4 completion): **`RworkClaudeCode/TerminalModeTracker`** (the 3rd parser, input-box mode
...
real-binary E2E 1/0; iOS build SUCCEEDED. R9 files changed: `Sources/RworkHost/{HostServer,
HostTitleBellSniffer,HostCommandStatusSniffer}.swift`, `Sources/RworkClaudeCode/TerminalModeTracker.swift`,
`Sources/RworkTransport/Mux/MuxRoutingCore.swift`, `Sources/RworkTransport/HostTransport.swift`. New tests
...
`pendingResultCap=4_096` eviction, and `clearOpenCard` on completion (45 RworkInspectorTests green). R10's
...
build SUCCEEDED. R10 files changed: `Sources/RworkTransport/HostTransport.swift`,
`Sources/RworkClientUI/iOS/IMEProxyTextView.swift`. New test in `KeyRepeaterTests`.
...
- **(LOW dead-code) removed `RworkVideoHostSession.handleReap()`.** Documented as wired via
  ...
  (verified); removed rather than left as a confusing public no-op. (160 RworkVideoHostTests still green.)
...
SUCCEEDED. R11 files changed: `Sources/RworkInspector/InspectorViewModel.swift`,
`Sources/RworkTransport/Mux/{NWMuxByteLink,MuxNWConnection}.swift`,
`Sources/RworkClientUI/Connection/ConnectionViewModel.swift`,
`Sources/RworkVideoHost/RworkVideoHostSession.swift`, `Sources/RworkTransport/HostTransport.swift`. New
...
- **(highest-value win) Humanized connect-failure errors.** `RworkTransportError` had no `LocalizedError`,
  ...
  failed — is this an rwork host?", …) + swapped the two `String(describing: error)` sites in
  `ConnectionViewModel` (connect + resume catch) for `error.localizedDescription`. Test:
  `RworkTransportErrorTests` (clean line + no detail-payload leak). Lowest blast radius, highest visibility.
...
- **(LOW regression — error dump) the connect/resume catches' `error.localizedDescription` humanizes
  `RworkTransportError` but bridges a NON-`LocalizedError` (`ClientError`, `CancellationError`) to
...
SUCCEEDED; HEAD still `b49ec37`. Files changed: `RworkTransportError.swift`, `ConnectionViewModel.swift`,
...
- **(LEAD, HIGH) First-run onboarding caption.** The connect form prefills a default host, misleading a
  ...
  rwork-hostd." under the form headline (`PaneLeafView.connectForm`). Biggest comprehension gain, zero risk.
...
- **(A#4) `HostController.describe`** had a DEAD `NSPOSIXErrorDomain && code==48` branch (the error is
  `RworkTransportError.listenerFailed`, never bridges to POSIX). Rewrote to match the enum + thread the
...
`Sources/RworkClientUI/iOS/KeyEncoding.swift` (mirroring the `FloatingCursorMapping` / `KeyboardAccessoryDecision`
...
- **(MED #2) Reconnect ack-ticker leak** — `RworkClient.connect()` re-checked liveness only BEFORE four
...
`ShellIntegration.swift`, `RworkClient.swift`, `ConnectionViewModel.swift`, `InspectorViewModel.swift`,
...
1. **(HIGH) reconnect supervisor never checked `closed`** — `ReconnectManager.start()` + `reconnectLoop()`
   ...
   (reachable in the headless CLI, which never cancels the supervisor). Fix = new `RworkClient.isClosed` +
...
6. **(LOW) `describe()` `.contains("48")` misclassified** — any error whose text embedded "48" (port 4843,
   ...
   `RworkTransportError.listenerDetailIndicatesAddressInUse` matching the "in use" phrase + errno 48 only as
...
**THE 7 (3 MED, 4 LOW):**
1. **(MED) CLI-1 — rwork-client never cancels the reconnect supervisor** (`@discardableResult`, discarded). On
...
2. **(MED) HOSTVIEW-1 — host port field accepts negative / out-of-range**, silently coerced
   ...
   Fix = pure `RworkTransport.PortValidation` (`isValid`/`port`/`clamped`) + disable Start when invalid + an
...
4. **(LOW) HOSTD-1 — rwork-hostd/videohostd SIGINT handler** spawns a Task ending in `exit(0)` per signal; a
...
5. **(LOW) HOSTD-2 — rwork-hostd inspector-bind failure `exit(1)`s without `server.stop()`**, leaking the
...
**R16 DEFERRED (recipe noted):** mutually-latch the rwork-hostd inspector-failure `exit(1)` against a
...
leaves a STALE incremental-link reference to the old symbol from an executable (rwork-hostd) — keep the public
...
**R16-deferred completion (shipped):** the rwork-hostd inspector-bind-failure `exit(1)` now routes through the
...
`RworkTransportError.waitingErrnoIsFatalBindConflict(_:)` (codebase idiom: pure decision + thin NW glue, since
...
---
*Generated autonomously. HW evidence: `/tmp/rwork-hw/*.png`. Memory:
`memory/rwork-core-mouse-clipboard-2026-06-06.md`. Build/HW logs under `.work/macos-verify/`.*
