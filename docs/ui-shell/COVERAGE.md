...
Method: lean per-section sonnet agents read each spec page, cross-checked the reference screenshot, and reported only self-verified gaps. Aislopdesk's terminal **emulation** is the embedded **libghostty** engine (the real ghostty), so the entire VT/Terminal-API section (C0/ESC/CSI/OSC *parsing*) is provided by libghostty, not reimplemented — only the **app-level** OSC behaviours (7/8/9/52/133/9;4/1337) are aislopdesk's own responsibility.
...
## A. Covered (aislopdesk implements the documented feature)
...
- **Cloud/sync features:** Data Sync and third-party SSH/Remote-Development tooling are out of scope — aislopdesk has its own remote model (host + client over a trusted WireGuard mesh). *(Recipes, Session-Recovery, CLI, and Frequent-Folders WERE implemented under other epics.)*
...
| **Theme catalog** — a wider built-in theme catalog including Nord (aislopdesk ships 8: 6 Monokai Pro + Paper + Dark, vs. a ~24-theme target) | customization/themes | Med | aislopdesk deliberately defaults to Monokai Pro; catalog breadth is the gap |
| **bash / fish shell integration** — OSC-133 injection for `~/.bashrc` + fish `vendor_conf.d` (aislopdesk is zsh-only) | terminal-features/shell-integration | Med | bash/fish users currently get no blocks/badges/notify/auto-progress |
