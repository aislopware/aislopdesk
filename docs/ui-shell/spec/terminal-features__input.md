...
- **Cut (`⌘X`)**: always copies the selection to the clipboard. If the selection is editable text on the prompt line, aislopdesk also deletes it. On read-only text (scrollback, program output) it falls back to a plain copy. Also accessible at Edit ▸ Cut.
...
- **Secure Keyboard Entry — Automatic**: with "Auto Secure Input" on (Settings → Controls, on by default), aislopdesk enables macOS Secure Keyboard Entry the moment the active session requests a hidden password (sudo, ssh, login prompts) and releases it afterward.
