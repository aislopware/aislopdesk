...
The Advanced tab in Aislopdesk Settings is the single-pane escape hatch for every configuration key
...
Opening the Advanced tab: Settings (`⌘,` or Aislopdesk menu → Settings…) → click the **Advanced** tab
...
- The All Settings list is **complete**: it contains every Aislopdesk configuration key, including
...
[Aislopdesk Configuration Reference](reference__configuration.md) for the full
...
- **Config File section** — the config file lives at aislopdesk's own
  `~/.config/aislopdesk/config.toml` path; an "Open Config File" button can use `NSWorkspace.open(_:)`.
...
- **"Learn more →" links into Configuration Reference** — should link to aislopdesk's own `reference__configuration.md` doc (in-app, this could be a bundled reference view or a web view onto the built docs site).
