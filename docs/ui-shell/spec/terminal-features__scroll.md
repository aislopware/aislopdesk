...
- **Title bar**: Single-line centred title "abner@MacBook-Pro: ~/Workspace/aislopdesk" in medium grey, no additional icons.
...
- **Prompt style**: `~/Workspace/aislopdesk (main x)●+↑ ▷` — coloured segments (cyan for path, purple/pink for git branch status characters `●+↑`, green triangle `▷` as prompt chevron). Shell Integration glyphs are active.
