...
Aislopdesk integrates with three coding-agent CLIs: Claude Code (`claude`), Codex (`codex`), and OpenCode (`opencode`). The user runs these agents exactly as they would in any terminal; Aislopdesk plugs into each via that agent's own hook/plugin mechanism and surfaces tab badges, notifications, session history, resume, and fork. The in-app experience is identical across all three. Integration is one-click: the first time Aislopdesk needs the integration it asks permission and installs the hook or plugin — it does not touch other settings.

Any other coding-agent CLI runs fine as a plain terminal process in an Aislopdesk tab; it just does not receive agent-aware features. Third-party agents can adopt the Terminal Resume Protocol (OSC 88) to gain resume support without a bespoke integration.
...
- **History** — completed sessions are captured and searchable; Aislopdesk can reload them.
- **Resume** — a past session can be reopened from within Aislopdesk by invoking the agent's resume flag with the session id.
- **Fork** — a past session can be branched into a new independent session.
- **One-click install** — the first time Aislopdesk needs the integration it prompts the user and installs the hook or plugin into the agent's own config directory. It does not touch any other settings.
- **One-click uninstall/re-enable** — the integration can be removed or re-enabled from within Aislopdesk.

### Claude Code (agent-specific)

> **Claude-Code-specific** — all details below apply only to `claude` (Anthropic's Claude Code CLI).

- Aislopdesk adds hook entries to `~/.claude/settings.json` that report each turn's state: `processing`, `idle`, `awaiting input`.
...
- Aislopdesk installs hooks into `~/.codex/hooks.json`.
- Codex only runs hooks when the `hooks` feature flag is enabled in `~/.codex/config.toml`. Aislopdesk sets `hooks = true` automatically when it installs the hooks.
- If the `hooks` feature is ever disabled, Aislopdesk offers a one-click re-enable (user must then restart Codex in that tab).
...
- Aislopdesk installs a small plugin into `~/.config/opencode/plugins` (not a hooks file).
- Resume uses `opencode --session <id>`.
- Fork uses `opencode --fork --session <id>`.
- **Quirk**: OpenCode does not announce a session when the user switches to it via `/sessions` inside the agent. Aislopdesk only picks up the session once the user sends a message in that session. If a freshly-switched tab appears unlinked, typing something causes it to connect.

### Other/unsupported agents

- Any other coding-agent CLI runs as a plain terminal program in an Aislopdesk tab — no tab badges, notifications, history, resume, or fork.
- Agents that implement the Terminal Resume Protocol (OSC 88) automatically receive resume support in Aislopdesk without a bespoke integration.
- All other agent-aware features are "not implemented yet"; Aislopdesk plans to open them over time, likely through extensions or a similar mechanism.
...
Aislopdesk writes into the agent's own config; it does not expose its own config keys for this feature on this page.

| Key | Default | Effect |
|-----|---------|--------|
| `~/.claude/settings.json` hooks entries | Absent until Aislopdesk installs | Reports Claude Code turn state (processing / idle / awaiting input) to Aislopdesk |
| `~/.codex/hooks.json` hooks entries | Absent until Aislopdesk installs | Reports Codex turn state to Aislopdesk |
| `~/.codex/config.toml` → `hooks = true` | `false` (Codex default) | Must be `true` for Codex to run hooks; Aislopdesk enables this on install |
| `~/.config/opencode/plugins` plugin | Absent until Aislopdesk installs | Wires OpenCode session state and events to Aislopdesk |
...
This page documents which agents Aislopdesk supports and how it hooks into each. Aislopdesk is implementing Claude Code support initially (as specified). Implementation considerations:
...
- **Hook installation into `~/.claude/settings.json`** — in aislopdesk, the Claude Code process runs on the REMOTE macOS HOST, not on the client. Therefore the hook entries must be installed into the host's `~/.claude/settings.json`, not the client machine's. The aislopdesk hostd (or a host-side setup step) must perform this installation. A purely local one-click install (writing to a config file on the same machine as the UI) isn't possible here; this is a host-side setup step, likely handled by the aislopdesk-ctl or a setup flow.
...
- **Session ID discovery** — session IDs need to be read from local transcript files or from hook payloads. In aislopdesk's remote architecture, the hostd must relay session IDs (from hook payloads received over the agent control socket `AISLOPDESK_AGENT_CONTROL=1`) to the client so it can display and act on them.
- **OSC 88 (Terminal Resume Protocol)** — any agent emitting OSC 88 should get resume support without a bespoke integration. Aislopdesk's terminal path (libghostty behind TerminalSurface seam) must pass OSC 88 through; whether libghostty parses it or it needs to be intercepted at the aislopdesk layer needs verification. Mark as NEEDS INVESTIGATION.
