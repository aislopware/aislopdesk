...
func diffPixels(_ a: CGImage, _ b: CGImage) -> (diff: Int, total: Int) {
    ...
    var diff = 0, i = 0
    while i < ba.count {
        let d = abs(Int(ba[i]) - Int(bb[i])) + abs(Int(ba[i + 1]) - Int(bb[i + 1])) +
...
func run() async {
    do {
        ...
        {
            print(
                "  - \"\(w.title ?? "")\" [\(w.owningApplication?.applicationName ?? "")] \(Int(w.frame.width))x\(Int(w.frame.height))",
            )
        }
        guard let win = cands.max(by: { $0.frame.width * $0.frame.height < $1.frame.width * $1.frame.height }) else {
            print(
                "Không tìm thấy cửa sổ app phù hợp. (Mở 1 app như TextEdit/Finder/Safari, hoặc truyền tên: /tmp/cursorstrip safari)",
            )
            exit(1)
        }
        print(
            "Target: \"\(win.title ?? "")\" [\(win.owningApplication?.applicationName ?? "")] \(Int(win.frame.width))x\(Int(win.frame.height)) @\(Int(win.frame.minX)),\(Int(win.frame.minY))",
        )
        ...
        print("Chụp showsCursor=TRUE (con trỏ ở giữa cửa sổ)...")
        let t = try await cap(true)
        savePNG(t, "/tmp/cursor_true.png")
        print("Chụp showsCursor=FALSE...")
        let f = try await cap(false)
        savePNG(f, "/tmp/cursor_false.png")

        let (d, total) = diffPixels(t, f)
        print(String(
            format: "\ndiff(true vs false) = %d pixels (%.4f%% của %d)",
            d,
            Double(d) / Double(max(total, 1)) * 100,
            total,
        ))
        if d > 20 {
            print(
                "=> PASS (auto): true có con trỏ, false KHÔNG → showsCursor=false strip sạch cursor khỏi per-window capture.",
            )
        } else {
        ...
        print("ERROR: \(error)")
        print(
            ">> Nếu lỗi quyền: System Settings > Privacy & Security > Screen Recording > bật cho Terminal, rồi chạy lại.",
        )
        exit(1)
    }
}
