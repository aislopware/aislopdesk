...
func throughput(_ K: Int, _ w: Int, _ h: Int, frames: Int) {
    ...
    guard sessions.count == K else { print("  K=\(K): only \(sessions.count) sessions created")
        return
    }
    let lat = NSLock()
    var allLat: [Double] = []
    ...
    for k in 0..<K {
        ...
        DispatchQueue.global(qos: .userInteractive).async {
            ...
            lat.lock()
            allLat.append(contentsOf: local)
            lat.unlock()
            g.leave()
        }
    }
    ...
    print(
        "  K=\(K) streams \(w)x\(h): agg \(String(format: "%.0f", aggFps)) fps total | per-stream \(String(format: "%.0f", aggFps / Double(K))) fps | encode p50=\(String(format: "%.1f", p50(allLat)))ms p99=\(String(format: "%.1f", p99(allLat)))ms | sustains 30fps/stream? \(aggFps / Double(K) >= 30 ? "YES" : "NO")",
    )
