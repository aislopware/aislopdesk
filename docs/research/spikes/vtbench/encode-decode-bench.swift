...
func testConcurrent(_ w: Int, _ h: Int, _ label: String) {
    ...
    while n < 48 {
        ...
        var encOk = false
        let est = VTCompressionSessionEncodeFrame(
            sess,
            imageBuffer: pb,
            presentationTimeStamp: CMTime(value: Int64(n), timescale: 30),
            duration: .invalid,
            frameProperties: nil,
            infoFlagsOut: nil,
        ) { status, _, sb in
        ...
        if est != noErr || pst != noErr || hw != "HW" {
            print(
                "  [\(label)] session #\(n + 1): prepare=\(pst) encode=\(est) backing=\(hw) -> CEILING (not HW or failed)",
            )
...
func encodeStream(
    ...
    frames: Int,
    lowLatency: Bool,
) -> (encMs: [Double], buffers: [CMSampleBuffer], hw: String) {
    ...
    var s: VTCompressionSession?
    let cst = VTCompressionSessionCreate(
        allocator: nil,
        width: Int32(w),
        height: Int32(h),
        codecType: kCMVideoCodecType_HEVC,
    ...
    guard cst == noErr, let sess = s else {
        print("  encode session create failed: \(cst)")
        return ([], [], "create-fail(\(cst))")
    }
    VTSessionSetProperty(sess, key: kVTCompressionPropertyKey_RealTime, value: kCFBooleanTrue)
...
func testDecode(_ buffers: [CMSampleBuffer], _ w: Int, _ h: Int) {
    ...
    guard st == noErr, let sess = ds else { print("  decode session create failed: \(st)")
        return
    }
    var hw: CFTypeRef?
    ...
    print(
        "  decode backing: \(hwStr) | frames=\(decMs.count) | p50=\(String(format: "%.2f", pct(decMs, 50)))ms p99=\(String(format: "%.2f", pct(decMs, 99)))ms max=\(String(format: "%.2f", decMs.max() ?? 0))ms",
    )
    print(
        "  DECISION (30fps frame=33.3ms): p99 \(pct(decMs, 99) < 33.3 ? "< 1 frame => PASS" : "> 1 frame => investigate")",
    )
}
...
func testLossless(_ w: Int, _ h: Int) {
    ...
    guard let sess = s else { print("  lossless probe: session create failed")
        return
    }
    let q = VTSessionSetProperty(sess, key: kVTCompressionPropertyKey_Quality, value: 1.0 as CFNumber)
    let atc = VTSessionSetProperty(
        sess,
    ...
    print(
        "  Session-B probe: Quality=1.0 status=\(q) | AllowTemporalCompression=false status=\(atc) | Lossless(raw key) status=\(l) (0=accepted)",
    )
    VTCompressionSessionInvalidate(sess)
}
...
print(
    "  low-latency HEVC backing=\(hwLL) | encode p50=\(String(format: "%.2f", pct(encLL, 50)))ms p99=\(String(format: "%.2f", pct(encLL, 99)))ms (one-in-one-out)",
)
if let b = bufsLL.dropFirst(3)
    .first { print("  NALUs per CMSampleBuffer (steady-state P-frame) = \(naluCount(b))  [macOS 26 multi-NALU check]") }
...
print(
    "  constant-quality HEVC backing=\(hwCQ) | encode p50=\(String(format: "%.2f", pct(encCQ, 50)))ms p99=\(String(format: "%.2f", pct(encCQ, 99)))ms",
)
