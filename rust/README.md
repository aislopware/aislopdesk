...
(built by `rust/build-apple.sh`; macOS arm64 by default, `--ios` adds device + simulator
slices) — or, for
...
The boundary is a zero-dependency C header (`aislopdesk_ffi.h`) **generated by cbindgen**
from the `#[repr(C)]` / `extern "C"` surface — Rust is the source of truth for the header
too, and a CI drift-gate regenerates and `cmp`s it (`make check`'s `check-ffi-header` + the
`rust` CI job), failing on any diff. cbindgen is a pinned build/dev tool (0.29.4) that ships
nothing into `libaislopdesk_ffi.a`, so the shipped library stays dependency-free. The choice
of a C ABI over uniffi stands (uniffi's callback interfaces are soft-deprecated and its
serialization is unfit for 60 fps); libsignal and ALVR use the same C-ABI boundary.
...
rust/
  ...
  aislopdesk-core/        the core (rlib, zero runtime dependencies)
    src/
      ...
      gf256.rs            GF(2⁸) field arithmetic (log/exp tables) for Reed–Solomon
      rs_matrix.rs        systematic Reed–Solomon generator/decode matrices
      fec.rs              FecScheme trait + Reed–Solomon parity (m=1 ≡ XOR; m≥2 multi-loss)
  ...
  aislopdesk-ffi/         the C-ABI boundary (rlib + staticlib + cdylib) — the ONLY crate that may use `unsafe`
    cbindgen.toml        cbindgen config (style=both, usize_is_size_t, curated banner)
    src/
      lib.rs             safe-bodied extern "C" surface over the core
      raw.rs             all genuinely-unsafe raw-pointer/heap primitives, isolated here
      gf_neon.rs         aarch64 NEON GF(2⁸) kernels for Reed–Solomon (asm-verified)
      frame_hash.rs      NEON frame-hash kernel
      video/             FFI shims for the video pipeline (reassembler / FEC / controllers)
    include/aislopdesk_ffi.h   C header GENERATED by cbindgen (do NOT hand-edit; drift-gated)
    tests/
...
- **Per-subsystem differential + fuzz tests** across the codec and controller families
  ...
  hostile-input drops).
- **`aislopdesk-ffi`**: 43 tests, `clippy --all-targets` clean, `fmt` clean, and a real C
  smoke test (`-Werror` against the `.a`) proving ABI agreement.
- **Full app suite** green (~2200 tests, 0 failures).
...
- the **C-ABI boundary** (`aislopdesk-ffi`): a wide `extern "C"` surface (~135 functions) —
  `staticlib` for Apple, `cdylib` for a dynamic/Android consumer — with a cbindgen-generated
  header and a real C smoke test. It now covers both paths: the terminal `FrameDecoder`
  (opaque handle), the `WireMessage` codec and mux header, wrap-aware `seq` arithmetic, and
  the whole video pipeline (packetizer, `Reassembler`, Reed–Solomon FEC, the realtime
  controllers, the routers, and the coordinate/geometry/system-dialog math) — with the
  aarch64 NEON GF(2⁸) and frame-hash kernels living in this crate. Every hard boundary
  pattern is exercised: opaque handles, Rust-owned buffers, status codes, null-safety.

Next:

1. **Android build**: `cargo-ndk` producing `.so`s for `aarch64/armv7/x86_64-linux-android`
   plus a JNI shim, behind a foreground service for Doze. The C ABI is already the portable
   spine (ALVR split: Rust owns reassembly/FEC/jitter/ABR/recovery + the server brain; the
