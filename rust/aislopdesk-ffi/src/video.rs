...
// ---------------------------------------------------------------------------------------
// recovery_idr_policy — opaque handle (host delivery-keyed recovery-IDR admission). Driven on the
// host session actor: note_keyframe_sent per encoded keyframe, note_keyframe_delivered per ack,
// decide() per recovery request (NOT per-frame). One owner (AislopdeskVideoHostSession),
// actor-serialized. Env knobs stay resolved Swift-side and cross as the seven Config scalars.
// Verdict crosses as a u8 discriminant. Same "Rust owns the state" boundary as the deduper.
// ---------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------
// pacer_depth_policy — opaque handle (client adaptive pacer-depth v3). Driven on the FramePacer
// (NSLock-serialized): note_arrival per decoded submit, note_present/note_reshow per vsync,
// note_network_late per owd spike, drain_counters per netstats report. The 19-field Config crosses
// as a flat repr(C) struct by value (env resolved Swift-side); GapClass crosses as a u8; the
// drained counters as a flat pair. The internal rings stay Rust-side (never marshaled). One owner
// (FramePacer). Same "Rust owns the state" boundary as the deduper.
// ---------------------------------------------------------------------------------------
