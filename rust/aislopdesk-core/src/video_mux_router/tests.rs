//! Unit tests for the `VideoMuxRouter`: admit/retire/route, the retired-set bound, the
//! drain state machine, and the bootstrap-action decision matrix.
...
impl DatagramHarness {
    ...
    fn feed_media(&mut self, channel_id: u32, channel: VideoChannel, payload: &[u8]) -> Decision {
