...
# Sources/RworkClientUI/iOS/). Without an iOS-triple build those compile only in someone's
# head and rot silently. This script is the explicit, repeatable command that compiles them:
# an unsigned iOS-Simulator build of the iOS app, which links the RworkClientUI package and so
...
# The app target links RworkClientUI + RworkVideoClient, so building it compiles the host +
# every `#if os(iOS)` source it depends on.
build "ClientApp-iOS"

# Belt-and-suspenders: the RworkClientUI scheme is exposed by the project (it carries the iOS
...
if xcodebuild -project "$PROJECT" -list 2>/dev/null | grep -qx '        RworkClientUI'; then
  build "RworkClientUI"
fi
