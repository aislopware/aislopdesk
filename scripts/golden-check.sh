...
# exact bytes. `aislopdesk-corevectors` regenerates the emitted subset from the live native-Swift
...
# an intentional wire change, regenerate with NO AISLOPDESK_* env set and merge surgically — never
# `> golden/golden_vectors.json` (that drops the frozen keys the generator doesn't emit).
set -euo pipefail
cd "$(dirname "$0")/.."

CORPUS="golden/golden_vectors.json"
REGEN="$(mktemp -t aislopdesk-golden.XXXXXX.json)"
trap 'rm -f "$REGEN"' EXIT

# The generator must resolve its compile-time-const defaults — strip any AISLOPDESK_* override.
# shellcheck disable=SC2046 # intentional word-splitting builds the `-u VAR` unset-flag list
env $(env | grep '^AISLOPDESK_' | cut -d= -f1 | sed 's/^/-u /' | tr '\n' ' ') \
  swift run -q aislopdesk-corevectors > "${REGEN}"
