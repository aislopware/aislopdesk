...
WORK="$REPO_ROOT/.work/sysdialog-verify"; mkdir -p "$WORK"
SHOT="$WORK/client-shot.png"; HOSTLOG="$WORK/host.log"; CLIENTLOG="$WORK/client.log"
MEDIA_PORT=9000; CURSOR_PORT=9001
APP_PROC_PAT="sysdialog-verify/DD.*MacOS/Aislopdesk"
HOSTD_PID=""
cleanup() { pkill -f "$APP_PROC_PAT" 2>/dev/null; [[ -n "$HOSTD_PID" ]] && kill "$HOSTD_PID" 2>/dev/null; pkill -x osascript 2>/dev/null; }
trap cleanup EXIT
...
[[ -z "$WID" ]] && { echo "FAIL: no primary window"; echo "$LISTING"; exit 1; }
...
HOSTD_PID=$!; sleep 1
kill -0 "$HOSTD_PID" 2>/dev/null || { echo "FAIL: host died"; cat "$HOSTLOG"; exit 1; }
echo "==> host up (pid $HOSTD_PID)"

# 3. Client: video-autoconnect (primary pane) + FORCE the system-dialog monitor on.
pkill -f "$APP_PROC_PAT" 2>/dev/null; sleep 0.5
...
for _ in $(seq 1 16); do pgrep -f "$APP_PROC_PAT" >/dev/null && break; sleep 0.5; done
pgrep -f "$APP_PROC_PAT" >/dev/null || { echo "FAIL: client never started"; cat "$CLIENTLOG"; exit 1; }
echo "==> client up; waiting for UDP connect…"
for _ in $(seq 1 20); do lsof -nP -iUDP:"$MEDIA_PORT" 2>/dev/null | grep -q "127.0.0.1:$MEDIA_PORT->" && { echo "==> connected ✅"; break; }; sleep 0.5; done
...
echo "==> host TX / dialog evidence:"; grep -iE 'dialog|listSystemDialogs|securityagent' "$HOSTLOG" | tail -8
echo "DONE — read $SHOT"
