...
/// The `@main` entry for the Rwork macOS HOST menu-bar app (issue #5, research §C / §C1).
///
/// A friendly menu-bar front-end over the existing headless host daemon. It runs the SAME
/// terminal ``RworkHost/HostServer`` the `rwork-hostd` CLI runs, but IN-PROCESS on a background
...
struct RworkHostApp: App {
    ...
    @AppStorage("rwork.host.port") private var port: Int = 7779

    var body: some Scene {
        MenuBarExtra("Rwork Host", systemImage: controller.menuBarSymbol) {
